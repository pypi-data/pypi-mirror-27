...
from converter.excel_util import flatten, is_number, is_collection, date_from_int, normalize_year, is_leap_year, \
    ...
    correct_numbers, normalize_fraction, round_excel
...
# Ik heb ook voor *, /, +, - gemaakt (die gebruiken die safe_nr() hieronder)

# TODOEd: test hieronder ook met een "tekst" als val1 of val2 en kijk hoe excel er mee om gaat en zorg dat hetzelfde hieronder
# gebeurd. Note: een string kan ook een geldig getal zijn (= try/catch)
...
def minus(val1, val2):
    ...
    if val2 in ERROR_VALUES:
        return val2
    val1, val2 = correct_numbers(val1, val2, 'minus')
    return round_excel(val1 - val2 if val1 is not None and val2 is not None else val1 if val1 is not None else val2)
...
def plus(val1, val2):
    ...
    if val2 in ERROR_VALUES:
        return val2
    val1, val2 = correct_numbers(val1, val2, 'plus')
    return round_excel(val1 + val2 if val1 is not None and val2 is not None else val1 if val1 is not None else val2)
...
def multiply(val1, val2):
    ...
    if val2 in ERROR_VALUES:
        return val2
    val1, val2 = correct_numbers(val1, val2, 'multiply')
    return round_excel(val1 * val2 if val1 is not None and val2 is not None else 0)
...
def divide(val1, val2):
    ...
    if val2 in ERROR_VALUES:
        return val2
    val1, val2 = correct_numbers(val1, val2, 'divide')
    return round_excel(val1 / val2 if val1 is not None and val2 is not None else 0 if val1 is None else ERR_DIV)
...
def sumif(spreadsheet, cell_address, criteria, sum_range=None):
    ...
    return round_excel(sum(to_values(spreadsheet, rng, True, is_valid_cell)))
...
def average(spreadsheet, cell_address):
    ...
    rng = get_range(spreadsheet, cell_address)
    iterate_cells(spreadsheet, rng, process)
    return round_excel(result / counter)
...
"""
#TODOMatthijs: ook hier moet volgens mij een error value zoals NA of VALUE! worden ge-returned als input zoals item niet correct is (zie index functie).
...
#TODOMatthijs: hoe/wanneer wordt deze gebruikt? Graag ook unit test toevoegen.
...
def vlookup(spreadsheet, item, cell_address, col_index_nr, approximate_match=False):
    #TODOMatthijs: die error waarden teste in excel, wanneer ERR_NA en ERR_VALUE, etc.. teruggeven (net zoiets als bij index, wel eerst unit test maken)
    # Dit geldt ook voor alle functies hier.
    ...
    # We assume the range has been corrected
    # Removed: we can't find it in the cellmap as we changed the cell address to overcome conflict with other cells using
    # the same range
    # rng = get_range(spreadsheet, cell_address)
    rng = cell_address
    item = str(item).lower()
...
"""
#TODOEd: remove/move this function?
...
def hlookup(spreadsheet, item, cell_address, row_index_nr, approximate_match=False):
    ...
    # We assume the range has already been corrected
    #  Removed: we can't find it in the cellmap as we changed the cell address to overcome conflict with other cells using
    # the same range
    rng = cell_address
    # rng = get_range(spreadsheet, cell_address)
    ...
    for col_nr in range(start_col_nr, end_col_nr + 1):
        ...
        if value == item or approximate_match and (
                                ln - 1 == col_nr and value < item or previous is not None and value > item and previous < item):
...
# TODOEd: difference between instanceof() and type() ? (same as in java?)
...
def match(spreadsheet, item, cell_address, match_type=1):
    ...
    def process(col_nr, row_nr, value):
        ...
        if match_type == 1:
            ...
            if not result:
                result = 0
            elif item < value and previous is not None and item > previous:
                result -= 1
                return False
        elif match_type == 0:
            ...
            if previous is not None and value > previous:
                raise Exception(
                    "Array must be in descending order for match type 1, previous: [%s], next: [%s], lookup_value: [%s, range: [%s]" % (
                        previous, next, item, rng))
            if item == value:
                result = col_nr if use_col else row_nr
                return True
...
def xround(spreadsheet, number, num_digits=0):
    ...
    # else:
    #     return normalize_fraction(round(number, num_digits))
...
# TODOEd: what to do with function below ?
def isNa(value):
    try:
        eval(value)
        return False
...
# TODOEd: what to do with functions below, where to put it ?
