...
# TODOMatthijs: what to do with function below ?
def isNa(value):
    try:
        eval(value)
        return False
...
# TODOMathijs: Dit in een eigen excel_functions.py plaatsen in het analyse backend project, in een python folder.
# Dit project wil ik zo algemeen mogelijk houden, in ieder geval de productie code. Via imports van specifieke files ofzo
# kunnen we eigen functies toevoegen. Hoe weet ik nog niet precies, maar ik wil onderstaande niet hier hebben.
