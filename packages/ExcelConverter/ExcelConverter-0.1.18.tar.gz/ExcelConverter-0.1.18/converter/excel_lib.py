...
from converter.excel_util import flatten, is_number, is_collection, date_from_int, normalize_year, is_leap_year, \
    find_corresponding_index, is_nr, ERR_NA, ERR_DIV, ERR_VALUE, rankdata, split_range, split_address, col2num, num2col, \
    evaluate_spreadsheet, to_values, is_valid, iterate_cells, get_range, safe_str as util_safe_str
...
def correct_numbers(val1, val2, fnc = ''):
    try:
        ...
        raise Exception("Incorrect values for function: [%s], val1=[%s], val2=[%s]" % (fnc, val1, val2)) from ex


def minus(val1, val2):
    val1, val2 = correct_numbers(val1, val2, 'minus')
    return val1 - val2 if val1 and val2 else val1 if val1 else val2
...
def xmax(spreadsheet, *args):
    ...
    if (is_collection(args)):
        ...
        if len(items):  # it concerns a collection of numbers
            return max(items) if len(items) else ERR_NA  # TODOMatthijs: ensure to test both these conditions
    ...
    return result if result else ERR_NA
...
# TODOMatthijs: maak een unit test voor deze functie Test alle condities hieronder a.u.b. Dus ook een range in een collection.
# maar is waarschijnlijk lastig
def xmin(spreadsheet, *args):
    result = None
    if (is_collection(args)):
        items = [x for x in args if is_nr(x)]
        if len(items):  # it concerns a collection of numbers
            return min(items) if len(items) else ERR_NA  # TODOMatthijs: ensure to test both these conditions
...
# TODOMatthijs: make a unit test please, ensure it deals with flexible argument (see sum function how it's done)
def right(spreadhseet, text, n):
...
def index(spreadsheet, cell_address, row_nr=None, col_nr=None):
    try:
        ...
        raise Exception("Error in index function, cell_address: [%s], row_nr: [%s], col_nr: [%s]" % (cell_address, row_nr, col_nr))
...
def match(spreadsheet, item, cell_address, match_type=1):
    ...
    def process(col_nr, row_nr, value):
        ...
        if (match_type == 1):
            ...
            if not result:
                result = 0
            elif item < value and previous and item > previous:
                result -= 1
                return False
        elif match_type == 0:
            ...
            if item == value:
                ...
                result = col_nr if use_col else row_nr
            elif value > item and previous and item < previous:
                result += 1
                return True
            elif item < value and previous and item > previous:
                result -= 1
