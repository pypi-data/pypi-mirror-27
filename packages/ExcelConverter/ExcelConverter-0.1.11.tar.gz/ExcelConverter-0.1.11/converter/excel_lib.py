        ...
        if value:
            ...
            if args:
                for ar in args:
                    ...
                    try:
                        val = evaluate_spreadsheet(spreadsheet, sheet, num2col(col2num(start_cl) + col_nr - start_col_nr),
...
def xmax(spreadsheet, *cell_address):
    if (not isinstance(cell_address, str)):
        items = [x for x in cell_address if is_nr(x)]
...
def rounddown(spreadsheet, number, num_digits=0):
    if num_digits != 0:
        raise Exception(
            "Currently only rounding down with 0 digits is supported, number: [%s], digits: [%s]" % (
                number, num_digits))
