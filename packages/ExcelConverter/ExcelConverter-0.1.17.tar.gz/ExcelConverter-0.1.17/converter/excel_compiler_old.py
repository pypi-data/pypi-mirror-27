...
class ExcelCompiler(object):
    ...
    def cell2code(self, cell):
        ...
        if cell.formula:
            expression = shunting_yard(cell.formula or str(cell.value))
            if expression is None or len(expression) == 0:
                raise Exception("Incorrect expression: [%s], cell: [%s], cell.formula: [%s], cell.value: [%s]"
                                % (expression, cell, cell.formula, cell.value))

            #Correct the ranges found concering the excel max col and row
            for e in expression:
                if isinstance(e, RangeNode):
                    pass
                    # e.token.tvalue = correct_range_single(self.excel, e.token.tvalue)

            ast, root = build_ast(expression)
    ...
    def gen_graph_multi(self, ranges):
        ...
        try:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.info("Ranges [%s] expanded into [%s] cell_len", ranges, len(seeds))
    ...
    def gen_graph_intern(self, seeds):
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("Filtered seeds (keeping formulas, strings, ints, floats): [%s] ", len(seeds))
        ...
        while todo:
            ...
            try:
                ...
                raise Exception(
                    "Error compiling/cell2code cell: [%s]" % (c1)) from ex

            if ast is not None:
                # get all the cells/ranges this formula refers to
                # Removed: moved to the correct_range function
                # TODOEdNew
                ...
                for dep in deps:
                    ...
                    if is_range(dep):
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug("Processing dependency [%s] of formula cell: [%s]", dep, c1)
                            logger.debug(("Cell count: [%s]", len(cellmap)))
...
class Spreadsheet(object):
    ...
    def set_value(self, cell, val, is_addr=True):
        if is_addr:
            ...
            if cell.value != val:
                ...
                # TODOReset: enable when reset works correctly
    ...
    def reset_all(self, cell, reset_successors):
        if cell.reset_inprogress:
            # TODOReset: Reset already in progress, that easily happens due to the current reset implementation. See
            # the redmine issue to improve this.
            return
        ...
        try:
            ...
            if not cell.dirty or isinstance(cell, CellRange):
                if (isinstance(cell, Cell)):
                    cell.dirty = True
                # TODOReset: Currently we don't reset ranges, as it will cause too many reset actions.
                # It should be done when the reset issue is solved
                # This seems to result in enough reset, as the unit test seem to work when changing values
                if (reset_successors):
                    list(map(self.reset, self.graph.successors_iter(cell)))
        finally:
    ...
    def evaluate(self, cell, is_addr=True):
        if is_addr:
            ...
            # TODOReset: enable when reset is solved. Below will result in every cell being eavaluated.
