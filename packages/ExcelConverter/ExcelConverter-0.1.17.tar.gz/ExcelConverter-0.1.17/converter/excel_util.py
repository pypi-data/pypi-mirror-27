...
# TODOEd: can't we remove the whole CellRange object? it's not needed anymore I think, it's not used anymore.
...
class Cell(object):
    ...
    def make_cells_multi(excel: object, ranges: object) -> object:
        ...
        for sheet, rng in ranges.items():
            ...
            rng = correct_range_single(excel, rng)  # Ensure the correct sheet is active before correcting the range
    ...
    @staticmethod
    def make_cells(excel: object, rng: object, sheet=None, cellmap=None, parent_ast=None) -> object:
        """
        :rtype: object
        """
        cells = []

        def convert_range(rng, cellmap=None, sheet=None, parent_ast=None):
            cells = []
            ...
            # REMOVED: it already occurs in the compiler when creating the ast node.
            ...
            # TODOEdNew
            rgs = None
            # ads, numrows, numcols = resolve_range(rng, sheet=sheet, parent_ast=parent_ast)
            ...
            if is_collection(rgs):
                ...
                for rn in rgs:
                    ...
                    fs.append(list(flatten(r.Formula)))
                    vs.append(list(flatten(r.value)))
            ...
            try:
                ...
                for ad_row_nr in range(len(ads)):
                    ...
                    for ad_cell_nr in range(len(ad_row)):
                        ...
                        #TODOEdNew: I don't think we can skip it as we it might be added as dependency to link it to the
                        # formula cell, else no reset occurs when this cell is changed.
                        # if (cellmap and a in cellmap):  # already present in cell map
                        #     if logger.isEnabledFor(logging.DEBUG):
                        #         logger.debug(
                        #             "Cell already present in mapping, skip it, range: [%s], cell address: [%s]", rng, a)
                        #     continue
                        ...
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug("Created cell for range: [%s], cell: [%s]", rng, cl)
        ...
        if isinstance(rng, list):  # if a list of cells
            ...
            for cell in rng:
                if is_range(cell):
                    try:
                        cs_in_range, nr, nc, rng = convert_range(cell, cellmap, sheet)
            ...
            if is_range(rng):
                try:
                    ...
                    cells, numrows, numcols, rng = convert_range(rng, cellmap, sheet, parent_ast)
                except Exception as ex:
...
def resolve_range(rng, sheet=None, flatten=False, parent_ast=None):
    ...
    #     sheet += "!"
    #
    # if not sheet: raise Exception("Sheet missing, rng: [%s], parent ast: [%s]" % sheet, parent_ast)
    #
    ...
    #
    # # The start/end row/col should be corrected such that it should be know.
...
def find_range_node(parent_ast, rg):
    # TODOEd: how to improve this import as we can't put this in the top of the file, as it will create a circular
    # reference as this file (and function) is also imported in excel_node that contains RangeNode
...
def find_formula_node(rng_node, parent_ast):
    ...
    if len(formula_node) > 1:
        raise Exception(
            "Only one formula expected: [%s], range: [%s], parent_ast: [%s]" % (formula_node, rng, parent_ast))
...
def resolve_range_simple(rng, sheet, flatten=False):
    ...
    if not start_row or not end_row or not start_col or not end_col:
        ...
        raise Exception("Row/Col start/end missing, range: [%s], start row: [%s], end row: [%s], start col: [%s], end col: [%s]"
