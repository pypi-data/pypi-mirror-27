...
from converter.excel_util import flatten, is_number, date_from_int, normalize_year, is_leap_year, \
    find_corresponding_index, is_nr, ERR_NA, rankdata, split_range, split_address, col2num, num2col, \
...
def index(spreadsheet, cell_address, row_nr, col_nr = 1):
    rng = get_range(spreadsheet, cell_address)
    sheet, start, end = split_range(rng)
    sh, start_col, start_row = split_address(start)
...
def match(spreadsheet, lookup_value, cell_address, match_type=1):
    ...
    sheet, start, end = split_range(rng)
    sh, start_col, start_row = split_address(start)
    return result - int(start_row) + 1 if result else ERR_NA;
...
def roundup(spreadsheet, number, num_digits=0):
    if num_digits != 0:
        raise Exception("Currently only rounding up with 0 digits is supported, number: [%s], digits: [%s]" % (number, num_digits))
