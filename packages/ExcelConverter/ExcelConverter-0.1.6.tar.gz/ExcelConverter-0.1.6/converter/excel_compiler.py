                    ...
                    if is_range(dep):
                        ...
                        if rng_address in cellmap:
                            ...
                            try:
                                ...
                                rng = CellRange(rng_details if rng_details else rng_address, sheet=dep_sheet)
...
class Spreadsheet(object):
    ...
    def set_value(self, cell, val, is_addr=True):
        if is_addr:
            ...
            if cell.value != val:
                ...
                # reference to another cell, setting the value will overwritte the behavior at that moment. We we "read"
                # the value through evaluation, it will evaluate the formula as it's dirty through the reset call in
                # this method, such that it will set/store the value of the reference and overwrite the set value "manually"
                # by calling this method (as if it wasn't set)
    ...
    def reset(self, cell):
        self.reset_all(cell, cell.value is not None)
    ...
    def evaluate(self, cell, is_addr=True):
        ...
        try:
            ...
            if str(ex).startswith("Problem evaluating"):
                ...
                raise Exception("Problem evaluating: [%s] for [%s], [%s]" % (ex, cell.address(), cell.python_expression))
        return cell.value
