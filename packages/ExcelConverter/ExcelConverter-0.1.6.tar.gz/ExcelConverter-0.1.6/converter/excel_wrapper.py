        ...
        if not self.values:
            ...
            for row in self.cellsDO:
                ...
                for cell in row:
                    ...
                    elif cell.data_type == 'e' and cell.value and cell.value == NA:
                        # It's better not to treat the #N/A value as None as it might result in unncessary evaluations
                        # as evaluation always occur in case the value is None. Ensure that it's dependences
                        # reset this cell when required, see: Spreadsheet.set_value()
                        col += (NA,)
