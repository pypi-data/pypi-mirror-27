...
from converter.excel_util import flatten, is_number, is_collection, date_from_int, normalize_year, is_leap_year, \
    ...
    evaluate_spreadsheet, to_values, is_valid, iterate_cells, get_range, safe_str as util_safe_str, correct_numbers
...
# TODOEd: when/where is this used? Should be put in utils?
def value(text):
...
def index_inter(spreadsheet, cell_address, row_nr=None, col_nr=None):
    ...
    sheet, start, end = split_range(rng)
    sh, start_col, start_row = split_address(start)
    if row_nr and col_nr is not None:
...
argument col index.
"""
# TODOEd: move this to utils? (still used?)
