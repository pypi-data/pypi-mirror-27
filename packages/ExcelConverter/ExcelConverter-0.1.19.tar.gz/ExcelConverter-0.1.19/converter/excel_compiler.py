        ...
        while todo:
            ...
            if ast is not None:
                ...
                for dep in deps:
                    ...
                    if is_range(dep):
                        try:
                            # TODOEd: we should be able to only create the rng_details and check if it's already present, if so, no
                            # need to create all cells.
                            ...
                            if cells is None or rng_address in cellmap:
                                ...
                                if logger.isEnabledFor(logging.DEBUG):
                                    logger.debug(
                                        "Skipping range dependency [%s] of formula cell: [%s], already present",
                                        rng_address, dep, c1)
                                continue
                            ...
                            rng = CellRange(rng_details, dep, rng_value, sheet=dep_sheet)
                            ...
                            #         cells, nrows, ncols, rng_details = Cell.make_cells(self.excel, dep, sheet=dep_sheet, # TODOEd: remove cellmap
                            #                                                            cellmap=cellmap, parent_ast=ast)
                            #         # TODOEdNew
                            #         # get the values so we can set the range value
                            #         rng_value = None
                            #         # if nrows == 1 or ncols == 1:
                    ...
                    for c2 in flatten(cells):
                        ...
                        if c2 is not None:
                            ...
                            if c2.address() in cellmap:
                                if logger.isEnabledFor(logging.DEBUG):
                                    logger.debug("Skipping [%s] cell for dependency [%s!%s], already present", c2,
                                                 dep_sheet, dep)
                            else:
                                if c2.formula:
                                ...
                                # save in the cellmap
                                if logger.isEnabledFor(logging.DEBUG):
                                    logger.debug("Adding cell [%s] to cellmap for dependency [%s!%s]", c2, dep_sheet,
                                                 dep)
...
class Spreadsheet(object):
    ...
    def create_exception(self, msg, rng):
        ...
        # plus(eval_cell("Vergelijking!E131"),eval_cell("Vergelijking!I131"))
        # if all([eval_cell("Vergelijking!F131")>= (0 if 0 is not None else float(\\'inf\\')),
        #         eval_cell("Vergelijking!J131")==0,
        #         not(self, eval_cell("Vergelijking!I131")==9999)])
        # else 9999
