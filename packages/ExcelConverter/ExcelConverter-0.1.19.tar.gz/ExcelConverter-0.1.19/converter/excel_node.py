...
from .excel_util import resolve_range, is_range, split_range, split_address, get_linest_degree
...
class OperatorNode(ASTNode):
    ...
    def emit(self, ast, context=None):
        ...
        ss = None
        if op == "**":
            if parent is not None and parent.tvalue.lower() == "linest":
                return args[0].emit(ast, context=context)
        ...
        if op == "<" or op == "<=":
            ...
            ss = to_safe_str(args[0].emit(ast, context=context)) + op + to_safe_str(args[1].emit(ast, context=context))
            # ss = 'safe_str(' + args[0].emit(ast, context=context) + ')' + op + 'safe_str(' + args[1].emit(ast, context=context) + ')'
        elif op == '*':
            ss = surround('multiply', args[0].emit(ast, context=context), args[1].emit(ast, context=context))
        elif op == '/':
            ss = surround('divide', args[0].emit(ast, context=context), args[1].emit(ast, context=context))
        elif op == '+':
            ss = surround('plus', args[0].emit(ast, context=context), args[1].emit(ast, context=context))
        elif op == '-':
            ss = surround('minus', args[0].emit(ast, context=context), args[1].emit(ast, context=context))
        else:
...
class FunctionNode(ASTNode):
    ...
    def emit(self, ast, context=None):
        ...
        if fun == "atan2":
            ...
            expr = "sin(%s,%s)" % (args[0].emit(ast, context=context), args[0].emit(ast, context=context))
        elif fun == "if":
            # inline the if
            if len(args) == 2:
            ...
            if len(args) > 1: raise Exception("Too many arguments for NOT function (1 expected): [%s], args: [%s]", len(args), args)
