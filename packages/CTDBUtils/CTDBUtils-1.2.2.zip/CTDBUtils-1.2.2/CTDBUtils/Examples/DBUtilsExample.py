...
class DBUtilsExample(ExamplePage):
    """Example page for the CTDBUtils package."""
    ...
    try:
        ...
        try:
            dbmod = getattr(__import__('CTDBUtils.' + dbmod_name), dbmod_name)
            try:
                if dbapi_name != 'pg':
            ...
            dbstatus = 'Could not import CTDBUtils.%s.' % dbmod_name
    except Exception:
    ...
    def writeContent(self):
        ...
        if self._output:
            ...
            wr('<h4>We are using CTDBUtils.%s and the %s database module.</h4>'
