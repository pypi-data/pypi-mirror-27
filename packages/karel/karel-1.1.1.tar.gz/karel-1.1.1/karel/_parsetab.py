...
_lr_productions = [
  ...
  ('prog -> DEF RUN LPAREN RPAREN LBRACE stmt RBRACE','prog',7,'p_prog','parser.py',109),
  ('stmt -> while','stmt',1,'p_stmt','parser.py',114),
  ('stmt -> repeat','stmt',1,'p_stmt','parser.py',115),
  ('stmt -> stmt_stmt','stmt',1,'p_stmt','parser.py',116),
  ('stmt -> action','stmt',1,'p_stmt','parser.py',117),
  ('stmt -> if','stmt',1,'p_stmt','parser.py',118),
  ('stmt -> ifelse','stmt',1,'p_stmt','parser.py',119),
  ('stmt_stmt -> stmt SEMI stmt','stmt_stmt',3,'p_stmt_stmt','parser.py',125),
  ('if -> IF LPAREN cond RPAREN LBRACE stmt RBRACE','if',7,'p_if','parser.py',133),
  ('ifelse -> IFELSE LPAREN cond RPAREN LBRACE stmt RBRACE ELSE LBRACE stmt RBRACE','ifelse',11,'p_ifelse','parser.py',139),
  ('while -> WHILE LPAREN cond RPAREN LBRACE stmt RBRACE','while',7,'p_while','parser.py',145),
  ('repeat -> REPEAT LPAREN cste RPAREN LBRACE stmt RBRACE','repeat',7,'p_repeat','parser.py',154),
  ('cond -> cond_without_not','cond',1,'p_cond','parser.py',163),
  ('cond -> NOT cond_without_not','cond',2,'p_cond','parser.py',164),
  ('cond_without_not -> FRONT_IS_CLEAR LPAREN RPAREN','cond_without_not',3,'p_cond_without_not','parser.py',174),
  ('cond_without_not -> LEFT_IS_CLEAR LPAREN RPAREN','cond_without_not',3,'p_cond_without_not','parser.py',175),
  ('cond_without_not -> RIGHT_IS_CLEAR LPAREN RPAREN','cond_without_not',3,'p_cond_without_not','parser.py',176),
  ('cond_without_not -> MARKERS_PRESENT LPAREN RPAREN','cond_without_not',3,'p_cond_without_not','parser.py',177),
  ('cond_without_not -> NO_MARKERS_PRESENT LPAREN RPAREN','cond_without_not',3,'p_cond_without_not','parser.py',178),
  ('action -> MOVE LPAREN RPAREN','action',3,'p_action','parser.py',184),
  ('action -> TURN_RIGHT LPAREN RPAREN','action',3,'p_action','parser.py',185),
  ('action -> TURN_LEFT LPAREN RPAREN','action',3,'p_action','parser.py',186),
  ('action -> PICK_MARKER LPAREN RPAREN','action',3,'p_action','parser.py',187),
  ('action -> PUT_MARKER LPAREN RPAREN','action',3,'p_action','parser.py',188),
  ('cste -> INT','cste',1,'p_cste','parser.py',196),
]
