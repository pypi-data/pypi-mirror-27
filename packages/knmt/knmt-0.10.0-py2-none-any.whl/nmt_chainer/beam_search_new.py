...
def ensemble_beam_search(model_ensemble, src_batch, src_mask, nb_steps, eos_idx, beam_width = 20, need_attention = False,
    ...
    return ensemble_beam_search_from_conditionalized_cell(dec_cell_ensemble, nb_steps, eos_idx, beam_width, need_attention,
                         force_finish,
                         prob_space_combination)

def ensemble_beam_search_from_conditionalized_cell(decoding_cell_ensemble, nb_steps, eos_idx, beam_width = 20, need_attention = False,
                         force_finish = False,
                         prob_space_combination = False):
...
#     assert mb_size == 1
    ...
    previous_states_ensemble = [None] * len(decoding_cell_ensemble)
    
    xp = decoding_cell_ensemble[0].xp
