...
def append_file_content_to(source, destination, wildcard, **kwargs):
...
def create_files(files_to_create, recipe, resource_path, resource_name):
    for file_to_create in files_to_create:
        ...
        with open(file_path, "w") as f:
            resource_key = '%s_name' % (recipe)
            substitution_dict = {resource_key: resource_name}
            f.write(replace_content(recipe_template_path, substitution_dict))
