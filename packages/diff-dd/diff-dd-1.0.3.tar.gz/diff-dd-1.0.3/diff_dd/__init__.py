# -*- coding: utf-8 -*-
from diff_dd.module import CreateDiff, ResoreDiff
...
def main():
    ...
    cli_parser.add_argument('-d', '--df', type=str, default=sys.stdin, help='Diff File. For backup mode: changed file. For restore mode: binary diff file')
    cli_parser.add_argument('-o', '--of', type=str, default=sys.stdout, help='Output File. For backup mode: binary diff. For restore mode: restored file')
    cli_parser.add_argument('-m', '--mode', choices=['backup', 'restore'], default='backup', type=str, required=False, help='Working mode')
    ...
    if cli.mode == 'backup':
        ...
        differ_retore = ResoreDiff(iffd=iffd, dffd=dffd, offd=offd, block_size=cli.block_size)
        differ_retore.start()
