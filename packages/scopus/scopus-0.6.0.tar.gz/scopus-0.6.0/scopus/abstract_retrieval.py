...
CITATION_OVERVIEW_DIR = os.path.expanduser('~/.scopus/abstracts_retrieval')
...
class CitationOverview(object):
    ...
    def __init__(self, eid, refresh=False):
        ...
        url = "http://api.elsevier.com/content/abstract/citations/{}".format(eid)
        res = get_content(qfile, url=url, refresh=refresh, accept='json')
        # can this be empty?
        data = loads(res.decode('utf-8'))['abstracts-retrieval-response']  # might cause troubles
