    ...
    def _create_instance_backup(self, domain):
        ...
        for disk in domain_disks:
            if (disk['source_type'] == 'dev') and (disk['path'] not in self.exclude_disks):
                ...
                offd = gzip.open('{0}/{1}/{2}.img.ddd.gz'.format(self.dest, domain.name(), os.path.basename(disk['path'])), 'wb')
                differ = ebt_files.ddd.CreateDiff(iffd=iffd, dffd=dffd, offd=offd, block_size=4096)
                differ.start()
                iffd.close()
                dffd.close()
                offd.close()
