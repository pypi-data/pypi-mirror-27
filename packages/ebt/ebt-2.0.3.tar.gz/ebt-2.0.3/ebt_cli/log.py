...
class Configurator:
    def __init__(self):
        ...
        log.basicConfig(format=self.msg_format, datefmt=self.date_format, level=log.DEBUG)
    ...
    def add_syslog_handler(self, address='/dev/log', facility='user'):
        syslog_handler = log.handlers.SysLogHandler(address=address, facility=facility)
    ...
    def add_file_handler(self, path, max_size):
        file_handler = log.handlers.RotatingFileHandler(path, maxBytes=max_size)
