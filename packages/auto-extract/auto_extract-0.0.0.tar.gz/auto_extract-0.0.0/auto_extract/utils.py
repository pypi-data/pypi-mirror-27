...
def extract_domain(url):
    ...
    if protocol == 'file:':
        ...
        protocol += '//'
    full = protocol + ".".join([x for x in tld if x])
    return full, tld.domain + tld.suffix
...
def extract_headings(tree):
    headings = [x.text_content() for x in tree.xpath("//h1")]
    if not headings:
        headings = [x.text_content() for x in tree.xpath("//h2")]
    if not headings:
        headings = [x.text_content() for x in tree.xpath("//h3")]
    return headings
