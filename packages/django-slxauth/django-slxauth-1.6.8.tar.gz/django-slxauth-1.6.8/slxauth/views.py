...
def get_callback_uri(request):
    if 'next' in request.GET and request.GET['next'].endswith('/'):
        request.GET['next'] = request.GET['next'][:-1] # strip final / to sanitize next url

    param = '?%s' % request.GET.urlencode()
    callback_uri = request.build_absolute_uri('%s%s' % (reverse('slxauth:callback'), param))
    logger.debug('generated callback uri: %s' % callback_uri)
    return callback_uri
...
class CallbackView(View):
    ...
    def get(self, request):
        ...
        if authresponse and authresponse.status_code == 200:
            ...
            if 'access_token' in data and 'expires_in' in data and 'refresh_token' in data:
                ...
                if login_using_token(request, access_token):
                    if 'next' in request.GET:
                        ...
                        response = HttpResponseRedirect('%s/' % settings.SCRIPT_NAME)
            ...
            logger.debug('received response %s from auth service. can\'t login' % authresponse.status_code)
