...
class CorexText_HyperParams(hyperparams.Hyperparams):
    ...
    n_hidden = hyperparams.Uniform(lower = 0, upper = 100, default = 10, q = 1, description = 'number of topics')
...
class CorexText(UnsupervisedLearnerPrimitiveBase[Input, Output, CorexText_Params, CorexText_HyperParams]):  #(Primitive):
    ...
    def _fit_transform(self, inputs : Input, timeout : float = None, iterations : int = None) -> CallResult[Output]: # TAKES IN DF with index column
        #self.columns = list(X)
        #X_ = X[self.columns].values # useless if only desired columns are passed
