...
class KadmosGraph(nx.DiGraph, EquationMixin, VistomsMixin):
    ...
    def merge_function_modes(self, *args, **kwargs):
        ...
        if 'version' in kwargs:
            ...
            version_str = ''
        for arg in args[1:]:
            node_name = args[0] + '[' + arg + ']' + instance_str + version_str
...
def _read_io_xml_file(file_path, mode, xsd_check):
    ...
    for el in tree.iter():
        ...
        if not el.getchildren():  # if leaf node
            ...
            if _check_execution_mode_for_element(el, tree, file_path, mode): # if right mode specification
...
def _check_execution_mode_for_element(element, tree, file, req_mode):
    """
