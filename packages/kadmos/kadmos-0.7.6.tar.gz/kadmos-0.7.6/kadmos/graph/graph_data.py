...
class DataGraph(KadmosGraph):
    ...
    def mark_as_constraint(self, node, operator, reference_value):
    ...
    def mark_as_constraints(self, nodes, operators, reference_values):
...
class RepositoryConnectivityGraph(DataGraph):
    ...
    def create_mathematical_problem(self, n_disciplines, coupling_strength, n_global_var=None, n_local_var=None,
        ...
        mathematical_problem['coupling_strength'] = coupling_strength
        ...
        # Create s-matrix: positive scalars used to calculate constraint values
        if s is None:
            ...
            for coupling_var in range(n_coupling_var[discipline]):
                ...
                for index, v in enumerate(values):
                    if index != coupling_var:
                        assert v == 0, 'Output variable y{0}{1} cannot be an input to discipline ' \
        ...
        self.add_node('F', category='function')
        for constraint in range(n_global_constraints):
            self.add_node('G{0}'.format(constraint + 1), category='function')
        ...
        for discipline in range(n_disciplines):
            for local_var in range(n_local_var[discipline]):
                self.add_node('/data_schema/local_design_variables/x{0}{1}'.format(discipline + 1, local_var + 1),
                              category='variable',
        ...
        for discipline in range(n_disciplines):
            for coupling_var in range(n_coupling_var[discipline]):
                self.add_node('/data_schema/coupling_variables/y{0}{1}'.format(discipline + 1, coupling_var + 1),
                              category='variable',
                              label='y{0}{1}'.format(discipline + 1, coupling_var + 1))

        # Objective variable node is defined
        self.add_node('/data_schema/objective/f',
                      category='variable',
                      label='f')

        # Constraint variable nodes are defined
        for constraint in range(n_global_constraints):
            self.add_node('/data_schema/constraints/g{0}'.format(constraint + 1),
                          category='variable',
            ...
            for local_var in range(n_local_var[local_var_disc]):
                for discipline in range(n_disciplines):
                    values = D[sum(n_coupling_var[:discipline]):sum(n_coupling_var[:discipline + 1]),
                               sum(n_local_var[:local_var_disc]) + local_var]
                    if not all(v == 0 for v in values):
                        self.add_edge('/data_schema/local_design_variables/x{0}{1}'.format(local_var_disc + 1,
                                      local_var + 1), 'D{0}'.format(local_var_disc + 1))
                self.add_edge('/data_schema/local_design_variables/x{0}{1}'.format(local_var_disc + 1, local_var + 1),
                              'F')
                for constraint in range(n_global_constraints):
                    self.add_edge('/data_schema/local_design_variables/x{0}{1}'.format(local_var_disc + 1,
                                  local_var + 1), 'G{0}'.format(constraint + 1))
            ...
            for coupling_var in range(n_coupling_var[coupling_var_disc]):
                for discipline in range(n_disciplines):
                    ...
                    if not discipline == coupling_var_disc and not all(v == 0 for v in values):
                        self.add_edge(
                            '/data_schema/coupling_variables/y{0}{1}'.format(coupling_var_disc + 1, coupling_var + 1),
                            'D{0}'.format(discipline + 1))
                self.add_edge('/data_schema/coupling_variables/y{0}{1}'.format(coupling_var_disc + 1, coupling_var + 1),
                              'F')
                for constraint in range(n_global_constraints):
        ...
        for discipline in range(n_disciplines):
            for coupling_var in range(n_coupling_var[discipline]):
                self.add_edge('D{0}'.format(discipline + 1),
                              '/data_schema/coupling_variables/y{0}{1}'.format(discipline + 1, coupling_var + 1))
        self.add_edge('F', '/data_schema/objective/f')
        for constraint in range(n_global_constraints):
            self.add_edge('G{0}'.format(constraint + 1), '/data_schema/constraints/g{0}'.format(constraint + 1))
        ...
        for discipline in range(n_disciplines):
            for output_var in range(n_coupling_var[discipline]):
                ...
                for local_var_disc in range(n_disciplines):
                    for local_var in range(n_local_var[local_var_disc]):
                        if D[sum(n_coupling_var[:discipline]) + output_var][
                            ...
                            equation += '-{0}*x{1}{2}'.format(D[sum(n_coupling_var[:discipline]) + output_var][
                                                                sum(n_local_var[:local_var_disc]) + local_var],
                    ...
                    for coupling_var in range(n_coupling_var[coupling_var_disc]):
                        if B[sum(n_coupling_var[:discipline]) + output_var][sum(n_coupling_var[:coupling_var_disc]) +
                           coupling_var] != 0 and (discipline, output_var) != (coupling_var_disc, coupling_var):
                            equation += '-{0}*y{1}{2}'.format(B[sum(n_coupling_var[:discipline]) + output_var][sum(
                ...
                self.add_equation(['D{0}'.format(discipline + 1),
                                  '/data_schema/coupling_variables/y{0}{1}'.format(discipline + 1, output_var + 1)],
                                  equation, 'Python')
                self.add_equation(['D{0}'.format(discipline + 1),
                                  '/data_schema/coupling_variables/y{0}{1}'.format(discipline + 1, output_var + 1)],
                                  equation, 'LaTeX')
            ...
            for discipline in range(n_disciplines):
                for local_var in range(n_local_var[discipline]):
                    ...
                    if I[constraint][sum(n_local_var[:discipline]) + local_var] != 0:
                        constraint_eq += '+{0}*x{1}{2}'.format(I[constraint][sum(n_local_var[:discipline]) + local_var],
                                                               discipline + 1, local_var + 1)
            for discipline in range(n_disciplines):
                for coupling_var in range(n_coupling_var[discipline]):
                    if J[constraint][sum(n_coupling_var[:discipline]) + coupling_var] != 0:
                        constraint_eq += '+{0}*y{1}{2}'.format(
                            J[constraint][sum(n_coupling_var[:discipline]) + coupling_var], discipline + 1,
                            coupling_var + 1)
            constraint_eq += '-{0}'.format(s[constraint])
            self.add_equation('G{0}'.format(constraint + 1), constraint_eq, 'Python')
            self.add_equation('G{0}'.format(constraint + 1), constraint_eq, 'LaTeX')
        ...
        if write_problem_to_textfile:
            file = open("Problem_formulation.txt", "w")
            file.write('Number of disciplines: ' + str(n_disciplines) + '\n')
            file.write('Number of global variables: ' + str(n_global_var) + '\n')
            file.write('Number of local variables: ' + str(n_local_var) + '\n')
            file.write('Number of coupling variables: ' + str(n_coupling_var) + '\n')
            file.write('Number of global constraints: ' + str(n_global_constraints) + '\n')
            file.write('B-matrix: ' + str(B) + '\n')
            file.write('C-matrix: ' + str(C) + '\n')
            file.write('D-matrix: ' + str(D) + '\n')
            file.write('H-matrix: ' + str(H) + '\n')
            file.write('I-matrix: ' + str(I) + '\n')
            file.write('J-matrix: ' + str(J) + '\n')
            file.write('s-matrix: ' + str(s) + '\n')
            file.close()
