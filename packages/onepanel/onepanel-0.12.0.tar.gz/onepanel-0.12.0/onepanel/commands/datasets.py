...
class Dataset:
    ...
    EXCLUSIONS = os.getenv('GIT_EXCLUSIONS', '.onepanel/dataset').split(', ')
...
def create_dataset(ctx, account_uid, home):
    ...
    if Dataset.exists_remote(dataset_uid, remote_list):
        if Dataset.exists_local(home):
            click.echo('Dataset already initialized')
        else:
            dataset = Dataset(account_uid, dataset_uid)
            dataset.save(home)
            git = GitWrapper()
            git.init(home, account_uid, dataset_uid)
        ...
        if Dataset.exists_local(home):
            can_create = click.confirm(
                'Dataset does not exists for {}, create the dataset and overwrite?'
                .format(account_uid))
...
@click.option('--account',
              help='Dataset is created and initialized for this account_uid.')
@click.pass_context
@login_required
def datasets_init(ctx, account):
    home = os.getcwd()
    ...
    if create_dataset(ctx, account, home):
...
@click.pass_context
@login_required
def datasets_create(ctx, name, account):
