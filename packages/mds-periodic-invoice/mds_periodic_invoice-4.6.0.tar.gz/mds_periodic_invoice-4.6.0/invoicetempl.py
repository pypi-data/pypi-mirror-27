...
class InvoiceTemplate(ModelSQL, ModelView):
    ...
    def on_change_invtype(self):
        ...
        journals = Journal.search([
                    ('type', '=', INV_TYPE2JOURNAL.get(self.invtype or 'out', 'revenue')),
                    ], limit=1)
        if journals:
    ...
    def get_account_payment_term(self):
        ...
        if self.party:
            if self.invtype == 'out':
                ...
                if (self.invtype == 'out') and self.party.customer_payment_term:
                    self.payment_term = self.party.customer_payment_term
            elif self.invtype == 'in':
                self.account = self.party.account_payable
                if (self.invtype == 'in') and self.party.supplier_payment_term:
                    self.payment_term = self.party.supplier_payment_term
        if self.company and (self.invtype in ['out', 'in']):
            if (self.invtype == 'out') and self.company.party.customer_payment_term:
                self.payment_term = self.company.party.customer_payment_term
            elif (self.invtype == 'in') and self.company.party.supplier_payment_term:
                        ...
                        ['OR',
                            ...
                            ('group.kind', 'in',
                                If(Bool(Eval('_parent_invoice')),
                                    If(Eval('_parent_invoice', {}).get('invtype') == 'out',
                                        ['sale', 'both'],
                                        ['purchase', 'both']
                                    ),
                                    If(Eval('inv_type') == 'out',
                                        ['sale', 'both'],
                                        ['purchase', 'both']
                                    )
                                )
        ...
        cls.account.domain = [
            ...
            If(Bool(Eval('_parent_invoice')),
                If(Eval('_parent_invoice', {}).get('invtype') == 'out',
                    ...
                    If(Eval('_parent_invoice', {}).get('invtype') == 'in',
