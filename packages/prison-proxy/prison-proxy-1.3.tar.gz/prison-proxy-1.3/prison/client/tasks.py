...
from prison.client.common import (duel_with_sucker,
                                  send_presents,
...
def _do_fight(user_id, auth_key, repeats):
    for repeat in range(0, repeats):
        duel_with_sucker(user_id, auth_key)


def _collect_rewards(user_id, auth_key):
    result = collect_building_rewards(user_id, auth_key)
...
def complete_tasks(user_id, auth_key):
    ...
    for task in tasks:
        if task["id"] == SEND_PRESENT and task["count"] < task["amount"]:
            _send_presents(user_id, auth_key)
            continue

        if task["id"] == DO_SPIT and task["count"] < task["amount"]:
            _spit(user_id, auth_key)
            continue

        if task["id"] == GOT_RESPECT and task["count"] < task["amount"]:
            # do action?
            continue

        if task["id"] == GOT_OFFICE and task["count"] < task["amount"]:
            _collect_office(user_id, auth_key)
            continue

        if task["id"] == DO_FIGHT and task["count"] < task["amount"]:
            repeats = (task["amount"] - task["count"])
            _do_fight(user_id, auth_key, repeats=repeats)
            continue

        if task["id"] == GOT_REWARDS and task["count"] < task["amount"]:
            _collect_rewards(user_id, auth_key)
            continue
