...
def adfs_init(app):
    ...
    # try:
    #     with open('{0}_x'.format(app.config.get('ADFS_CERT_LOCATION'))) as cert:
...
def _get_certs_from_adfs(app, retry=False):
    ...
    app.logger.info('adfs primary cert:')
    if _public_key:
        app.logger.info(_public_key)
    else:
        app.logger.info('unable to retrieve cert from adfs')

    app.logger.info('adfs secondary cert:')
    if _public_key_x:
        app.logger.info(_public_key_x)
    else:
        app.logger.info('unable to retrieve additional cert from adfs')
