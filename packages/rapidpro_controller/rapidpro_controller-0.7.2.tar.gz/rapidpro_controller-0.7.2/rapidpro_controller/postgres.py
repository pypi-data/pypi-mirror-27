...
def configure_postgres_slave():
    ...
    def retrieve_backup():
        ...
        # copy back the pg_hba.conf file
        src = Path(os.path.join(POSTGRES_DATABACKUP_DIR, 'pg_hba.conf'))
        dst = Path(os.path.join(POSTGRES_DATA_DIR, 'pg_hba.conf'))
        if not dst.exists():
            try:
                src.copy2(dst)
                dst.chown(POSTGRES_USER, POSTGRES_GROUP)
            except:
                return False
