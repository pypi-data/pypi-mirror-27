...
import math
import functools
from typing import Any, Tuple, Dict, Iterable, Callable, Optional


def _get_reasonable_process_count(cpu_count: int=multiprocessing.cpu_count()) -> int:
...
def _thread_run_user_function(user_function: Callable,
...
def _thread_run_user_iterator(user_function: Callable,
                              result_queue: JoinableQueue,
...
def parallel_map(data: Iterable,
...
def parallel_iterator(data: Iterable,
                      user_iterator: Callable,
                      args: Optional[Tuple]=None,
                      kwargs: Optional[Dict]=None,
                      process_count: Optional[int]=None):
    """
    ...
    count_queue_results = [count_queue.get() for _ in range(process_count)]
    total_result_count = sum(count_queue_results)

    # Drain any items that might still be in the result_queue so that we can safely join the threads.
    final_results = []
    for _ in range(total_result_count - result_count):
        final_results.append(result_queue.get())

    # The queue is empty and the threads have all signaled that they're done
    for process in processes:
        process.join()

    # In case we got any results when draining the queue, we need to yield them here.
