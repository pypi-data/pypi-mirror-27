...
#        self.conv_layer_out.append(self.batch_dropout(self.h_pool[layer_num]))  
     ...
     def fc_layer(self,layer_num):
        if layer_num ==0:
            convout=self.conv_layer_out[len(self.conv_layer_out)-1]
            self.fc_layer_out.append(tf.reshape(convout, [self.batch_size,self.convout[1]*self.convout[2]*self.convout[3]]))
            self.w_fc.append(self.weight_init([self.convout[1]*self.convout[2]*self.convout[3], self.n_hidden[layer_num]]))
        else:
