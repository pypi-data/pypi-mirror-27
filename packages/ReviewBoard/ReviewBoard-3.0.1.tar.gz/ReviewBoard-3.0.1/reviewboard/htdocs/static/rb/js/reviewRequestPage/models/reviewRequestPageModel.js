...
RB.ReviewRequestPage.ReviewRequestPage = RB.ReviewablePage.extend({
     ...
     *         The updates payload.
     */
    _processUpdates: function _processUpdates(rsp) {
        var len = rsp.length;
        var i = 0;
        ...
        while (i < len) {
            ...
            var j = rsp.indexOf('\n', i);
            var metadataLen = parseInt(rsp.substr(i, j - i), 10);
            i = j + 1;

            /* Read the metadata content and parse it. */
            var metadata = JSON.parse(rsp.substr(i, metadataLen));
            i += metadataLen;

            /* Read the length of the HTML content. */
            j = rsp.indexOf('\n', i);
            var htmlLen = parseInt(rsp.substr(i, j - i), 10);
            i = j + 1;

            /* Read the HTML content. */
            var html = rsp.substr(i, htmlLen);
            i += htmlLen;
