...
class FilterData(dict):
    ...
    def _entry_filter(self, entry, path_list):
        ...
        while remaining:
            ...
            if len(current_path['path']) <= 1:
                ...
                if not self._value_check(entry, current_path):
                    return False
        ...
        for group in path_groups:
            ...
            if isinstance(entry, list):
                ...
                for end_path in end_group:
                    if not self._value_check(entry, end_path):
                        return False
    ...
    def _value_check(self, entry, path):
        ...
        try:
            ...
            self._add_to_missing_key(entry_index)
            return False
        ...
        try:
            ...
            self._add_to_regex_errors(entry_index)
            return False

        ## if list, check against all
        if isinstance(entry, list):
            for item in entry:
                if re.search(regex_value, str(item)) or filter_datetime(value, item):
                    return True
            return False
