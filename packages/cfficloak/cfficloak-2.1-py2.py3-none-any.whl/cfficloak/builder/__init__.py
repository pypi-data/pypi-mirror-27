...
def generate_definitions_file(definitions_file, libname, comments_callback=None):
    ...
    open(_skelfile, 'a').close()  # Ensure file exists
    with open(_skelfile, 'r') as skelfile:
        existing = skelfile.read().split("# Functions")[-1].strip()

    with open(_skelfile, 'w') as skelfile:
