...
class Client(object):
    ...
    def _wrap_request(payload):
        ...
        def wrapper(self, path, method, params=None, timeout=None):
            _log.info("@@@ Wrapper: %s %s '%s' %s" % (method, path, params, timeout))
            response = False
            ...
            while not response:
                _log.info("@@@ start of loop: %s" % path)
                ...
                try:
                    ...
                    if (isinstance(params, dict) and
                        ...
                        _log.error("Watch timed out. Raising error")
                    ...
                    if self._allow_reconnect:
                        ...
                        _log.error("Reconnection disabled, giving up.")
                    ...
                    _log.error("Unexpected request failure, re-raising.")
                    raise
                ...
                if some_request_failed:
                    if not self._use_proxies:
                        ...
                        self._machines_cache.remove(self._base_uri)
            _log.info("@@@ returning with response: %s '%s'" % (
                      response.status, str(response.data)[:80]))
    ...
    def _handle_server_response(self, response):
        ...
        if response.status in [200, 201]:
            _log.info("@@@ _handle_server_response OK")
            return response

        else:
            _log.info("@@@ _handle_server_response issue: %s" % response.status)
