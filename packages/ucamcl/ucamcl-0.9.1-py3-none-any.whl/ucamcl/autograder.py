...
def autograder(url, course, section=None):
    ...
    grader = Autograder(url=url, prefix=prefix, key=key)
    ...
    if not (use_ipython and get_ipython()):
        ...
        <div>
        <span id='{id1}'>
        <a href="{url}?{show}&signing_key={signing_key}" target='grader_login'>log in</a>
        </span>
        </div>
...
class Autograder(object):
    ...
    def subsection(self, prefix):
        return Autograder(self.url, self.prefix + prefix, self.key)
    def for_course(self, course, section=None):
        prefix = '/'.join(s for s in [course,section] if s is not None)
        return Autograder(self.url, prefix, self.key)
    ...
    def set_question(self, label, parameters, answer, description=None, tag=None):
...
class SetQuestion:
    def __init__(self, qspec):
        ...
        self.id = qspec['id']
        self._parameters = lib_json.loads(qspec['parameters'])
