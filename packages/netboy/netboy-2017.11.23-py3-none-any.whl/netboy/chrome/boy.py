...
class ChromeBoy:
    ...
    def new_page(self, **kwargs):
        ...
        try:
            ...
            print(e, type(e), 'error when get value: {}'.format(url))
            r = {'url': url, 'state': 'error', 'mode': kwargs.get('mode'), 'spider': 'chrome'}
            self._tabs[c] = r
    ...
    def chrome(self):
        try:
            browser_socket = websocket.create_connection(self._browser_url,
                                                         # + '/9d220c5c-7eba-4927-a95c-dfd56c689779',
                                                         timeout=self._socket_timeout)
            ci = ChromeInterface(browser_socket, self._host, self._port), browser_socket
        except Exception as e:
            print(e, type(e), 'error when get chrome interface')
            try:
                browser_socket = websocket.create_connection(self._browser_url,
                                                             # + '/9d220c5c-7eba-4927-a95c-dfd56c689779',
                                                             timeout=self._socket_timeout * 2)
                ci = ChromeInterface(browser_socket, self._host, self._port), browser_socket
            except Exception as e:
