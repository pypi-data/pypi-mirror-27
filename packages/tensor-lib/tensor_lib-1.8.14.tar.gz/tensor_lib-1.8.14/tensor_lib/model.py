...
class Model:
  ...
  def train(self, data_label_pair, add_summary=False):
    ...
    with self.sess.as_default():
      if add_summary:
        _, loss, summary = self.sess.run([self.train_step, self.losses, self.merged], feed_dict={self.input: data[0], self.ground_truth: data[1], self.is_training: True })
        self.train_writer.add_summary(summary, self.global_step.eval())
      else:
        _, loss = self.sess.run([self.train_step, self.losses], feed_dict={self.input: data[0], self.ground_truth: data[1], self.is_training: True })
      return loss
