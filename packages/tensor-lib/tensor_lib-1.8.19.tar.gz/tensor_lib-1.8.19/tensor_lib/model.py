...
class Model:
  ...
  def feedforward(self, data, target_layer=None, is_training=False):
    try:
      assert(self.is_continue and self.sess is not None)
      with self.sess.as_default():
        if target_layer is None:
          output = self.sess.run(self.output_layers, feed_dict={self.input: data, self.is_training: is_training})
        else:
          assert(type(target_layer) is str)
          target_tensor = self.sess.graph.get_tensor_by_name(target_layer + ':0')
          output = self.sess.run(target_tensor, feed_dict={self.input: data, self.is_training: is_training})
        # print('Outcome:')
        # print(output)
        return output
