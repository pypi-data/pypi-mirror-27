    ...
    '''  # line 337
        ...
        if argument.endswith(SLASH):  # line 341
            try:  # line 342
                ...
                Exit("Unknown branch label '%s'" % argument)  # line 343
        if SLASH in argument:  # line 344
            ...
            try:  # line 346
                ...
                Exit("Unknown branch label or wrong number format '%s/%s'" % (b, r))  # line 347
...
def remove(relPath: 'str', pattern: 'str') -> 'None':  # line 703
    ...
    if pattern not in m.branches[m.branch].tracked:  # line 708
        ...
        if suggestion:  # TODO use same wording as in move  # line 712
            print("Do you mean any of the following tracked file patterns? '%s'" % (", ".join(sorted(suggestion))))  # TODO use same wording as in move  # line 712
...
def ls(argument: '_coconut.typing.Optional[str]'=None, options: '_coconut.typing.Sequence[str]'=[]) -> 'None':  # line 718
    ...
    folder = "." if argument is None else argument  # type: str  # line 720
    ...
    relPath = os.path.relpath(folder, m.root).replace(os.sep, SLASH)  # type: str  # line 724
    ...
    if '--patterns' in options:  # line 726
        ...
        if out:  # line 728
            print(out)  # line 728
        return  # line 729
    files = list(sorted((entry for entry in os.listdir(folder) if os.path.isfile(os.path.join(folder, entry)))))  # type: List[str]  # line 730
        ...
        if not ignore:  # line 738
            ...
            for pattern in (p for p in trackingPatterns if os.path.dirname(p).replace(os.sep, SLASH) == relPath):  # only patterns matching current folder  # line 740
                if fnmatch.fnmatch(file, os.path.basename(pattern)):  # line 741
                    matches.append(os.path.basename(pattern))  # line 741
        matches.sort(key=lambda element: len(element))  # line 742
        print("%s %s%s" % ("IGN" if ignore is not None else ("TRK" if len(matches) > 0 else "..."), file, "  (%s)" % ignore if ignore is not None else ("  (%s)" % ("; ".join(matches)) if len(matches) > 0 else "")))  # line 743

def log(options: '_coconut.typing.Sequence[str]') -> 'None':  # line 745
    ''' List previous commits on current branch. '''  # line 746
    m = Metadata(os.getcwd())  # type: Metadata  # line 747
    m.loadBranches()  # knows current branch  # line 748
    m.loadBranch(m.branch)  # knows commit history  # line 749
    info((lambda _coconut_none_coalesce_item: "r%02d" % m.branch if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)("//|\\\\ Offline commit history of branch '%s'" % m.branches[m.branch].name))  # TODO also retain "from branch/revision" on branching?  # line 750
    nl = len("%d" % max(m.commits))  # determine space needed for revision  # line 751
    changeset = m.computeSequentialPathSetIterator(m.branch, max(m.commits))  # line 752
    maxWidth = max([wcswidth((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(commit.message)) for commit in m.commits.values()])  # type: int  # line 753
    for no in range(max(m.commits) + 1):  # line 754
        commit = m.commits[no]  # type: CommitInfo  # line 755
        changes = next(changeset)  # type: ChangeSet  # side-effect: updates m.paths  # line 756
        newTextFiles = len([file for file in changes.additions.keys() if m.isTextType(file)])  # type: int  # line 757
        print("  %s r%s @%s (+%02d/-%02d/*%02d +%02dT/%05.1f%%) |%s|" % ("*" if commit.number == max(m.commits) else " ", ("%%%ds" % nl) % commit.number, strftime(commit.ctime), len(changes.additions), len(changes.deletions), len(changes.modifications), newTextFiles, 0. if len(changes.additions) == 0 else newTextFiles * 100. / len(changes.additions), ((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(commit.message)).ljust(maxWidth)))  # line 758
        if '--changes' in options:  # line 759
            m.listChanges(changes)  # line 759

def config(argument: 'str', options: 'List[str]'=[]) -> 'None':  # line 761
    if argument not in ["set", "unset", "show", "list", "add", "rm"]:  # line 762
        Exit("Unknown config command")  # line 762
    if not configr:  # line 763
        Exit("Cannot execute config command. 'configr' module not installed")  # line 763
    c = loadConfig()  # type: Union[configr.Configr, Accessor]  # line 764
    if argument == "set":  # line 765
        if len(options) < 2:  # line 766
            Exit("No key nor value specified")  # line 766
        if options[0] not in (["defaultbranch"] + CONFIGURABLE_FLAGS + CONFIGURABLE_LISTS):  # line 767
            Exit("Unsupported key %r" % options[0])  # line 767
        if options[0] in CONFIGURABLE_FLAGS and options[1].lower() not in TRUTH_VALUES + FALSE_VALUES:  # line 768
            Exit("Cannot set flag to '%s'. Try on/off instead" % options[1].lower())  # line 768
        c[options[0]] = options[1].lower() in TRUTH_VALUES if options[0] in CONFIGURABLE_FLAGS else (options[1].strip() if options[0] not in CONFIGURABLE_LISTS else safeSplit(options[1], ";"))  # TODO sanitize texts?  # line 769
    elif argument == "unset":  # line 770
        if len(options) < 1:  # line 771
            Exit("No key specified")  # line 771
        if options[0] not in c.keys():  # line 772
            Exit("Unknown key")  # line 772
        del c[options[0]]  # line 773
    elif argument == "add":  # line 774
        if len(options) < 2:  # line 775
            Exit("No key nor value specified")  # line 775
        if options[0] not in CONFIGURABLE_LISTS:  # line 776
            Exit("Unsupported key for add %r" % options[0])  # line 776
        if options[0] not in c.keys():  # add list  # line 777
            c[options[0]] = [options[1]]  # add list  # line 777
        elif options[1] in c[options[0]]:  # line 778
            Exit("Value already contained")  # line 778
        c[options[0]].append(options[1])  # line 779
    elif argument == "rm":  # line 780
        if len(options) < 2:  # line 781
            Exit("No key nor value specified")  # line 781
        if options[0] not in c.keys():  # line 782
            Exit("Unknown key specified: %r" % options[0])  # line 782
        if options[1] not in c[options[0]]:  # line 783
            Exit("Unknown value: %r" % options[1])  # line 783
        c[options[0]].remove(options[1])  # line 784
    else:  # Show  # line 785
        for k, v in sorted(c.items()):  # line 786
            print("%s => %r" % (k, v))  # line 786
        return  # line 787
    f, g = saveConfig(c)  # line 788
    if f is None:  # line 789
        error("Error saving user configuration: %r" % g)  # line 789

def move(relPath: 'str', pattern: 'str', newRelPath: 'str', newPattern: 'str', options: 'List[str]'=[]) -> 'None':  # line 791
    ''' Path differs: Move files, create folder if not existing. Pattern differs: Attempt to rename file, unless exists in target or not unique. '''  # line 792
    force = '--force' in options  # type: bool  # line 793
    soft = '--soft' in options  # type: bool  # line 794
    if not os.path.exists(relPath.replace(SLASH, os.sep)) and not force:  # line 795
        Exit("Source folder doesn't exist. Use --force to proceed anyway")  # line 795
    m = Metadata(os.getcwd())  # type: Metadata  # line 796
    matching = fnmatch.filter(os.listdir(relPath.replace(SLASH, os.sep)) if os.path.exists(relPath.replace(SLASH, os.sep)) else [], os.path.basename(pattern))  # type: _coconut.typing.Sequence[str]  # find matching files in source  # line 797
    matching[:] = [f for f in matching if len([n for n in m.c.ignores if fnmatch.fnmatch(f, n)]) == 0 or len([p for p in m.c.ignoresWhitelist if fnmatch.fnmatch(f, p)]) > 0]  # line 798
    if not matching and not force:  # line 799
        Exit("No files match the specified file pattern. Use --force to proceed anyway")  # line 799
    m.loadBranches()  # knows current branch  # line 800
    if not m.track and not m.picky:  # line 801
        Exit("Repository is in simple mode. Simply use basic file operations to modify files, then execute 'sos commit' to version the changes")  # line 801
    if pattern not in m.branches[m.branch].tracked:  # line 802
        for tracked in (t for t in m.branches[m.branch].tracked if os.path.dirname(t) == relPath):  # for all patterns of the same source folder  # line 803
            alternative = fnmatch.filter(matching, os.path.basename(tracked))  # type: _coconut.typing.Sequence[str]  # find if it matches any of the files in the source folder, too  # line 804
            if alternative:  # line 805
                info("  '%s' matches %d files" % (tracked, len(alternative)))  # line 805
        if not (force or soft):  # line 806
            Exit("File pattern '%s' is not tracked on current branch. 'sos move' only works on tracked patterns" % pattern)  # line 806
    basePattern = os.path.basename(pattern)  # type: str  # pure glob without folder  # line 807
    newBasePattern = os.path.basename(newPattern)  # type: str  # line 808
    if basePattern.count("*") < newBasePattern.count("*") or (basePattern.count("?") - basePattern.count("[?]")) < (newBasePattern.count("?") - newBasePattern.count("[?]")) or (basePattern.count("[") - basePattern.count("\\[")) < (newBasePattern.count("[") - newBasePattern.count("\\[")) or (basePattern.count("]") - basePattern.count("\\]")) < (newBasePattern.count("]") - newBasePattern.count("\\]")):  # line 809
        Exit("Glob markers from '%s' to '%s' don't match, cannot move/rename tracked matching files" % (basePattern, newBasePattern))  # line 813
    oldTokens = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 814
    newToken = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 814
    oldTokens, newTokens = tokenizeGlobPatterns(os.path.basename(pattern), os.path.basename(newPattern))  # line 815
    matches = convertGlobFiles(matching, oldTokens, newTokens)  # type: _coconut.typing.Sequence[Tuple[str, str]]  # computes list of source - target filename pairs  # line 816
    if len({st[1] for st in matches}) != len(matches):  # line 817
        Exit("Some target filenames are not unique and different move/rename actions would point to the same target file")  # line 817
    matches = reorderRenameActions(matches, exitOnConflict=not soft)  # attempts to find conflict-free renaming order, or exits  # line 818
    if os.path.exists(newRelPath):  # line 819
        exists = [filename[1] for filename in matches if os.path.exists(os.path.join(newRelPath, filename[1]).replace(SLASH, os.sep))]  # type: _coconut.typing.Sequence[str]  # line 820
        if exists and not (force or soft):  # line 821
            Exit("%s files would write over existing files in %s cases. Use --force to execute it anyway" % ("Moving" if relPath != newRelPath else "Renaming", "all" if len(exists) == len(matches) else "some"))  # line 821
    else:  # line 822
        os.makedirs(os.path.abspath(newRelPath.replace(SLASH, os.sep)))  # line 822
    if not soft:  # perform actual renaming  # line 823
        for (source, target) in matches:  # line 824
            try:  # line 825
                shutil.move(os.path.abspath(os.path.join(relPath, source).replace(SLASH, os.sep)), os.path.abspath(os.path.join(newRelPath, target).replace(SLASH, os.sep)))  # line 825
            except Exception as E:  # one error can lead to another in case of delicate renaming order  # line 826
                error("Cannot move/rename file '%s' to '%s'" % (source, os.path.join(newRelPath, target)))  # one error can lead to another in case of delicate renaming order  # line 826
    m.branches[m.branch].tracked[m.branches[m.branch].tracked.index(pattern)] = newPattern  # line 827
    m.saveBranches()  # line 828

def parse(root: 'str', cwd: 'str'):  # line 830
    ''' Main operation. Main has already chdir into VCS root folder, cwd is original working directory for add, rm. '''  # line 831
    debug("Parsing command-line arguments...")  # line 832
    command = sys.argv[1].strip() if len(sys.argv) > 1 else ""  # line 833
    argument = sys.argv[2].strip() if len(sys.argv) > 2 and not sys.argv[2].startswith("--") else None  # line 834
    options = sys.argv[3:] if command == "config" else [c for c in sys.argv[2:] if c.startswith("--")]  # consider second parameter for no-arg command, otherwise from third on  # line 835
    onlys, excps = parseOnlyOptions(cwd, options)  # extracts folder-relative information for changes, commit, diff, switch, update  # line 836
    debug("Processing command %r with argument '%s' and options %r." % ("" if command is None else command, "" if argument is None else argument, options))  # line 837
    if command[:1] in "amr":  # line 838
        relPath, pattern = relativize(root, os.path.join(cwd, "." if argument is None else argument))  # line 838
    if command[:1] == "m":  # line 839
        if not options:  # line 840
            Exit("Need a second file pattern argument as target for move/rename command")  # line 840
        newRelPath, newPattern = relativize(root, os.path.join(cwd, options[0]))  # TODO add error message if target pattern is missing (???)  # line 841
    if command[:1] == "a":  # line 842
        add(relPath, pattern, options)  # line 842
    elif command[:1] == "b":  # line 843
        branch(argument, options)  # line 843
    elif command[:2] == "ch":  # line 844
        changes(argument, options, onlys, excps)  # line 844
    elif command[:3] == "com":  # line 845
        commit(argument, options, onlys, excps)  # line 845
    elif command[:2] == "ci":  # line 846
        commit(argument, options, onlys, excps)  # line 846
    elif command[:3] == 'con':  # line 847
        config(argument, options)  # line 847
    elif command[:2] == "de":  # line 848
        delete(argument, options)  # line 848
    elif command[:2] == "di":  # line 849
        diff(argument, options, onlys, excps)  # line 849
    elif command[:1] == "h":  # line 850
        usage()  # line 850
    elif command[:2] == "lo":  # line 851
        log(options)  # line 851
    elif command[:2] == "li":  # TODO handle absolute paths as well, also for Windows  # line 852
        ls(relativize(root, cwd if argument is None else os.path.join(cwd, argument))[1], options)  # TODO handle absolute paths as well, also for Windows  # line 852
    elif command[:2] == "ls":  # TODO avoid root super paths (..)  # line 853
        ls(relativize(root, cwd if argument is None else os.path.join(cwd, argument))[1], options)  # TODO avoid root super paths (..)  # line 853
    elif command[:1] == "m":  # line 854
        move(relPath, pattern, newRelPath, newPattern, options[1:])  # line 854
    elif command[:2] == "of":  # line 855
        offline(argument, options)  # line 855
    elif command[:2] == "on":  # line 856
        online(options)  # line 856
    elif command[:1] == "r":  # line 857
        remove(relPath, pattern)  # line 857
    elif command[:2] == "st":  # line 858
        status()  # line 858
    elif command[:2] == "sw":  # line 859
        switch(argument, options, onlys, excps)  # line 859
    elif command[:1] == "u":  # line 860
        update(argument, options, onlys, excps)  # line 860
    else:  # line 861
        Exit("Unknown command '%s'" % command)  # line 861
    sys.exit(0)  # line 862

def main() -> 'None':  # line 864
    global debug, info, warn, error  # line 865
    logging.basicConfig(level=level, stream=sys.stderr, format=("%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '--log' in sys.argv else "%(message)s"))  # line 866
    _log = Logger(logging.getLogger(__name__))  # line 867
    debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 867
    for option in (o for o in ['--log', '--verbose', '-v', '--sos'] if o in sys.argv):  # clean up program arguments  # line 868
        sys.argv.remove(option)  # clean up program arguments  # line 868
    if '--help' in sys.argv or len(sys.argv) < 2:  # line 869
        usage()  # line 869
        Exit()  # line 869
    command = sys.argv[1] if len(sys.argv) > 1 else None  # type: _coconut.typing.Optional[str]  # line 870
    root, vcs, cmd = findSosVcsBase()  # root is None if no .sos folder exists up the folder tree (still working online); vcs is checkout/repo root folder; cmd is the VCS base command  # line 871
    debug("Found root folders for SOS|VCS: %s|%s" % ("" if root is None else root, "" if vcs is None else vcs))  # line 872
    defaults["defaultbranch"] = vcsBranches.get(cmd, "trunk")  # sets dynamic default with SVN fallback  # line 873
    if force_sos or root is not None or ("" if command is None else command)[:2] == "of" or ("" if command is None else command)[:1] == "h":  # in offline mode or just going offline TODO what about git config?  # line 874
        cwd = os.getcwd()  # line 875
        os.chdir(cwd if command[:2] == "of" else cwd if root is None else root)  # line 876
        parse(root, cwd)  # line 877
    elif cmd is not None:  # online mode - delegate to VCS  # line 878
        info("SOS: Running '%s %s'" % (cmd, " ".join(sys.argv[1:])))  # line 879
        import subprocess  # only required in this section  # line 880
        process = subprocess.Popen([cmd] + sys.argv[1:], shell=False, stdin=subprocess.PIPE, stdout=sys.stdout, stderr=sys.stderr)  # line 881
        inp = ""  # type: str  # line 882
        while True:  # line 883
            so, se = process.communicate(input=inp)  # line 884
            if process.returncode is not None:  # line 885
                break  # line 885
            inp = sys.stdin.read()  # line 886
        if sys.argv[1][:2] == "co" and process.returncode == 0:  # successful commit - assume now in sync again (but leave meta data folder with potential other feature branches behind until "online")  # line 887
            if root is None:  # line 888
                Exit("Cannot determine VCS root folder: Unable to mark repository as synchronized and will show a warning when leaving offline mode")  # line 888
            m = Metadata(root)  # line 889
            m.loadBranches()  # read repo  # line 890
            m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], inSync=True)  # mark as committed  # line 891
            m.saveBranches()  # line 892
    else:  # line 893
        Exit("No offline repository present, and unable to detect VCS file tree")  # line 893


# Main part
level = logging.DEBUG if os.environ.get("DEBUG", "False").lower() == "true" or '--verbose' in sys.argv or '-v' in sys.argv else logging.INFO  # line 897
force_sos = '--sos' in sys.argv  # line 898
_log = Logger(logging.getLogger(__name__))  # line 899
debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 899
if __name__ == '__main__':  # line 900
    main()  # line 900
