...
# TODO test same for simple mode
    ...
    def testOnly(_):  # line 750
        _.assertEqual((_coconut.frozenset(("./A", "x/B")), _coconut.frozenset(("./C",))), sos.parseOnlyOptions(".", ["abc", "def", "--only", "A", "--x", "--only", "x/B", "--except", "C", "--only"]))  # line 751
        _.assertEqual(_coconut.frozenset(("B",)), sos.conditionalIntersection(_coconut.frozenset(("A", "B", "C")), _coconut.frozenset(("B", "D"))))  # line 752
        _.assertEqual(_coconut.frozenset(("B", "D")), sos.conditionalIntersection(_coconut.frozenset(), _coconut.frozenset(("B", "D"))))  # line 753
        _.assertEqual(_coconut.frozenset(("B", "D")), sos.conditionalIntersection(None, _coconut.frozenset(("B", "D"))))  # line 754
        sos.offline(os.getcwd(), ["--track", "--strict"])  # line 755
        _.createFile(1)  # line 756
        _.createFile(2)  # line 757
        sos.add(".", "./file1")  # line 758
        sos.add(".", "./file2")  # line 759
        sos.commit(onlys=_coconut.frozenset(("./file1",)))  # line 760
        _.assertEqual(2, len(os.listdir(branchFolder(0, 1))))  # only meta and file1  # line 761
        sos.commit()  # adds also file2  # line 762
        _.assertEqual(2, len(os.listdir(branchFolder(0, 2))))  # only meta and file1  # line 763
        _.createFile(1, "cc")  # modify both files  # line 764
        _.createFile(2, "dd")  # line 765
        sos.config("set", ["texttype", "file2"])  # line 766
        changes = sos.changes(excps=["./file1"])  # line 767
        _.assertEqual(1, len(changes.modifications))  # only file2  # line 768
        _.assertTrue("./file2" in changes.modifications)  # line 769
        _.assertIn("DIF ./file2", wrapChannels(lambda: sos.diff(onlys=_coconut.frozenset(("./file2",)))))  # line 770
        _.assertAllNotIn(["MOD ./file1", "DIF ./file1", "MOD ./file2"], wrapChannels(lambda: sos.diff(onlys=_coconut.frozenset(("./file2",)))))  # line 771

    def testDiff(_):  # line 773
        sos.config("set", ["texttype", "file1"])  # line 774
        sos.offline(options=["--strict"])  # line 775
        _.createFile(1)  # line 776
        _.createFile(2)  # line 777
        sos.commit()  # line 778
        _.createFile(1, "sdfsdgfsdf")  # line 779
        _.createFile(2, "12343")  # line 780
        sos.commit()  # line 781
        _.createFile(1, "foobar")  # line 782
        _.assertAllIn(["MOD ./file2", "DIF ./file1", "- | 0000 |xxxxxxxxxx|", "+ | 0000 |foobar|"], wrapChannels(lambda: sos.diff("/-2")))  # vs. second last  # line 783
        _.assertAllNotIn(["MOD ./file1", "DIF ./file1"], wrapChannels(lambda: sos.diff("/-2", onlys=_coconut.frozenset(("./file2",)))))  # line 784

    def testReorderRenameActions(_):  # line 786
        result = sos.reorderRenameActions([("123", "312"), ("312", "132"), ("321", "123")], exitOnConflict=False)  # type: _coconut.typing.Sequence[Tuple[str, str]]  # line 787
        _.assertEqual([("312", "132"), ("123", "312"), ("321", "123")], result)  # line 788
        try:  # line 789
            sos.reorderRenameActions([("123", "312"), ("312", "123")], exitOnConflict=True)  # line 789
            _.fail()  # line 789
        except:  # line 790
            pass  # line 790

    def testMove(_):  # line 792
        sos.offline(options=["--strict", "--track"])  # line 793
        _.createFile(1)  # line 794
        sos.add(".", "./file?")  # line 795
# test source folder missing
        try:  # line 797
            sos.move("sub", "sub/file?", ".", "?file")  # line 797
            _.fail()  # line 797
        except:  # line 798
            pass  # line 798
# test target folder missing: create it
        sos.move(".", "./file?", "sub", "sub/file?")  # line 800
        _.assertTrue(os.path.exists("sub"))  # line 801
        _.assertTrue(os.path.exists("sub/file1"))  # line 802
        _.assertFalse(os.path.exists("file1"))  # line 803
# test move
        sos.move("sub", "sub/file?", ".", "./?file")  # line 805
        _.assertTrue(os.path.exists("1file"))  # line 806
        _.assertFalse(os.path.exists("sub/file1"))  # line 807
# test nothing matches source pattern
        try:  # line 809
            sos.move(".", "a*", ".", "b*")  # line 809
            _.fail()  # line 809
        except:  # line 810
            pass  # line 810
        sos.add(".", "*")  # anything pattern  # line 811
        try:  # TODO check that alternative pattern "*" was suggested (1 hit)  # line 812
            sos.move(".", "a*", ".", "b*")  # TODO check that alternative pattern "*" was suggested (1 hit)  # line 812
            _.fail()  # TODO check that alternative pattern "*" was suggested (1 hit)  # line 812
        except:  # line 813
            pass  # line 813
# test rename no conflict
        _.createFile(1)  # line 815
        _.createFile(2)  # line 816
        _.createFile(3)  # line 817
        sos.add(".", "./file*")  # line 818
        sos.config("set", ["ignores", "file3;file4"])  # define an ignore pattern  # line 819
        sos.config("set", ["ignoresWhitelist", "file3"])  # line 820
        sos.move(".", "./file*", ".", "fi*le")  # line 821
        _.assertTrue(all((os.path.exists("fi%dle" % i) for i in range(1, 4))))  # line 822
        _.assertFalse(os.path.exists("fi4le"))  # line 823
# test rename solvable conflicts
        [_.createFile("%s-%s-%s" % tuple((c for c in n))) for n in ["312", "321", "123", "231"]]  # line 825
#    sos.move("?-?-?")
# test rename unsolvable conflicts
# test --soft option
        sos.remove(".", "./?file")  # was renamed before  # line 829
        sos.add(".", "./?a?b", ["--force"])  # line 830
        sos.move(".", "./?a?b", ".", "./a?b?", ["--force", "--soft"])  # line 831
        _.createFile("1a2b")  # should not be tracked  # line 832
        _.createFile("a1b2")  # should be tracked  # line 833
        sos.commit()  # line 834
        _.assertEqual(2, len(os.listdir(branchFolder(0, 1))))  # line 835
        _.assertTrue(os.path.exists(branchFolder(0, 1) + os.sep + "93b38f90892eb5c57779ca9c0b6fbdf6774daeee3342f56f3e78eb2fe5336c50"))  # a1b2  # line 836
        _.createFile("1a1b1")  # line 837
        _.createFile("1a1b2")  # line 838
        sos.add(".", "?a?b*")  # line 839
        _.assertIn("not unique", wrapChannels(lambda: sos.move(".", "?a?b*", ".", "z?z?")))  # should raise error due to same target name  # line 840
...
# TODO add test if two single question marks will be moved into adjacent characters
    ...
    def testFindBase(_):  # line 844
        old = os.getcwd()  # line 845
        try:  # line 846
            os.mkdir("." + os.sep + ".git")  # line 847
            os.makedirs("." + os.sep + "a" + os.sep + sos.metaFolder)  # line 848
            os.makedirs("." + os.sep + "a" + os.sep + "b")  # line 849
            os.chdir("a" + os.sep + "b")  # line 850
            s, vcs, cmd = sos.findSosVcsBase()  # line 851
            _.assertIsNotNone(s)  # line 852
            _.assertIsNotNone(vcs)  # line 853
            _.assertEqual("git", cmd)  # line 854
        finally:  # line 855
            os.chdir(old)  # line 855


if __name__ == '__main__':  # line 858
    logging.basicConfig(level=logging.DEBUG if '-v' in sys.argv or os.getenv("DEBUG", "false").strip().lower() == "true" or os.getenv("CI", "false").strip().lower() == "true" else logging.INFO, stream=sys.stderr, format="%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '-v' in sys.argv or os.getenv("DEBUG", "false") == "true" else "%(message)s")  # line 859
    if configr:  # line 860
        c = configr.Configr("sos")  # line 861
        c.loadSettings()  # line 861
        if len(c.keys()) > 0:  # line 862
            sos.Exit("Cannot run test suite with existing local SOS user configuration (would affect results)")  # line 862
    unittest.main(testRunner=debugTestRunner() if '-v' in sys.argv and not os.getenv("CI", "false").lower() == "true" else None)  # warnings = "ignore")  # line 863
