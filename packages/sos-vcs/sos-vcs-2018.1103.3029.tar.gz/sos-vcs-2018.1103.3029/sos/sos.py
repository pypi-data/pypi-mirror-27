...
def config(argument: 'str', options: 'List[str]'=[]) -> 'None':  # line 772
    ...
    else:  # Show or list  # line 796
        for k, v in sorted(c.items()):  # line 797
            print("%s: %r" % (k.rjust(20), v))  # line 797
        if len(c.keys()) == 0:  # TODO list defaults instead? Or display [DEFAULT] for each item above if unmodified  # line 798
            info("No configuration stored, using only defaults.")  # TODO list defaults instead? Or display [DEFAULT] for each item above if unmodified  # line 798
        return  # line 799
    f, g = saveConfig(c)  # line 800
    if f is None:  # line 801
        error("Error saving user configuration: %r" % g)  # line 801

def move(relPath: 'str', pattern: 'str', newRelPath: 'str', newPattern: 'str', options: 'List[str]'=[]) -> 'None':  # line 803
    ''' Path differs: Move files, create folder if not existing. Pattern differs: Attempt to rename file, unless exists in target or not unique. '''  # line 804
    force = '--force' in options  # type: bool  # line 805
    soft = '--soft' in options  # type: bool  # line 806
    if not os.path.exists(relPath.replace(SLASH, os.sep)) and not force:  # line 807
        Exit("Source folder doesn't exist. Use --force to proceed anyway")  # line 807
    m = Metadata(os.getcwd())  # type: Metadata  # line 808
    matching = fnmatch.filter(os.listdir(relPath.replace(SLASH, os.sep)) if os.path.exists(relPath.replace(SLASH, os.sep)) else [], os.path.basename(pattern))  # type: List[str]  # find matching files in source  # line 809
    matching[:] = [f for f in matching if len([n for n in m.c.ignores if fnmatch.fnmatch(f, n)]) == 0 or len([p for p in m.c.ignoresWhitelist if fnmatch.fnmatch(f, p)]) > 0]  # line 810
    if not matching and not force:  # line 811
        Exit("No files match the specified file pattern. Use --force to proceed anyway")  # line 811
    m.loadBranches()  # knows current branch  # line 812
    if not m.track and not m.picky:  # line 813
        Exit("Repository is in simple mode. Simply use basic file operations to modify files, then execute 'sos commit' to version the changes")  # line 813
    if pattern not in m.branches[m.branch].tracked:  # line 814
        for tracked in (t for t in m.branches[m.branch].tracked if os.path.dirname(t) == relPath):  # for all patterns of the same source folder  # line 815
            alternative = fnmatch.filter(matching, os.path.basename(tracked))  # type: _coconut.typing.Sequence[str]  # find if it matches any of the files in the source folder, too  # line 816
            if alternative:  # line 817
                info("  '%s' matches %d files" % (tracked, len(alternative)))  # line 817
        if not (force or soft):  # line 818
            Exit("File pattern '%s' is not tracked on current branch. 'sos move' only works on tracked patterns" % pattern)  # line 818
    basePattern = os.path.basename(pattern)  # type: str  # pure glob without folder  # line 819
    newBasePattern = os.path.basename(newPattern)  # type: str  # line 820
    if basePattern.count("*") < newBasePattern.count("*") or (basePattern.count("?") - basePattern.count("[?]")) < (newBasePattern.count("?") - newBasePattern.count("[?]")) or (basePattern.count("[") - basePattern.count("\\[")) < (newBasePattern.count("[") - newBasePattern.count("\\[")) or (basePattern.count("]") - basePattern.count("\\]")) < (newBasePattern.count("]") - newBasePattern.count("\\]")):  # line 821
        Exit("Glob markers from '%s' to '%s' don't match, cannot move/rename tracked matching files" % (basePattern, newBasePattern))  # line 825
    oldTokens = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 826
    newToken = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 826
    oldTokens, newTokens = tokenizeGlobPatterns(os.path.basename(pattern), os.path.basename(newPattern))  # line 827
    matches = convertGlobFiles(matching, oldTokens, newTokens)  # type: _coconut.typing.Sequence[Tuple[str, str]]  # computes list of source - target filename pairs  # line 828
    if len({st[1] for st in matches}) != len(matches):  # line 829
        Exit("Some target filenames are not unique and different move/rename actions would point to the same target file")  # line 829
    matches = reorderRenameActions(matches, exitOnConflict=not soft)  # attempts to find conflict-free renaming order, or exits  # line 830
    if os.path.exists(newRelPath):  # line 831
        exists = [filename[1] for filename in matches if os.path.exists(os.path.join(newRelPath, filename[1]).replace(SLASH, os.sep))]  # type: _coconut.typing.Sequence[str]  # line 832
        if exists and not (force or soft):  # line 833
            Exit("%s files would write over existing files in %s cases. Use --force to execute it anyway" % ("Moving" if relPath != newRelPath else "Renaming", "all" if len(exists) == len(matches) else "some"))  # line 833
    else:  # line 834
        os.makedirs(os.path.abspath(newRelPath.replace(SLASH, os.sep)))  # line 834
    if not soft:  # perform actual renaming  # line 835
        for (source, target) in matches:  # line 836
            try:  # line 837
                shutil.move(os.path.abspath(os.path.join(relPath, source).replace(SLASH, os.sep)), os.path.abspath(os.path.join(newRelPath, target).replace(SLASH, os.sep)))  # line 837
            except Exception as E:  # one error can lead to another in case of delicate renaming order  # line 838
                error("Cannot move/rename file '%s' to '%s'" % (source, os.path.join(newRelPath, target)))  # one error can lead to another in case of delicate renaming order  # line 838
    m.branches[m.branch].tracked[m.branches[m.branch].tracked.index(pattern)] = newPattern  # line 839
    m.saveBranches()  # line 840

def parse(root: 'str', cwd: 'str'):  # line 842
    ''' Main operation. Main has already chdir into VCS root folder, cwd is original working directory for add, rm. '''  # line 843
    debug("Parsing command-line arguments...")  # line 844
    try:  # line 845
        command = sys.argv[1].strip() if len(sys.argv) > 1 else ""  # line 846
        argument = sys.argv[2].strip() if len(sys.argv) > 2 and not sys.argv[2].startswith("--") else None  # line 847
        options = sys.argv[3:] if command == "config" else [c for c in sys.argv[2:] if c.startswith("--")]  # consider second parameter for no-arg command, otherwise from third on  # line 848
        onlys, excps = parseOnlyOptions(cwd, options)  # extracts folder-relative information for changes, commit, diff, switch, update  # line 849
        debug("Processing command %r with argument '%s' and options %r." % ("" if command is None else command, "" if argument is None else argument, options))  # line 850
        if command[:1] in "amr":  # line 851
            relPath, pattern = relativize(root, os.path.join(cwd, "." if argument is None else argument))  # line 851
        if command[:1] == "m":  # line 852
            if not options:  # line 853
                Exit("Need a second file pattern argument as target for move/rename command")  # line 853
            newRelPath, newPattern = relativize(root, os.path.join(cwd, options[0]))  # line 854
        if command[:1] == "a":  # line 855
            add(relPath, pattern, options)  # line 855
        elif command[:1] == "b":  # line 856
            branch(argument, options)  # line 856
        elif command[:2] == "ch":  # line 857
            changes(argument, options, onlys, excps)  # line 857
        elif command[:3] == "com":  # line 858
            commit(argument, options, onlys, excps)  # line 858
        elif command[:2] == "ci":  # line 859
            commit(argument, options, onlys, excps)  # line 859
        elif command[:3] == 'con':  # line 860
            config(argument, options)  # line 860
        elif command[:2] == "de":  # line 861
            delete(argument, options)  # line 861
        elif command[:2] == "di":  # line 862
            diff(argument, options, onlys, excps)  # line 862
        elif command[:1] == "h":  # line 863
            usage()  # line 863
        elif command[:2] == "lo":  # line 864
            log(options)  # line 864
        elif command[:2] == "li":  # TODO handle absolute paths as well, also for Windows  # line 865
            ls(relativize(root, cwd if argument is None else os.path.join(cwd, argument))[1], options)  # TODO handle absolute paths as well, also for Windows  # line 865
        elif command[:2] == "ls":  # TODO avoid root super paths (..)  # line 866
            ls(relativize(root, cwd if argument is None else os.path.join(cwd, argument))[1], options)  # TODO avoid root super paths (..)  # line 866
        elif command[:1] == "m":  # line 867
            move(relPath, pattern, newRelPath, newPattern, options[1:])  # line 867
        elif command[:2] == "of":  # line 868
            offline(argument, options)  # line 868
        elif command[:2] == "on":  # line 869
            online(options)  # line 869
        elif command[:1] == "r":  # line 870
            remove(relPath, pattern)  # line 870
        elif command[:2] == "st":  # line 871
            status()  # line 871
        elif command[:2] == "sw":  # line 872
            switch(argument, options, onlys, excps)  # line 872
        elif command[:1] == "u":  # line 873
            update(argument, options, onlys, excps)  # line 873
        elif command[:1] == "v":  # line 874
            usage(short=True)  # line 874
        else:  # line 875
            Exit("Unknown command '%s'" % command)  # line 875
    except (Exception, RuntimeError) as E:  # line 876
        print(str(E))  # line 877
        import traceback  # line 878
        traceback.print_exc()  # line 879
        traceback.print_stack()  # line 880
        try:  # line 881
            traceback.print_last()  # line 881
        except:  # line 882
            pass  # line 882
        print("An internal error occurred in SOS. Please report above message to the project maintainer at  https://github.com/ArneBachmann/sos/issues  via 'New Issue'.\nPlease state your installed version via 'sos version', and what you were doing.")  # line 883
    sys.exit(0)  # line 884

def main() -> 'None':  # line 886
    global debug, info, warn, error  # line 887
    logging.basicConfig(level=level, stream=sys.stderr, format=("%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '--log' in sys.argv else "%(message)s"))  # line 888
    _log = Logger(logging.getLogger(__name__))  # line 889
    debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 889
    for option in (o for o in ['--log', '--verbose', '-v', '--sos'] if o in sys.argv):  # clean up program arguments  # line 890
        sys.argv.remove(option)  # clean up program arguments  # line 890
    if '--help' in sys.argv or len(sys.argv) < 2:  # line 891
        usage()  # line 891
        Exit()  # line 891
    command = sys.argv[1] if len(sys.argv) > 1 else None  # type: _coconut.typing.Optional[str]  # line 892
    root, vcs, cmd = findSosVcsBase()  # root is None if no .sos folder exists up the folder tree (still working online); vcs is checkout/repo root folder; cmd is the VCS base command  # line 893
    debug("Found root folders for SOS|VCS: %s|%s" % ("" if root is None else root, "" if vcs is None else vcs))  # line 894
    defaults["defaultbranch"] = vcsBranches.get(cmd, "trunk")  # sets dynamic default with SVN fallback  # line 895
    if force_sos or root is not None or ("" if command is None else command)[:2] == "of" or ("" if command is None else command)[:1] in ["h", "v"]:  # in offline mode or just going offline TODO what about git config?  # line 896
        cwd = os.getcwd()  # line 897
        os.chdir(cwd if command[:2] == "of" else cwd if root is None else root)  # line 898
        parse(root, cwd)  # line 899
    elif force_vcs or cmd is not None:  # online mode - delegate to VCS  # line 900
        info("SOS: Running '%s %s'" % (cmd, " ".join(sys.argv[1:])))  # line 901
        import subprocess  # only required in this section  # line 902
        process = subprocess.Popen([cmd] + sys.argv[1:], shell=False, stdin=subprocess.PIPE, stdout=sys.stdout, stderr=sys.stderr)  # line 903
        inp = ""  # type: str  # line 904
        while True:  # line 905
            so, se = process.communicate(input=inp)  # line 906
            if process.returncode is not None:  # line 907
                break  # line 907
            inp = sys.stdin.read()  # line 908
        if sys.argv[1][:2] == "co" and process.returncode == 0:  # successful commit - assume now in sync again (but leave meta data folder with potential other feature branches behind until "online")  # line 909
            if root is None:  # line 910
                Exit("Cannot determine VCS root folder: Unable to mark repository as synchronized and will show a warning when leaving offline mode")  # line 910
            m = Metadata(root)  # line 911
            m.loadBranches()  # read repo  # line 912
            m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], inSync=True)  # mark as committed  # line 913
            m.saveBranches()  # line 914
    else:  # line 915
        Exit("No offline repository present, and unable to detect VCS file tree")  # line 915


# Main part
level = logging.DEBUG if os.environ.get("DEBUG", "False").lower() == "true" or '--verbose' in sys.argv or '-v' in sys.argv else logging.INFO  # line 919
force_sos = '--sos' in sys.argv  # type: bool  # line 920
force_vcs = '--vcs' in sys.argv  # type: bool  # line 921
_log = Logger(logging.getLogger(__name__))  # type: logging.Logger  # line 922
debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 922
if __name__ == '__main__':  # line 923
    main()  # line 923
