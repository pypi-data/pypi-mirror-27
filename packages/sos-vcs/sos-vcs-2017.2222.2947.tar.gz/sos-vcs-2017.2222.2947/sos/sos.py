  ...
  '''  # line 103
    ...
    '''  # line 258
        ...
        for path, dirnames, filenames in os.walk(_.root):  # line 270
            ...
            walk = filenames if considerOnly is None else list(reduce(lambda last, pattern: last | set(fnmatch.filter(filenames, os.path.basename(pattern))), (p for p in considerOnly if os.path.dirname(p) == relpath), _coconut.set()))  # line 275
            if dontConsider:  # line 276
                walk[:] = [fn for fn in walk if not any((fnmatch.fnmatch(fn, os.path.basename(p)) for p in dontConsider if os.path.dirname(p) == relpath))]  # TODO dirname is correct noramalized? same above?  # line 277
...
def online(options: '_coconut.typing.Sequence[str]'=[]) -> 'None':  # line 432
    ...
    if options.count("--force") < 2:  # line 439
        changes = m.findChanges(checkContent=strict, considerOnly=None if not m.track else m.getTrackingPatterns())  # type: ChangeSet  # HINT no option for --only/--except here on purpose. No check for picky here, because online is not a command that considers staged files (but we could use --only here, alternatively)  # line 440
...
def changes(argument: 'str'=None, options: '_coconut.typing.Sequence[str]'=[], onlys: '_coconut.typing.Optional[FrozenSet[str]]'=None, excps: '_coconut.typing.Optional[FrozenSet[str]]'=None) -> 'ChangeSet':  # line 464
    ...
    changes = m.findChanges(checkContent=strict, considerOnly=onlys if not m.track else conditionalIntersection(onlys, m.getTrackingPatterns() | m.getTrackingPatterns(branch)), dontConsider=excps)  # type: ChangeSet  # line 476
...
def diff(argument: 'str', options: '_coconut.typing.Sequence[str]'=[], onlys: '_coconut.typing.Optional[FrozenSet[str]]'=None, excps: '_coconut.typing.Optional[FrozenSet[str]]'=None) -> 'None':  # line 480
    ...
    changes = m.findChanges(checkContent=strict, inverse=True, considerOnly=onlys if not m.track else conditionalIntersection(onlys, m.getTrackingPatterns() | m.getTrackingPatterns(branch)), dontConsider=excps)  # type: ChangeSet  # line 492
...
def add(relpath: 'str', pattern: 'str', options: '_coconut.typing.Sequence[str]'=[]) -> 'None':  # line 685
    ...
    if not force and len(fnmatch.filter(os.listdir(os.path.abspath(relpath)), os.path.basename(pattern.replace(SLASH, os.sep)))) == 0:  # doesn't match any current file  # line 692
        Exit("Pattern doesn't match any file in specified folder. Use --force to add it anyway")  # line 693
    m.branches[m.branch].tracked.append(pattern)  # TODO set insync flag to False? same for rm  # line 694
    m.saveBranches()  # line 695
    info("Added tracking pattern '%s' for folder '%s'" % (os.path.basename(pattern.replace(SLASH, os.sep)), os.path.abspath(relpath)))  # line 696

def rm(relpath: 'str', pattern: 'str') -> 'None':  # line 698
    ''' Remove a tracked files pattern from current branch's tracked files. '''  # line 699
    m = Metadata(os.getcwd())  # type: Metadata  # line 700
    m.loadBranches()  # line 701
    if not m.track and not m.picky:  # line 702
        Exit("Repository is in simple mode. Needs 'offline --track' or 'offline --picky' instead")  # line 702
    if pattern not in m.branches[m.branch].tracked:  # line 703
        suggestion = _coconut.set()  # type: Set[str]  # line 704
        for pat in m.branches[m.branch].tracked:  # line 705
            if fnmatch.fnmatch(pattern, pat):  # line 706
                suggestion.add(pat)  # line 706
        if suggestion:  # line 707
            print("Do you mean any of the following tracked globs? '%s'" % (", ".join(sorted(suggestion))))  # line 707
        Exit("Tracked pattern '%s' not found" % pattern)  # line 708
    m.branches[m.branch].tracked.remove(pattern)  # line 709
    m.saveBranches()  # line 710
    info("Removed tracking pattern '%s' for folder '%s'" % (os.path.basename(pattern.replace(SLASH, os.sep)), os.path.abspath(relpath)))  # line 711

def ls(argument: '_coconut.typing.Optional[str]'=None) -> 'None':  # line 713
    ''' List specified directory, augmenting with repository metadata. '''  # line 714
    cwd = os.getcwd() if argument is None else argument  # type: str  # line 715
    m = Metadata(cwd)  # type: Metadata  # line 716
    m.loadBranches()  # line 717
    relpath = os.path.relpath(cwd, m.root).replace(os.sep, SLASH)  # type: str  # line 718
    files = list(sorted((entry for entry in os.listdir(cwd) if os.path.isfile(entry))))  # type: List[str]  # line 719
    trackingPatterns = m.getTrackingPatterns() if m.track or m.picky else _coconut.frozenset()  # type: FrozenSet[str]  # for current branch  # line 720
    for file in files:  # line 721
        ignore = None  # type: _coconut.typing.Optional[str]  # line 722
        for ig in m.c.ignores:  # line 723
            if fnmatch.fnmatch(file, ig):  # remember first match TODO document this  # line 724
                ignore = ig  # remember first match TODO document this  # line 724
                break  # remember first match TODO document this  # line 724
        if ig:  # line 725
            for wl in m.c.ignoresWhitelist:  # line 726
                if fnmatch.fnmatch(file, wl):  # found a white list entry for ignored file, undo ignoring it  # line 727
                    ignore = None  # found a white list entry for ignored file, undo ignoring it  # line 727
                    break  # found a white list entry for ignored file, undo ignoring it  # line 727
        if ignore is None:  # line 728
            matches = []  # type: List[str]  # line 729
            for pattern in (p for p in trackingPatterns if os.path.dirname(p) == relpath):  # only patterns matching current folder  # line 730
                if fnmatch.fnmatch(file, os.path.basename(pattern)):  # TODO or only file basename?  # line 731
                    matches.append(pattern)  # TODO or only file basename?  # line 731
        print("%s %s%s" % ("IGN" if ignore is not None else ("TRK" if len(matches) > 0 else "   "), file, ' by "%s"' % ignore if ignore is not None else (" by " + ";".join(['"%s"' % match for match in matches]) if len(matches) > 0 else "")))  # line 732

def log(options: '_coconut.typing.Sequence[str]') -> 'None':  # line 734
    ''' List previous commits on current branch. '''  # line 735
    m = Metadata(os.getcwd())  # type: Metadata  # line 736
    m.loadBranches()  # knows current branch  # line 737
    m.loadBranch(m.branch)  # knows commit history  # line 738
    info((lambda _coconut_none_coalesce_item: "r%02d" % m.branch if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)("//|\\\\ Offline commit history of branch '%s'" % m.branches[m.branch].name))  # TODO also retain "from branch/revision" on branching?  # line 739
    nl = len("%d" % max(m.commits))  # determine space needed for revision  # line 740
    changeset = m.computeSequentialPathSetIterator(m.branch, max(m.commits))  # line 741
    maxWidth = max([wcswidth((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(commit.message)) for commit in m.commits.values()])  # type: int  # line 742
    for no in range(max(m.commits) + 1):  # line 743
        commit = m.commits[no]  # type: CommitInfo  # line 744
        changes = next(changeset)  # type: ChangeSet  # side-effect: updates m.paths  # line 745
        newTextFiles = len([file for file in changes.additions.keys() if m.isTextType(file)])  # type: int  # line 746
        print("  %s r%s @%s (+%02d/-%02d/*%02d +%02dT/%05.1f%%) |%s|" % ("*" if commit.number == max(m.commits) else " ", ("%%%ds" % nl) % commit.number, strftime(commit.ctime), len(changes.additions), len(changes.deletions), len(changes.modifications), newTextFiles, 0. if len(changes.additions) == 0 else newTextFiles * 100. / len(changes.additions), ((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(commit.message)).ljust(maxWidth)))  # line 747
        if '--changes' in options:  # line 748
            m.listChanges(changes)  # line 748

def config(argument: 'str', options: 'List[str]'=[]) -> 'None':  # line 750
    if argument not in ["set", "unset", "show", "list", "add", "rm"]:  # line 751
        Exit("Unknown config command")  # line 751
    if not configr:  # line 752
        Exit("Cannot execute config command. 'configr' module not installed")  # line 752
    c = loadConfig()  # type: Union[configr.Configr, Accessor]  # line 753
    if argument == "set":  # line 754
        if len(options) < 2:  # line 755
            Exit("No key nor value specified")  # line 755
        if options[0] not in (["defaultbranch"] + CONFIGURABLE_FLAGS + CONFIGURABLE_LISTS):  # line 756
            Exit("Unsupported key %r" % options[0])  # line 756
        if options[0] in CONFIGURABLE_FLAGS and options[1].lower() not in TRUTH_VALUES + FALSE_VALUES:  # line 757
            Exit("Cannot set flag to '%s'. Try on/off instead" % options[1].lower())  # line 757
        c[options[0]] = options[1].lower() in TRUTH_VALUES if options[0] in CONFIGURABLE_FLAGS else (options[1].strip() if options[0] not in CONFIGURABLE_LISTS else safeSplit(options[1], ";"))  # TODO sanitize texts?  # line 758
    elif argument == "unset":  # line 759
        if len(options) < 1:  # line 760
            Exit("No key specified")  # line 760
        if options[0] not in c.keys():  # line 761
            Exit("Unknown key")  # line 761
        del c[options[0]]  # line 762
    elif argument == "add":  # line 763
        if len(options) < 2:  # line 764
            Exit("No key nor value specified")  # line 764
        if options[0] not in CONFIGURABLE_LISTS:  # line 765
            Exit("Unsupported key for add %r" % options[0])  # line 765
        if options[0] not in c.keys():  # add list  # line 766
            c[options[0]] = [options[1]]  # add list  # line 766
        elif options[1] in c[options[0]]:  # line 767
            Exit("Value already contained")  # line 767
        c[options[0]].append(options[1])  # line 768
    elif argument == "rm":  # line 769
        if len(options) < 2:  # line 770
            Exit("No key nor value specified")  # line 770
        if options[0] not in c.keys():  # line 771
            Exit("Unknown key specified: %r" % options[0])  # line 771
        if options[1] not in c[options[0]]:  # line 772
            Exit("Unknown value: %r" % options[1])  # line 772
        c[options[0]].remove(options[1])  # line 773
    else:  # Show  # line 774
        for k, v in sorted(c.items()):  # line 775
            print("%s => %r" % (k, v))  # line 775
        return  # line 776
    f, g = saveConfig(c)  # line 777
    if f is None:  # line 778
        error("Error saving user configuration: %r" % g)  # line 778

def move(relpath: 'str', pattern: 'str', newrelpath: 'str', newpattern: 'str', options: 'List[str]'=[]) -> 'None':  # line 780
    pass  # line 781

def parse(root: 'str', cwd: 'str'):  # line 783
    ''' Main operation. Main has already chdir into VCS root folder, cwd is original working directory for add, rm. '''  # line 784
    debug("Parsing command-line arguments...")  # line 785
    command = sys.argv[1].strip() if len(sys.argv) > 1 else ""  # line 786
    argument = sys.argv[2].strip() if len(sys.argv) > 2 and not sys.argv[2].startswith("--") else None  # line 787
    options = sys.argv[3:] if command == "config" else [c for c in sys.argv[2:] if c.startswith("--")]  # consider second parameter for no-arg command, otherwise from third on  # line 788
    onlys, excps = parseOnlyOptions(cwd, options)  # extracts folder-relative information for changes, commit, diff, switch, update  # line 789
    debug("Processing command %r with argument '%s' and options %r." % ("" if command is None else command, "" if argument is None else argument, options))  # line 790
    if command[:1] in "amr":  # line 791
        relpath, pattern = relativize(root, os.path.join(cwd, argument))  # line 791
    if command[:1] == "m":  # TODO add error message if target pattern is missing  # line 792
        newrelpath, newpattern = relativize(root, os.path.join(cwd, options[0]))  # TODO add error message if target pattern is missing  # line 792
    if command[:1] == "a":  # line 793
        add(relpath, pattern, options)  # line 793
    elif command[:1] == "b":  # line 794
        branch(argument, options)  # line 794
    elif command[:2] == "ch":  # line 795
        changes(argument, options, onlys, excps)  # line 795
    elif command[:3] == "com":  # line 796
        commit(argument, options, onlys, excps)  # line 796
    elif command[:2] == "ci":  # line 797
        commit(argument, options, onlys, excps)  # line 797
    elif command[:3] == 'con':  # line 798
        config(argument, options)  # line 798
    elif command[:2] == "de":  # line 799
        delete(argument, options)  # line 799
    elif command[:2] == "di":  # line 800
        diff(argument, options, onlys, excps)  # line 800
    elif command[:1] == "h":  # line 801
        usage()  # line 801
    elif command[:2] == "lo":  # line 802
        log(options)  # line 802
    elif command[:2] == "li":  # line 803
        ls(argument)  # line 803
    elif command[:2] == "ls":  # line 804
        ls(argument)  # line 804
    elif command[:1] == "m":  # line 805
        move(relpath, pattern, newrelpath, newpattern, options[1:])  # line 805
    elif command[:2] == "of":  # line 806
        offline(argument, options)  # line 806
    elif command[:2] == "on":  # line 807
        online(options)  # line 807
    elif command[:1] == "r":  # line 808
        rm(relpath, pattern)  # line 808
    elif command[:2] == "st":  # line 809
        status()  # line 809
    elif command[:2] == "sw":  # line 810
        switch(argument, options, onlys, excps)  # line 810
    elif command[:1] == "u":  # line 811
        update(argument, options, onlys, excps)  # line 811
    else:  # line 812
        Exit("Unknown command '%s'" % command)  # line 812
    sys.exit(0)  # line 813

def main() -> 'None':  # line 815
    global debug, info, warn, error  # line 816
    logging.basicConfig(level=level, stream=sys.stderr, format=("%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '--log' in sys.argv else "%(message)s"))  # line 817
    _log = Logger(logging.getLogger(__name__))  # line 818
    debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 818
    for option in (o for o in ['--log', '--verbose', '-v', '--sos'] if o in sys.argv):  # clean up program arguments  # line 819
        sys.argv.remove(option)  # clean up program arguments  # line 819
    if '--help' in sys.argv or len(sys.argv) < 2:  # line 820
        usage()  # line 820
        Exit()  # line 820
    command = sys.argv[1] if len(sys.argv) > 1 else None  # type: _coconut.typing.Optional[str]  # line 821
    root, vcs, cmd = findSosVcsBase()  # root is None if no .sos folder exists up the folder tree (still working online); vcs is checkout/repo root folder; cmd is the VCS base command  # line 822
    debug("Found root folders for SOS|VCS: %s|%s" % ("" if root is None else root, "" if vcs is None else vcs))  # line 823
    defaults["defaultbranch"] = vcsBranches.get(cmd, "trunk")  # sets dynamic default with SVN fallback  # line 824
    if force_sos or root is not None or ("" if command is None else command)[:2] == "of" or ("" if command is None else command)[:1] == "h":  # in offline mode or just going offline TODO what about git config?  # line 825
        cwd = os.getcwd()  # line 826
        os.chdir(cwd if command[:2] == "of" else cwd if root is None else root)  # line 827
        parse(root, cwd)  # line 828
    elif cmd is not None:  # online mode - delegate to VCS  # line 829
        info("SOS: Running '%s %s'" % (cmd, " ".join(sys.argv[1:])))  # line 830
        import subprocess  # only required in this section  # line 831
        process = subprocess.Popen([cmd] + sys.argv[1:], shell=False, stdin=subprocess.PIPE, stdout=sys.stdout, stderr=sys.stderr)  # line 832
        inp = ""  # type: str  # line 833
        while True:  # line 834
            so, se = process.communicate(input=inp)  # line 835
            if process.returncode is not None:  # line 836
                break  # line 836
            inp = sys.stdin.read()  # line 837
        if sys.argv[1][:2] == "co" and process.returncode == 0:  # successful commit - assume now in sync again (but leave meta data folder with potential other feature branches behind until "online")  # line 838
            if root is None:  # line 839
                Exit("Cannot determine VCS root folder: Unable to mark repository as synchronized and will show a warning when leaving offline mode")  # line 839
            m = Metadata(root)  # line 840
            m.loadBranches()  # read repo  # line 841
            m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], insync=True)  # mark as committed  # line 842
            m.saveBranches()  # line 843
    else:  # line 844
        Exit("No offline repository present, and unable to detect VCS file tree")  # line 844


# Main part
level = logging.DEBUG if os.environ.get("DEBUG", "False").lower() == "true" or '--verbose' in sys.argv or '-v' in sys.argv else logging.INFO  # line 848
force_sos = '--sos' in sys.argv  # line 849
_log = Logger(logging.getLogger(__name__))  # line 850
debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 850
if __name__ == '__main__':  # line 851
    main()  # line 851
