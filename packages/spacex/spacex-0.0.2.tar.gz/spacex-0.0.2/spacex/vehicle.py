...
def get_all_capsule_details():
    """Returns details for all capsules.
    
    Returns
    -------
    List[:class:`CapsuleDetail`]
        A list of all capsule details.
    """
    data = request('v2', 'parts', 'caps')

    return [CapsuleDetail(c) for c in data]


def get_filtered_capsule_details(**filters):
    """Returns a list of filtered capsule details using the given kwargs.
    ...
    +------------------+----------------------------------+
    | landings         | Filter by number of landings     |
    +------------------+----------------------------------+
    | type             | Filter by type of dragon capsule |
    +------------------+----------------------------------+
    ...
    **filters
    ...
    -------
    List[:class:`CapsuleDetail`]
        A list of filtered capsule details.
    """
    data = request('v2', 'parts', 'caps', **filters)
...
class Capsule:
    ...
    def __repr__(self):
        return '<Capsule id={}>'.format(self.id)


class CapsuleDetail:
    """Represents information on specific capsules.
    
    Attributes
    ----------
    capsule_serial: str
        The capsule's serial.
    status: str
        The capsule's status.
    original_launch: str
        The original launch date and time.
    missions: List[str]
        A list of missions the capsule was used on.
    landings: int
        The amount of landings the capsule performed.
    type: str
        The capsule's type.
    details: str
        Extra details relating to the capsule.
    """
    def __init__(self, data):
        self._from_data(data)
    ...
    def __repr__(self):
        return '<CapsuleDetail capsule_serial={}>'.format(self.capsule_serial)
...
class PayloadWeights:
    ...
    def __repr__(self):
        return '<PayloadWeights id={}>'.format(self.id)
