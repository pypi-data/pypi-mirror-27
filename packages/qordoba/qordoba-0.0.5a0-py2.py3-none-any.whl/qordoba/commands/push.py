...
def upload_file(api, path, file_path, version=None, **kwargs):
    log.info('Uploading {}'.format(path.native_path))

    file_name = path.unique_name
    if file_path:
        file_path = report = os.path.realpath(file_name)
    ...
    resp = api.append_file(resp['upload_id'], file_name, file_path, version_tag=version_tag, **kwargs)

    log.info('Uploaded {} successfully as {}'.format(path.native_path, file_path + file_name))


def update_file(api, path, file_path, remote_files, version=None):
    file_name = path.unique_name
...
def final_push(project, curdir, pattern, api, file_path, update, version):
    ...
    for file in files:
        ...
        if remote_file_pages and update:
            update_file(api, path, file_path, remote_file_pages, version=version)
        else:
            upload_file(api, path, file_path, version=version)


def push_command(curdir, config, file_path=False, update=False, version=None, files=()):
        ...
        for pattern in pattern_list:
            ...
            if pattern is not None and find_directories(pattern):
                    ...
                    for dir_ in directory_list:
                        ...
                        final_push(project, curdir, dir_, api, file_path,  update=False, version=None)
            else:
                final_push(project, curdir, pattern, api, file_path, update=False, version=None)
