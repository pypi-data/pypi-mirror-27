...
- monorepo â€” one monolithic repository containing everything

- multiple repos â€” split project into many repositories, each managing a
  single component
...
- ``pip install mugit`` â€” download and install from PyPI

- ``pip install mugit-1.0.0-py2.py3-none-any.whl`` â€” install from local
  wheel file

- ``pip install -e .`` â€” install in `editable mode`_ from mugit
  source folder

Usage
-----

- ``mugit [COMMAND] --help`` â€” get more help overall or for a specific command

- ``mugit select [-c] <MANIFEST>`` â€” select manifest to use in other
  mugit commands

- ``mugit pull [-c]`` â€” clone missing repositories and fetch/rebase
  existing ones

- ``mugit status [-c] [-a]`` â€” generate report on the state of the workspace

  - Use ``-a`` to list all repositories, not just ones with changes.

- ``mugit update [-c]`` â€” update manifest to reflect current workspace state

- ``mugit add [-c] --all | <PATH>..`` â€” add repositories to the manifest

  - Specify repositories by path or use ``--all`` to automatically scan
    for them.

- ``mugit remove <PATH>..`` â€” remove repositories from the manifest

- ``mugit release <MANIFEST>`` â€” create or update a release/snapshot manifest

- ``mugit --version`` â€” display tool version information
