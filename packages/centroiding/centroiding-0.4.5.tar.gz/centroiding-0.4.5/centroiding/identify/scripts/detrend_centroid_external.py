...
#        super(centroid, self).__init__(parent)
        ...
        if not self.silent: print 'N_neighbours:',self.N_neighbours
...
#            print self.dic_nb['corrcoeff_y']
        ...
        if not self.silent: print 'N_top_x', N_top_x
        if not self.silent: print 'N_top_y', N_top_y
...
#        ind_y = np.argpartition(self.dic_nb['corrcoeff_y'], -N_top)[-N_top:]
        ...
        if not self.silent: print self.dic_nb['corrcoeff_x'][ind_x]
        if not self.silent: print self.dic_nb['corrcoeff_y'][ind_y]
        
        if not self.silent: print 'corrcoeff analysis done.'
...
#            result = differential_evolution(errfct, bounds=bounds, args=(centdxy_phase, xy_phase)) 
        ...
        if self.valid:
            ...
            warnings.warn('Reference set to constant zero!')
