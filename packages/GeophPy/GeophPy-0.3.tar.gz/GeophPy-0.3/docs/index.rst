...
- an ascii file (*.csv* for instance):
  ...
  The first line must contain the geographic reference system and the other lines contain the point number followed by its GPS coordinates (longitude and lattitude or UTM Easting and Northing) and eventually its local coordinates:
