...
class EncryptedField(TypedField):
    ...
    def encrypt(self, data):
        ...
        if not data or data.startswith(self.crypt_header):
            return data
        return (self.crypt_header + self._crypt.encrypt(data)).decode()

    def decrypt(self, data):
        data = data.encode() if isinstance(data, str) else data
        if not data.startswith(self.crypt_header):
            return data
        return (self._crypt.decrypt(data[len(self.crypt_header):])).decode()
