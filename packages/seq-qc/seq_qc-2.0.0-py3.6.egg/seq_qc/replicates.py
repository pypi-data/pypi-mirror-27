#! /usr/bin/env python
"""
Remove replicate sequences. The types of replicates that this program can 
check for are exact matches, 5'-prefixes, or reverse-complement exact and 
5'-prefixes.
 
For split read pairs:
    filter_replicates [flags] -o out.forward -v out.reverse in.forward \
        in.reverse
...
or single-end reads, use /dev/stdin to indicate that input should be taken 
from standard input (stdin). Similarly, leaving out the -o argument will cause 
