...
<https://cdaweb.gsfc.nasa.gov/WebServices/REST/>_` for detailed
...
def get_observatory_groups(dataview, instrumentType=None):
    """Queries server for descriptions of observatory groups. Refer to
    ...
    """
    url = '/'.join(
        [AI_CDAS_BASEURL, 'dataviews', dataview, 'observatoryGroups']
    )
...
def get_instrument_types(dataview, observatory=None, observatoryGroup=None):
    """Queries server for descriptions of instrument types. Refer to
    `CDAS RESTful Web Services
    <https://cdaweb.gsfc.nasa.gov/WebServices/REST/>_` for detailed
    description of parameters.
    ...
    """
    url = '/'.join(
        [AI_CDAS_BASEURL, 'dataviews', dataview, 'instrumentTypes']
    )
    params = {
...
def get_instruments(dataview, observatory=None):
    """Queries server for descriptions of the instruments. Refer to
    `CDAS RESTful Web Services
    <https://cdaweb.gsfc.nasa.gov/WebServices/REST/>_` for detailed
    description of parameters.
...
def get_observatories(dataview, instrument=None, instrumentType=None):
    """Queries server for descriptions of the observatories. Refer to
    `CDAS RESTful Web Services
    <https://cdaweb.gsfc.nasa.gov/WebServices/REST/>_` for detailed
    description of parameters.
...
def get_observatory_groups_and_instruments(dataview, instrumentType=None):
    ...
    get_observatories, and get_instruments. Refer to `CDAS RESTful Web
    Services <https://cdaweb.gsfc.nasa.gov/WebServices/REST/>_` for
...
def get_datasets(
        ...
        labelPattern=None,
        notesPattern=None):
    """Queries server for descriptions of the datasets. Refer to `CDAS
...
def get_inventory(dataview, dataset):
    ...
    dataset. Refer to `CDAS RESTful Web Services
    <https://cdaweb.gsfc.nasa.gov/WebServices/REST/>_` for detailed
    description of parameters.
...
def get_variables(dataview, dataset):
    """Queries server for descriptions of variables in a dataset. Refer
    to `CDAS RESTful Web Services
    <https://cdaweb.gsfc.nasa.gov/WebServices/REST/>_` for detailed
    description of parameters.
...
def get_data(
        ...
        cdf=False,
        progress=True):
    """Queries server (and data cache) for data. Refer to `CDAS RESTful
    Web Services <https://cdaweb.gsfc.nasa.gov/WebServices/REST/>_` for
    detailed description of parameters.
    ...
    if cdf:
        ...
        ext = 'txt'
    if AI_CDAS_CACHE:
        cached_filename = '.'.join(
            [
                hashlib.md5(
                    (uri+params['format']).encode('utf-8')
                ).hexdigest(),
                ext
            ]
        )
