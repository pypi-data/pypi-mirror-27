"""VirtuinTestView consists of two classes: VirtuinTestViewServer
and VirtuinTestViewClient
VirtuinTestViewServer shall be used by test process to listen for
requests from companion test view.
VirtuinTestViewClient shall be used by test view to send requests
to running test process.
"""
...
class VirtuinTestViewServer(object):
    ...
    def __init__(self, stationName, testName):
        ...
        self.exName = str.format('{:s}_view', self.stationName)
        self.qName = testName
        self.conn = None
        self.ch = None
        self.isOpen = False
...
class VirtuinTestViewClient(object):
    """VirtuinTestViewServer is used to perform requests to view server
    over AMQP (via RabbitMQ).
