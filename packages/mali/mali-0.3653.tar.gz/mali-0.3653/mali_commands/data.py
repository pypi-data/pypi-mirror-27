...
@click.pass_context
def sync_to_data_volume(ctx, volumeid, files, commit, processes, no_progressbar):
    with with_repo(ctx.obj.config, volumeid) as repo:
        data_sync = DataSync(ctx, repo, no_progressbar)
        combined_meta_files, combined_index_files = data_sync.create_combined_index_and_metadata(files)
