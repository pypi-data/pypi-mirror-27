...
def with_repo_dynamic(ctx, volume_id):
    repo_root = default_data_volume_path(volume_id)

    config = ctx.obj.config

    try:
        return closing(MlRepo(config, repo_root))
    except NotGitRepository:
        map_volume(ctx, volume_id, None)
        return closing(MlRepo(config, repo_root, read_only=True, require_path=False))
...
def with_repo(config, volume_id, read_only=False, require_map=True):
    ...
    try:
        return closing(MlRepo(config, repo_root, read_only=read_only, require_path=require_map))
    except NotGitRepository:
