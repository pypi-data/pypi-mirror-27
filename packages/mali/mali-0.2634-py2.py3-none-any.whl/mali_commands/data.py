...
def clone_data(volume, query, path, config_path, config_prefix):
    ...
    def download_all(current_results):
        ...
        while True:
            for item in results:
                ...
                multi_process_control.execute(
                    __download_entity, args=(config_init_dict, volumeid, dest, item), callback=on_download)
    ...
    try:
        with with_repo(config, volumeid, read_only=True) as repo:
            ...
            with tqdm(total=total_data_points, desc="Downloading", unit=' data point', ncols=80, disable=True) as bar:
