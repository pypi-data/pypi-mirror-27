...
from mali_commands.path_utils import expend_and_validate_path, get_total_files_in_path, add_sys_var, fill_in_vars, \
    safe_make_dirs, path_elements, sys_vars_in_path, enumerate_paths, get_batch_of_files_from_paths, safe_rm_tree
...
def __get__full_oath(dest, item):
    phase = item.get('@phase')

    path_no_ext, file_extension = os.path.splitext(item['@path'])
    ...
    add_sys_var('ext', file_extension, item)
    add_sys_var('extension', file_extension, item)
    add_sys_var('base_name', os.path.basename(path_no_ext), item)
...
def clone_data(ctx, volumeid, dest, query, delete, batchsize, processes, no_progressbar):
    ...
    has_vars = os.path.abspath(root_dest) != dest

    has_phase_in_dest = sys_vars_in_path(dest, ['phase', '$@'])
    if not has_vars and not has_phase_in_dest:
        dest = os.path.join(dest, '$@')

    if dest.endswith(os.sep) or '$' not in dest:
    ...
    def download_all(current_results):
        ...
        while True:
            for item in results:
                ...
                full_path = __get__full_oath(dest, item)
