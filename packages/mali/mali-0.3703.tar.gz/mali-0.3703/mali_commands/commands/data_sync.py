...
def _download_entity(config_init_dict, volume_id, dest_file, item):
    DataSync.download_entity(config_init_dict, volume_id, dest_file, item)
...
class DataSync(object):
    ...
    def __upload_file_for_processing(self, file_obj, file_description):
        ...
        data_object_name = '%s/temp/%s.json' % (volume_id, uuid4().hex)
    ...
    @classmethod
    def download_entity(cls, config_init_dict, volume_id, dest_file, item):
    ...
    def __download_results(self, process_control, dest_pattern, results, callback):
        ...
        for item in results:
            ...
            full_path = DestPathEnum.get_full_path(dest_pattern, item)

            process_control.execute(
                _download_entity, args=(config_init_dict, volume_id, full_path, item),
                callback=wrap_callback_func(item, full_path))

    def __download_all(self, process_control, query, root_folder, dest_pattern, batch_size):
        ...
        with tqdm(total=total_data_points, desc="Downloading", unit=' data point', ncols=80, disable=self.__no_progressbar) as bar:
            while True:
                self.__download_results(process_control, dest_pattern, results, handle_item)
