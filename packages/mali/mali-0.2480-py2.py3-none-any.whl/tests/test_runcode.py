...
def normalized_src(source):
    return parse.quote(source_tracking.remote_to_template(source), safe='')
...
class TestAuth(BaseCliTest):
    ...
    def test_code_works_with_explicit_source(self, handle_api):
        ...
        encoded_url = normalized_src(hurl)

        handle_api.expects_call().with_matching_args(
            arg.any(),  # ctx
            arg.any(),  # http method
            '{}/code/track/{}'.format(org_id, encoded_url),
            {
                'target': gurl
            }
        ).returns({})
