...
def lib_show(library, json_output):
    ...
    if "repository" in lib:
        click.echo("Repository Type: %s, URL: %s" %
                (lib['repository']['type'], lib['repository']['url']))
    ...
    for author in lib.get("authors", []):
        ...
        for key in ("name", "email", "url", "maintainer"):
            if not author.has_key(key):
                continue
            if key == "email":
                _data.append(("<%s>" % author[key]) if (author[key] != "") else "")
            elif key == "maintainer":
                _data.append("(maintainer)" if author[key] else "")
            else:
