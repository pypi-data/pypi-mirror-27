...
def _get_registry_platforms():
    platforms = util.get_api_result("/v1/platform", cache_valid="30d")
    pm = PlatformManager()
...
def platform_frameworks(query, json_output):
    frameworks = []
    for framework in util.get_api_result("/v1/framework", cache_valid="30d"):
        if query == "all":
            query = ""
        ...
        if query and query.lower() not in search_data.lower():
            continue
        framework['homepage'] = (
            "http://docs.intoyun.com/ide/frameworks/" + framework['name'])
        framework['platforms'] = [
