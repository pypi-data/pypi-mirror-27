...
class ErrorMissingConfig(Exception):
    pass
...
class Vlab(object):
    ...
    def __init__(self, working_directory=None, config_file=None, gdb_mode=False, sudo_mode=False, print_trace=False, trace_output_file="", print_uart=False, uart_output_file=""):
        ...
        if not os.path.isfile(config_file):
            raise ErrorMissingConfig('Config file not found at: {}'.format(os.path.abspath(config_file)))
        ...
        if not self._local_jemu:
            if token is not None:
                ...
                raise ErrorMissingConfig('Config file not found at: {}'.format(config_file))
        ...
        if self._local_jemu:
            if gen_new:
                ...
                subprocess.check_call(
                    transpiler_cmd, cwd=self._transpiler_dir, stdout=open(os.devnull, 'w'), stderr=None
                )
            else:
                make_cmd = ['make', '-C', 'emulator', 'DEBUG=1', 'PLATFORM=nrf52', '-j8']
                subprocess.check_call(
                    make_cmd, cwd=self._transpiler_dir, stdout=open(os.devnull, 'w'), stderr=None
                )
