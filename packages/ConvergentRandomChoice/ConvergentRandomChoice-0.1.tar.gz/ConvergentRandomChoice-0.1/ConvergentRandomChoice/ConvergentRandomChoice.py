...
class ConvergentRandomChoice:
    def __init__(self, iterable, pchange='uniform', initials=None):
        flag=False
        if type(pchange) == str and pchange != 'uniform':
            flag=True
        elif type(pchange) == float and (pchange < 0 or pchange > 1):
            flag=True
        elif type(pchange) == int and not (pchange == 0 or pchange == 1):
            flag=True
        if flag: raise ValueError("pchange must be a float value between 0 and 1 or 'uniform'")
        
        if initials and (len(initials) != len(iterable) or not isclose(sum(initials), 1)):
            raise ValueError("Error with initial probabilities, either the correct number of values is not passed in or probabilities do not sum to 1.")
