...
def target2orient(target, orient_type='quat'):
    ...
    orientation : array (3,) or (N,3)
        ...
        For rotation matrices, same sequence the matrices [deg].
        Note that the last column will always be zero, because a rotation about
    ...
    >>> skinematics.vector.target2orientation([3, 0, 0])
    array([[xxx]])
    
    >>> v = [[pi, 2, 3], [2, 0, 0]]
    >>> skinematics.vector.target2orientation(v)
    array([[xxx],
       [ xxx]])
    ...
    if orient_type == 'quat':
        return q_shortest_rotation([1,0,0], target)
...
if __name__=='__main__':
    ...
    orientation = target2orient(np.array([a,b]))
    print(orientation)
