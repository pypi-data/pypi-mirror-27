...
class Rollingpredict:
    ...
    def __init__(self, DataFrame = 0, path = 0, pathmode = 1):
        """This function will initialize the class, and the DataFrame or path will contain the DataFrame or path for the datafile you want to use later. If pathmode ==2, then the datas contain semicolon (default is 1, which is comma). An example to use: Rollingpredict(path ='D:\Datafiles\data.csv') """
        if DataFrame == 0:
            self.full_path = path
            self.mode = 2
        if path == 0:
            self.df_dataframe = DataFrame
            self.mode = 1
        if pathmode == 2:
            self.pathmode = 2
        else: 
    ...
    def read_original(self):
        ...
        if self.mode == 1:
            self.df_original = self.df_dataframe
        if self.mode == 2 and self.pathmode == 1:
            self.df_original = pd.read_csv(self.full_path)
        if self.mode == 2 and self.pathmode == 2:
            self.df_original = pd.read_csv(self.full_path, sep=';')
    ...
    def read_calendar(self, DataFrame, path, pathmode): 
        ...
        if DataFrame == 0 and pathmode ==1:
            self.df_calendar = pd.read_csv(path)
        if DataFrame == 0 and pathmode ==2:
            self.df_calendar = pd.read_csv(path, sep=';')
        if path == 0:
            self.df_calendar = DataFrame
    ...
    def ordered_predicted_values_custom(self, startindex, endindex, showcustom):
        """This function will create a dataframe. The dataframe contains the predicted values and their datas between the startindex and endindex of the originalindex"""
    ...
    def ordered_predicted_values_custom_tocsv(self, startindex, endindex, name):
        """This function will create a pdf. The pdf contains the predicted values and their datas between the startindex and endindex of the originalindex"""
        self.y_predictedvalues_all = []
        self.y_originalvalues_all = []
        self.indexoriginal_predictedvalues_all = []
    ...
    def draw_plot_error_custom(self, showmae = -1, showrmse = -1, showmape = -1, name = 'Test_1.png'):
