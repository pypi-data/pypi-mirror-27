...
logging.basicConfig(level=getattr(logging, os.getenv('LOG', 'INFO'), logging.INFO))
...
class PubSub(object):
        ...
        if self.mq:
            ...
            if self.pre_pipe:
                self.consumer = KafkaConsumer(
                    self.pre_pipe,
                    bootstrap_servers=self.mq,
                    group_id='group_{:s}'.format(self.pre_pipe)
                )
    ...
    @staticmethod
    def r2s(msg):
        return '{:s} {:<5.5s} {:<15.15s} {:<s}'.format(msg['id'], msg.get('url_type', ' '), msg['title'], msg['url'])
