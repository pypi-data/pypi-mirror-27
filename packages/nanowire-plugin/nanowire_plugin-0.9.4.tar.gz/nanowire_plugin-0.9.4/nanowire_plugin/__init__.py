...
class on_request_class():
    ...
    def __init__(self, function, name, minio_client, output_channel, monitor_url):
        ...
        if not str(type(output_channel)) == "<class 'pika.adapters.blocking_connection.BlockingChannel'>" and "mock" not in str(type(output_channel)):
...
def send(name, payload, input_channel, output_channel, method, properties, minio_client, monitor_url, function):
    ...
    if not str(type(input_channel)) == "<class 'pika.adapters.blocking_connection.BlockingChannel'>"  and "mock" not in str(type(input_channel)):
