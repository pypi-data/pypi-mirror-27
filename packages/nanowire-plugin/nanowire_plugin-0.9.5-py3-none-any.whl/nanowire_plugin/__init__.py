...
class heart_runner():
    ...
    def __init__(self, connection):
        ...
        if "pika.connection" not in str(type(connection)) and "mock" not in str(connection).lower():
...
def clean_function_output(result, payload):
    ...
    if not isinstance(result, dict):
        logger.error("Return value from clean function output is not a dictionary it is:- %s"%result)
