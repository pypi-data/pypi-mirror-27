...
with open(ExecutorFiles.SETTINGS, 'rt') as _settings_file:
    DESERIALIZED_FILES[ExecutorFiles.SETTINGS] = json.load(_settings_file)
    for _file_name in DESERIALIZED_FILES[ExecutorFiles.SETTINGS][ExecutorFiles.FILE_LIST_KEY]:
        with open(_file_name, 'rt') as _json_file:
            DESERIALIZED_FILES[_file_name] = json.load(_json_file)

SETTINGS = DESERIALIZED_FILES[ExecutorFiles.SETTINGS]
...
class BaseFlowExecutor(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.queue_response_channel = '{}.{}'.format(SETTINGS['REDIS_CHANNEL_PAIR'][1], DATA['id'])
    ...
    def _send_manager_command(self, cmd, expect_reply=True, extra_fields={}):
        ...
        queue_channel = SETTINGS['REDIS_CHANNEL_PAIR'][0]
        try:
        ...
        while True:
            for _ in range(3):
                ...
                # NOTE: If there's still no response after three seconds, the system is broken
                ...
                raise RuntimeError("No response from the manager after 3 retries.")
