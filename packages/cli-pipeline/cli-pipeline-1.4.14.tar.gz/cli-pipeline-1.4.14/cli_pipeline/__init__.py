...
#    _default_input_path = './pipeline_input'
    ...
    def version(self):
        ...
        print('capabilities_available: %s' % ['train-cluster-*', 'predict-cluster-*', 'predict-test-stream', 'optimize-*'])
        print('')
    ...
    def _create_predict_server_Dockerfile(self,
        ...
        rendered_Dockerfile = '%s/.pipeline-generated-%s-%s-%s-%s-%s-Dockerfile' % (build_context_path, image_registry_namespace, model_runtime, model_type, model_name, model_tag)
...
#            rendered_filenames += [rendered_filename]
        ...
        model_router_routerules_yaml_templates_path = os.path.join(pipeline_templates_path, PipelineCli._kube_router_routerules_template_registry['predict-router-split'][0][0])
        path, filename = os.path.split(model_router_routerules_yaml_templates_path)
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(context)
        rendered_filename = './.pipeline-generated-%s-%s-%s-%s-%s-router-split-routerules.yaml' % (image_registry_namespace, model_runtime, model_type, model_name, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
