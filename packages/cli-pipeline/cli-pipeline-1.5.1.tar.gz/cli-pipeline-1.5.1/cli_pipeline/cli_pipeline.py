...
class PipelineCli(object):
    ...
    def _create_stream_kube_Kubernetes_yaml(self,
                                            model_name,
                                            log_stream_topic,
                                            input_stream_topic,
                                            output_stream_topic,
        ...
        if not image_registry_namespace:
            image_registry_namespace = PipelineCli._default_image_registry_stream_namespace
        ...
        model_stream_svc_yaml_templates_path = os.path.normpath(os.path.join(pipeline_templates_path, PipelineCli._kube_stream_svc_template_registry['stream'][0][0]))
        path, filename = os.path.split(model_stream_svc_yaml_templates_path)
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-svc.yaml' % (image_registry_namespace, model_name))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]

        model_stream_deploy_yaml_templates_path = os.path.normpath(os.path.join(pipeline_templates_path, PipelineCli._kube_stream_deploy_template_registry['stream'][0][0]))
        path, filename = os.path.split(model_stream_deploy_yaml_templates_path)
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-deploy.yaml' % (image_registry_namespace, model_name))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
    ...
    def predict_server_start(self,
        ...
        cmd = 'docker run -itd -p %s:8080 -p %s:9090 -p %s:3000 -e PIPELINE_LOG_STREAM_URL=%s -e PIPELINE_LOG_STREAM_TOPIC=%s -e PIPELINE_INPUT_STREAM_URL=%s -e PIPELINE_INPUT_STREAM_TOPIC=%s -e PIPELINE_OUTPUT_STREAM_URL=%s -e PIPELINE_OUTPUT_STREAM_TOPIC=%s --name=%s -m %s %s/%s/%s-%s:%s serve' % (predict_port, prometheus_port, grafana_port, log_stream_url, log_stream_topic, input_stream_url, input_stream_topic, output_stream_url, output_stream_topic, container_name, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
        print("")
    ...
    def predict_kube_start(self,
        ...
        return "Use 'pipeline predict_kube_describe' to get the model endpoint url (hostname or ip) for '%s'" % model_name


    def predict_kafka_start(self,
                            model_name,
                            log_stream_topic=None,
                            input_stream_topic=None,
                            output_stream_topic=None,
        ...
        rendered_yamls = self._create_stream_kube_Kubernetes_yaml(
                                          model_name=model_name,
                                          log_stream_topic=log_stream_topic,
                                          input_stream_topic=input_stream_topic,
                                          output_stream_topic=output_stream_topic,
        ...
        for rendered_yaml in rendered_yamls:
            self._kube_apply(yaml_path=rendered_yaml,
                             cluster_namespace=cluster_namespace)


    def predict_kafka_describe(self,
                                model_name,
                                cluster_namespace=None,
                                image_registry_namespace=None,
                                timeout_seconds=15):
        ...
        stream_url = self._get_cluster_service(service_name=service_name,
                                               cluster_namespace=cluster_namespace)
        ...
        response = requests.get(url=full_stream_url,
    ...
    def predict_kafka_consume(self,
                              model_name,
                              stream_topic,
        ...
        response = requests.post(url=full_stream_url,
        ...
        response = requests.delete(url=full_stream_url,
                                   headers=content_type_headers,
                                   timeout=timeout_seconds)
    ...
    def predict_kafka_test(self,
        ...
        print("")
        print("Producing stream for topic '%s' at '%s'." % (stream_topic, full_stream_url))
        print("")
        ...
        return self.predict_kafka_consume(model_name,
                                          output_stream_topic) 
    ...
    def predict_server_test(self,
        ...
        with ThreadPoolExecutor(max_workers=test_request_concurrency) as executor:
            for _ in range(test_request_concurrency):
                executor.submit(self.predict_http_test(
    ...
    def predict_kube_test(self,
        ...
        if not model_endpoint_url:
            ...
            model_endpoint_url = self._get_cluster_ingress(ingress_name=ingress_name,
                                                           cluster_namespace=cluster_namespace)
        ...
        model_endpoint_url = 'http://%s/%s/invocations' % (model_endpoint_url, model_name)
    ...
    def _get_cluster_service(self,
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            service = kubeclient_v1.read_namespaced_service(service_name,
                                                            cluster_namespace)
...
#        pass
    ...
    def train_server_start(self,
        ...
        cmd = 'docker run -itd -p 2222:2222 -p 6006:6006 -e PIPELINE_LOG_STREAM_URL=%s -e PIPELINE_LOG_STREAM_TOPIC=%s -e PIPELINE_INPUT_STREAM_URL=%s -e PIPELINE_INPUT_STREAM_TOPIC=%s -e PIPELINE_OUTPUT_STREAM_URL=%s -e PIPELINE_OUTPUT_STREAM_TOPIC=%s -e TF_CONFIG=%s -e PIPELINE_MODEL_TRAIN_ARGS=%s -v %s:/opt/ml/input/ -v %s:/opt/ml/output/ --name=%s -m %s %s/%s/%s-%s:%s train' % (log_stream_url, log_stream_topic, input_stream_url, input_stream_topic, output_stream_url, output_stream_topic, tf_config_local_run, train_args, input_path, output_path, image_registry_namespace, container_name, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
