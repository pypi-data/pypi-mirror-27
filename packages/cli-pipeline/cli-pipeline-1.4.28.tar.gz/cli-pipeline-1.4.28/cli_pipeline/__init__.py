...
#             pipeline experiment-update           <-- Update Experiment (Bandit-based Routing)
    ...
    def train_server_push(self,
        ...
        cmd = 'docker push %s/%s/%s-%s:%s-%s-%s-%s' % (image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_tag, mode_type, model_runtime, model_chip)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
