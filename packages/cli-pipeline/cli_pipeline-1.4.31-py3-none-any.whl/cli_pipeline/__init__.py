...
class PipelineCli(object):
    ...
    def version(self):
        ...
        print('')
        print('capabilities_enabled: %s' % ['train-server-*', 'predict-server-*', 'predict-test-http', 'predict-test-sage'])
    ...
    def _create_predict_cluster_Kubernetes_yaml(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-%s-%s-%s-%s-router-split-autoscale.yaml' % (image_registry_namespace, model_name, model_tag, model_type, model_runtime, model_chip))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
    ...
    def predict_server_pull(self,
        ...
        cmd = 'docker pull %s/%s/%s-%s:%s' % (image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_tag, model_type, model_runtime, model_chip)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def predict_server_start(self,
        ...
        cmd = 'docker run -itd -p 8080:8080 -p 9090:9090 -p 3000:3000 -e PIPELINE_INPUT_STREAM_URL=%s -e PIPELINE_INPUT_STREAM_TOPIC=%s -e PIPELINE_OUTPUT_STREAM_URL=%s -e PIPELINE_OUTPUT_STREAM_TOPIC=%s --name=%s-%s-%s-%s-%s-%s -m %s %s/%s/%s-%s:%s serve' % (input_stream_url, input_stream_topic, output_stream_url, output_stream_topic, image_registry_namespace, model_name, model_tag, model_type, model_runtime, model_chip, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
...
#             pipeline experiment-update           <-- Update Experiment (Bandit-based Routing)
    ...
    def train_server_start(self,
        ...
        cmd = 'docker run -itd -p 2222:2222 -p 6006:6006 -e PIPELINE_INPUT_STREAM_URL=%s -e PIPELINE_INPUT_STREAM_TOPIC=%s -e PIPELINE_OUTPUT_STREAM_URL=%s -e PIPELINE_OUTPUT_STREAM_TOPIC=%s -e TF_CONFIG=%s -e PIPELINE_MODEL_TRAIN_ARGS=%s -v %s:/opt/ml/input/ -v %s:/opt/ml/output/ --name=%s-%s-%s-%s-%s-%s -m %s %s/%s/%s-%s:%s train' % (input_stream_url, input_stream_topic, output_stream_url, output_stream_topic, tf_config_local_run, train_args, input_path, output_path, image_registry_namespace, model_name, model_tag, model_type, model_runtime, model_chip, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_tag)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def _create_train_cluster_Kubernetes_yaml(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-cluster-%s-%s-%s-%s-%s.yaml' % (image_registry_namespace, model_name, model_tag, model_type, model_runtime, model_chip))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
        print("'%s' => '%s'." % (filename, rendered_filename))
