...
class PipelineCli(object):
    ...
    def version(self):
        ...
        print('')
        print('default train base image: %s/%s/%s-%s:%s' % (PipelineCli._default_image_registry_url, PipelineCli._default_image_registry_repo, PipelineCli._default_image_registry_train_namespace, PipelineCli._default_image_registry_base_chip, PipelineCli._default_image_registry_base_tag))
        print('default predict base image: %s/%s/%s-%s:%s' % (PipelineCli._default_image_registry_url, PipelineCli._default_image_registry_repo, PipelineCli._default_image_registry_predict_namespace, PipelineCli._default_image_registry_base_chip, PipelineCli._default_image_registry_base_tag))
        print('')
    ...
    def predict_cluster_connect(self,
        ...
        service_name = '%s-%s-%s-%s-%s-%s' % (image_registry_namespace, model_name, model_chip, model_tag, model_runtime, model_type)
    ...
    def _create_predict_server_Dockerfile(self,
        ...
        rendered_Dockerfile = os.path.normpath('%s/.pipeline-generated-%s-%s-%s-%s-%s-%s-Dockerfile' % (build_context_path, image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime))
    ...
    def predict_server_build(self,
        ...
        if build_type == 'docker':
            ...
            cmd = 'docker build -t %s/%s/%s-%s-%s:%s-%s-%s -f %s %s' % (image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime, generated_Dockerfile, build_context_path)
    ...
    def _create_predict_cluster_Kubernetes_yaml(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-%s-%s-%s-%s-router-split-deploy.yaml' % (image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime))
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-%s-%s-%s-router-split-ingress.yaml' % (image_registry_namespace, model_name, model_chip, model_type, model_runtime))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-%s-%s-%s-router-split-svc.yaml' % (image_registry_namespace, model_name, model_chip, model_type, model_runtime))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-%s-%s-%s-%s-router-split-autoscale.yaml' % (image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
    ...
    def predict_server_shell(self,
        ...
        cmd = 'docker exec -it %s-%s-%s-%s-%s-%s bash' % (image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def predict_server_push(self,
        ...
        cmd = 'docker push %s/%s/%s-%s-%s:%s-%s-%s' % (image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def predict_server_pull(self,
        ...
        cmd = 'docker pull %s/%s/%s-%s-%s:%s-%s-%s' % (image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def predict_server_start(self,
        ...
        cmd = 'docker run -itd -p 8080:8080 -p 9090:9090 -p 3000:3000 -e PIPELINE_INPUT_STREAM_URL=%s -e PIPELINE_INPUT_STREAM_TOPIC=%s -e PIPELINE_OUTPUT_STREAM_URL=%s -e PIPELINE_OUTPUT_STREAM_TOPIC=%s --name=%s-%s-%s-%s-%s-%s -m %s %s/%s/%s-%s-%s:%s-%s-%s serve' % (input_stream_url, input_stream_topic, output_stream_url, output_stream_topic, image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def predict_server_stop(self,
        ...
        cmd = 'docker rm -f %s-%s-%s-%s-%s-%s' % (image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
    ...
    def predict_server_logs(self,
        ...
        cmd = 'docker logs -f %s-%s-%s-%s-%s-%s' % (image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
    ...
    def _tar(self,
        ...
        tar_filename = '%s-%s-%s-%s-%s.tar.gz' % (model_name, model_chip, model_tag, model_type, model_runtime)
    ...
    def _predict_cluster_deploy(self,
        ...
        full_model_deploy_url = "%s/api/%s/model/deploy/%s/%s/%s/%s/%s" % (deploy_server_url.rstrip('/'), PipelineCli._pipeline_api_version, model_name, model_chip, model_tag, model_type, model_runtime) 
    ...
    def _test_single_prediction_sage(self,
        ...
        print("")
        print("Predicting with file '%s' using '%s'" % (test_request_path, model_endpoint_name))
        print("")
    ...
    def predict_cluster_shell(self,
        ...
        service_name = '%s-%s-%s-%s-%s-%s' % (image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
    ...
    def predict_cluster_autoscale(self,
        ...
        cmd = "kubectl autoscale deployment %s-%s-%s-%s-%s-%s --cpu-percent=%s --min=%s --max=%s" % (image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime, cpu_percent, min_replicas, max_replicas)
        print("")
        print("Running '%s'." % cmd)
        print("")
        subprocess.call(cmd, shell=True)
    ...
    def traffic_router_split(self,
        ...
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-%s-%s-%s-%s-router-split-routerules.yaml' % (image_registry_namespace, model_name, model_chip, model_type, model_runtime, tag_weight_filename_snippet))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
...
#             pipeline experiment-update           <-- Update Experiment (Bandit-based Routing)
    ...
    def _create_train_server_Dockerfile(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('%s/.pipeline-generated-%s-%s-%s-%s-%s-%s-Dockerfile' % (build_context_path, image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
    ...
    def train_server_start(self,
        ...
        cmd = 'docker run -itd -p 2222:2222 -p 6006:6006 -e PIPELINE_INPUT_STREAM_URL=%s -e PIPELINE_INPUT_STREAM_TOPIC=%s -e PIPELINE_OUTPUT_STREAM_URL=%s -e PIPELINE_OUTPUT_STREAM_TOPIC=%s -e TF_CONFIG=%s -e PIPELINE_MODEL_TRAIN_ARGS=%s -v %s:/opt/ml/input/ -v %s:/opt/ml/output/ --name=%s-%s-%s-%s-%s-%s -m %s %s/%s/%s-%s-%s:%s-%s-%s train' % (input_stream_url, input_stream_topic, output_stream_url, output_stream_topic, tf_config_local_run, train_args, input_path, output_path, image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime, memory_limit, image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)
    ...
    def _create_train_cluster_Kubernetes_yaml(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-cluster-%s-%s-%s-%s-%s.yaml' % (image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
        print("'%s' => '%s'." % (filename, rendered_filename))
    ...
    def _sage_tag_and_push(self,
        ...
        cmd = "docker tag %s/%s/%s-%s-%s:%s-%s-%s %s/%s/%s-%s-%s:%s-%s-%s" % (src_image_registry_url, src_image_registry_repo, src_image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime, dest_image_registry_url, dest_image_registry_repo, dest_image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)

        cmd = "docker push %s/%s/%s-%s-%s:%s-%s-%s" % (dest_image_registry_url, dest_image_registry_repo, dest_image_registry_namespace, model_name, model_chip, model_tag, model_type, model_runtime)
        print(cmd)
        print("")
        process = subprocess.call(cmd, shell=True)        
