...
class PipelineCli(object):
    ...
    def version(self):
        ...
        print('capabilities_available: %s' % ['train-cluster-*', 'predict-cluster-*', 'predict-test-stream', 'optimize-predict-*', 'optimize-train-*', 'traffic-router-*', 'spark-cluster-*', 'airflow-cluster-*', 'jupyter-cluster-*', 'presto-cluster-*', 'redis-cluster-*', 'elasticsearch-cluster-*', 'cassandra-cluster-*', 'zeppelin-cluster-*', 'kafka-cluster-*', 'mysql-cluster-*'])
        print('')
    ...
    def validate_and_prep_model_tag(self,
                                    model_tag):
    ...
    def validate_and_prep_model_weight_list(self,
                                            model_weight_list):
    ...
    def predict_cluster_connect(self,
        ...
        model_tag = self.validate_and_prep_model_tag(model_tag)
...
#        return rendered_filenames
    ...
    def traffic_router_split(self,
        ...
        self.validate_and_prep_model_weight_list(model_weight_list)

        model_tag_list = [ self.validate_and_prep_model_tag(model_tag) for model_tag in model_tag_list ]
        ...
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-%s-%s-%s-router-split-routerules.yaml' % (image_registry_namespace, model_runtime, model_type, model_name, tag_weight_filename_snippet))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'." % (filename, rendered_filename))
