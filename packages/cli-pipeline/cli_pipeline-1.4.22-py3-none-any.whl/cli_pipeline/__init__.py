...
class PipelineCli(object):
    ...
    def _create_predict_cluster_Kubernetes_yaml(self,
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-%s-%s-%s-router-split-ingress.yaml' % (image_registry_namespace, model_name, model_type, model_runtime, model_chip))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
        ...
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = os.path.normpath('.pipeline-generated-%s-%s-%s-%s-%s-router-split-svc.yaml' % (image_registry_namespace, model_name, model_type, model_runtime, model_chip))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
    ...
    def _test_single_prediction_http(self,
        ...
        #print('model_endpoint_url: %s' % model_endpoint_url)
        #print('model_runtime: %s' % model_runtime)
        #print('model_type: %s' % model_type)
        #print('model_name: %s' % model_name)
        #print('model_tag: %s' % model_tag)
        ...
        #print('test_request_timeout_seconds: %s' % test_request_timeout_seconds)
    ...
    def _test_single_prediction_sage(self,
        ...
        #tmp_output_path = '.pipeline-sage-output'
        #cmd = "aws runtime.sagemaker invoke-endpoint --endpoint-name='%s' --body='%s' %s" % (model_endpoint_name, body, tmp_output_path)
        #process = subprocess.call(cmd, shell=True)
        end_time = datetime.now()
