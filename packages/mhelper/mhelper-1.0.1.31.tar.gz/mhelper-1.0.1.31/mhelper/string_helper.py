...
def fix_width( text: object, width: int = 20, char = " ", justify = -1 ) -> str:
    if justify < 0:
        justify_fn = ljust
    elif justify == 0:
        justify_fn = cjust
    ...
    return justify_fn( max_width( str( text ), width ), width, char )
...
def cjust( text: str, width: int, char: str ) -> str:
    ...
    text = rjust( text, needed // 2, char )
    text = ljust( text, needed, char )
    return text
