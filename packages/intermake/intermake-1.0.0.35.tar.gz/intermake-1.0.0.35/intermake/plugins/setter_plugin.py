...
class SetterPlugin( Plugin ):
    ...
    def __init__( self, name: str, description: str, visibility: Optional[VisibilityClass] = None, highlight: Optional[VisibilityClass] = None ):
        """
    ...
    def extract_arguments( sample, formatter = str ):
        ...
        for field_name, field_value in sample.__dict__.items():
            ...
            if documentation is None:
                msg = "A SetterPlugin references the field " + Theme.HIGHLIGHT_IN_ERROR + "`{}::{}`" + Theme.RESET + " (under the name `{}`) on the sample object «{}», but does not provide any documentation for that field."
