...
def use( category: Optional[str] = None, all: bool = False ) -> None:
    ...
    for name in sorted( names, key = lambda x: x.name ):  # type: VisibilityClass
        shown = name()
        MCMD.print( "{}{}{}{} {}".format( (Theme.STATUS_IS_NOT_SET if name.user_override is None else Theme.STATUS_IS_SET),
                                          "[X]" if shown else "[ ]",
