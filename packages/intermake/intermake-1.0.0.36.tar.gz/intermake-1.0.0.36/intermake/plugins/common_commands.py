...
from intermake.engine import cli_helper, function_inspector, m_print_helper
...
@command( names = ["show", "hide"], visibility = visibilities.ADVANCED )
...
def topics_help() -> str:
    ...
    r.append( Theme.TITLE + "-------------------- HELP TOPICS --------------------" + Theme.RESET )
...
def cmd_help( command_name: Optional[str] = None, argument_name: Optional[str] = None ) -> None:
    ...
    if not argument_name:
        ...
        m_print_helper.print_box( MCMD.print, 4, 30, console_width, Theme.FIELD_NAME + "name       " + Theme.RESET, Theme.ARGUMENT_NAME + argument.name + Theme.RESET )
        m_print_helper.print_box( MCMD.print, 4, 30, console_width, Theme.FIELD_NAME + "type name  " + Theme.RESET, t.__name__ )
        m_print_helper.print_box( MCMD.print, 4, 30, console_width, Theme.FIELD_NAME + "optional   " + Theme.RESET, str( argument.can_be_none ) )
        m_print_helper.print_box( MCMD.print, 4, 30, console_width, Theme.FIELD_NAME + "default    " + Theme.RESET, Theme.COMMAND_NAME + str( argument.default ) + Theme.RESET )
        m_print_helper.print_box( MCMD.print, 4, 30, console_width, Theme.FIELD_NAME + "description" + Theme.RESET, cli_helper.highlight_keywords( argument.description, command ) )
        
        # Type docs
        docs = function_inspector.extract_documentation( t.__doc__, "data" )
        m_print_helper.print_box( MCMD.print, 4, 30, console_width, Theme.FIELD_NAME + "type       " + Theme.RESET, docs[""] or "Values:" )
        
        for key, value in docs.items():
            if key and value:
                m_print_helper.print_box( MCMD.print, 34, 50, console_width, Theme.ENUMERATION + key + Theme.RESET, value )
...
def cli() -> None:
    """
    Legacy command equivalent to `ui cli`
    """
    ui( ERunMode.CLI )
