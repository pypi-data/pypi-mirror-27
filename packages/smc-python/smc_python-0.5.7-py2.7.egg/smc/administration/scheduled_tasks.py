...
after defined, or scheduled to run on a regular basis. Scheduled tasks in the
SMC are defined under Administration->Tasks->Definition. 

Some tasks are read-only, meaning they are system elements and cannot be modified
or copied and can therefore only be scheduled (these classes will not have a `create`
method). Other tasks can be created and custom settings can be defined. Check the
documentation for each task to determine the capabilities.
...
An example follows that shows how to use a refresh policy task. Other tasks all
...
Create a task for tasks that are not read-only::
...
A task can always be run at any time without having to set a schedule
for continuous task by calling `start` on the task::
...
This can be done when creating the task, or the task can be retrieved after::
    ...
    task.add_schedule(
        ...
        activation_date='2017-10-04T09:33:09.890000-05:00', 
        comment='example scheduled task')
        
Or obtain an existing task and add a schedule to run one time on
...
monthly::
    ...
    task = RefreshPolicyTask('refresh_time')
    task.add_schedule(
        name='run_monthly', 
        activation_date='2017-10-04T23:58:52-06:00',
        day_period='monthly')
...
date::
    ...
    task.add_schedule(
        ...
        activation_date='2017-10-04T06:00:00-06:00',
        day_period='monthly',
        repeat_until_date='2018-04-04T06:00:00-06:00')
...
class TaskSchedule(SubElement):
    ...
    def suspend(self):
        ...
        :raises ActionCommandFailed: failed to suspend
        :return: None
        """
        self.upd_cmd(
            ActionCommandFailed,
...
class ScheduledTaskMixin(object):
    ...
    def add_schedule(self, name, activation_date, day_period='one_time',
                     ...
                     minute_period='one_time', repeat_until_date=None,
                     comment=None):
        """
        Add a schedule to an existing task.
        
        :param str name: name for this schedule
        :param str activation_date: when to start this task. Activation date
            should be an iso8601 formatted string including the offset, i.e:
            '2017-10-04T09:33:09.890000-05:00' - i.e. 9:33:09 in CST timezone.
