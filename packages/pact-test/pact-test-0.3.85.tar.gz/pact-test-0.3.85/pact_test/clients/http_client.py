...
def execute_interaction_request(url, port, interaction):
    ...
    server_response = _server_response(method, url=url, body=body)
...
def _server_response(method, url, body):
    try:
        payload = json.dumps(body)
        return Right(requests.request(method, url=url, data=payload))
