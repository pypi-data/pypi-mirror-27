...
import numpy as np
from EPIC.observables import CMBdistances, PlanckDistancePriors, H, a, lognormal_parameters_function
from EPIC.utils import bckg_cosmo, nv_clusters
from EPIC.utils.numbers import G, speedoflight, ln10, GmumH #, mumH
from EPIC.utils.io_tools import readfile, pasta, if_not_given
...
class Covs(object):
    ...
    def COV(self, A):
        C = A * self.Ceta * A.T 
        sig2z = np.matrix(np.diag(5 * 150e3/speedoflight / self.sz / ln10)**2)
...
class Cluster(object):
    ...
    #        self.rhoc = bckg_cosmo.rho_critical(Hz)
    ...
    #    xi = if_not_given(params, fixed, 'xi', 0)
    #    dlnrhow = (self.c * self.gc - 3)/self.r200
    #    sigX = (self.kTX/mumH)**(1/2)
    #    vth = (self.M200 / self.fc / self.r200)**(1/2) * (2/3 *(1+3*xi)/(2+3*xi) * G * 1e14)**(1/2)
    #    rdot = sigX - vth
    #    res = dlnrhow * rdot
    ...
    def dlnrhoH2(self, cosmology):
        ...
        self.rhoc = bckg_cosmo.rho_critical(self.Hz)
        #Gprime = 300 * self.rhoc / mumH  # this is Msun Mpc^-3 / keV (km/2)^-2
        ...
        Gprime_over_F *= 1 / self.rhoc * (- 9)/(8*np.pi) /200 / GmumH # 1/keV
        dlnF_times_r200 = 2 - self.dFdc/self.fc * self.c # this product is adimensional
        res = (3 * Gprime_over_F * self.kTX + dlnF_times_r200)

        g = if_not_given(cosmology.params, cosmology.fixed, 'gamma', 0.5)
        res *= (g**2 * 3**(g+1))**(1/(1-g))
        res /= self.Hz
        #np.savetxt("res_%s.txt" % self.name, res)
        #return statistics.MixedDist(*statistics.fit_asymmetrical(res))
        return res
...
def clusters(obsble):
    ...
    if obsble.endswith('.txt'):
        cluster_collection = readfile(obsble)
...
def SNeJLA(obsble):
    ...
    C_eta = sum([fits.getdata(mat) for mat in glob.glob(os.path.join(obsble, 'covmat', 'C*.fits'))])
...
def simplest_data(obsble, name, tex):
    print('    Reading %s data...' % name)
    print('        %s' % obsble)
    dataset = [Measurement(data) for data in np.loadtxt(obsble)]
    return tex, len(dataset), dataset

def BAO(obsble):
    return simplest_data(
            obsble, 'BAO (6dF+SDSS+BOSS+Lyalpha+WiggleZ)', 'BAO'
            )

def fs8(obsble):
    return simplest_data(obsble, 'fs8', r'$f\sigma_8$')

def CosmicChrono(obsble):
    return simplest_data(obsble, 'cosmic chronometer H(z)', 'CC+$H_0$')
