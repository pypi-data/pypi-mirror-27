...
from EPIC.utils.numbers import CL68, CL95
import scipy.stats as st
import scipy.interpolate as interpolate
from EPIC.utils.math_functions import normalize
...
def draw_on_axes(listdirlen, parname, customunits, ax, smooth_hist, hist_properties, title_above=True, MHIST=None, CRANGE=False, UseTex=False, title_pos='upper right', fontsize=8, args=None):
    ...
    if args:
        title_pos = io_tools.lookarg1list('title_pos=', args)
        if title_pos:
            dict_title = {}
        ...
        bfmark = io_tools.lookarg1('bf=', args)

        fontsize = io_tools.lookarg1('fontsize=', args, instance=int) or fontsize
    ...
    if listdirlen == 1:
        ...
        for sigma_up, alpha in zip(hist_properties.sigma_up, alphas):
            sigma_lim = normalize(sigma_up/hist_properties.norm, (0, MHIST), newlims=nrange)
...
def make_histograms(nparams, parnames, tex, listwdir, bar, priors=None, derived=False, listofcolors=['C%i' % i for i in range(10) if not i==3]+['C3'], singlecolor=['C0'], nocustomunits=False, FMT=5, LEVELS=[1, 2], smooth=None, CFONT=None, args=None):
    ...
    if args:
        ...
        # other of your like.
        singlecolor = io_tools.lookarg1list('singlecolor=', args) or singlecolor

        # a custom list of colors for multiple plots can be passed as a list of strings
        listofcolors = io_tools.lookarg1list('colors=', args) or listofcolors

        smooth = io_tools.lookarg1('smooth_hist=', args) or smooth

        # One can disable customunits although it is unlikely this will be used
        nocustomunits = io_tools.lookarg('nocustomunits', args) or nocustomunits

        FMT = io_tools.lookarg1('fmt=', args, instance=int) or FMT
        LEVELS = io_tools.lookarg1list('levels=', args, instance=int) or LEVELS

        CFONT = io_tools.lookarg1('font=', args) or CFONT

    inifile = io_tools.get_inifile_name(os.path.join(listwdir[0], '..', '..', derived and '..' or ''))
    ...
    for wdir in listwdir:
        ...
        results_hist_table = io_tools.create_tex_table(os.path.join(wdir, 'hist_table.tex'), cfont=CFONT)
        io_tools.table_headings(results_hist_table, levels=LEVELS)
        if smooth and smooth.lower() in ['kde', 'kde1', 'kdh']:
            results_kde_table = io_tools.create_tex_table(os.path.join(wdir, '..', 'kde_estimates', 'kde_table.tex'), cfont=CFONT)
        ...
        for i, par in enumerate(parnames):
            try:
                ...
                sigmasCL, sigma_up = data_tools.get_1D_confidence_regions(count, seps, parfit=par_fits[i], fmt=FMT, unit=UNIT, levels=LEVELS, normal=par in normal_kde)
                prior = not derived and priors[par] or ''
                io_tools.append_distribution_parameters_to_tex(results_hist_table, tex[par], bestfit[i], sigmasCL, prior=prior, unit=UNIT, fmt=FMT)
                ...
                if smooth and smooth.lower() in ['kde', 'kde1', 'kdh']:
                    ...
                    kde_sigmasCL, sigma_up = data_tools.get_1D_confidence_regions(
                            kde_density, kde_support, parfit=par_fits[i],
                            fmt=FMT, unit=UNIT, levels=LEVELS, normal=par in
                            normal_kde)
                    sigma_up *= normalization
                    io_tools.append_distribution_parameters_to_tex(results_kde_table, tex[par], bestfit[i], kde_sigmasCL, prior=prior, unit=UNIT, fmt=FMT)
...
def der_drawhistograms(parnames, fig, ax, tex, listwdir, list_hist_properties, bar, nocustomticks=False, nocustomrange=False, nocustomunits=False, smooth=False, show_gaussian_fit=False, show_hist=False, UseTex=False, gname=False, png=False, fontsize=8, args=None):
    ...
    if args:
        ...
        nocustomrange = io_tools.lookarg('nocustomrange', args) or nocustomrange
        ...
        nocustomticks = io_tools.lookarg('nocustomticks', args) or nocustomticks
        ...
        show_gaussian_fit = io_tools.lookarg('show_gaussian_fit', args) or show_gaussian_fit
        show_hist = io_tools.lookarg('show_hist', args) or show_hist
        ...
        gname = io_tools.lookarg1('gname=', args) or gname
    ...
    inifile = io_tools.get_inifile_name(os.path.join(listwdir[0], '..', '..', '..'))
...
def make_2dhistograms(grid, subplots, nuisance, modelname,
    ...
    if args:
        ...
        bfmark = io_tools.lookarg1('bf=', args) or bfmark
        ...
        CMAP = io_tools.lookarg1list('cmap=', args) or CMAP
        contours = io_tools.lookarg1('contours=', args) or contours
    ...
    if show_gaussian_fit:
        from EPIC.utils.math_functions import draw_ellipse
                ...
                try:
                    ...
                    if CMAP:
                        ...
                        if smooth and smooth.lower() in ['kde', 'kde2', 'kdh']:
                            ...
                            kde_sigmalevels, kde_H = data_tools.sigmalevels_2D(kde_H, kde_xsupport, kde_ysupport, levels=LEVELS)
                    ...
                    if show_gaussian_fit:
                        ...
                        thx1s, thy1s = draw_ellipse(IF, (mu[wdir][jx], mu[wdir][jy]), CLsigma=CL68)
                        subplots[jx][jy].plot(thy1s, thx1s, ls='-', color='k', lw=0.3)
                        thx2s, thy2s = draw_ellipse(IF, (mu[wdir][jx], mu[wdir][jy]), CLsigma=CL95)
            ...
            for wdir, hist_properties in zip(notallwdir, list_hist_properties):
                ...
                if show_gaussian_fit:
                    ...
                    if true_fit:
                        ...
                        subplots[jx][jy].plot(X, normalize(
                            1/mhist * D, 
                            (0, 1),
    ...
    if len(listwdir) > 1 and not isinstance(grid, list):
        ...
        subplots[-1][-1].legend(title=modelname, handles=legs, frameon=False,
                borderaxespad=0.5, handletextpad=0.8, bbox_to_anchor=(1,1),
                bbox_transform=grid.transFigure)
...
def determine_customticks(args, inifile, ticks=True):
    ...
    if ticks:
        ...
        ticks = ticks or io_tools.lookarg1('ticks=', args)
        if ticks:
            for (par, xyticks) in load_ticks(ticks):
                cticks[par] = xyticks     
    else:
        ticks = io_tools.lookarg1list('ticks=', args)
        if ticks:
            for tck in ticks:
...
def determine_customunits(args, inifile, nocustomunits=False):
    ...
    if unitsfile and not nocustomunits:
        ...
        customunits = io_tools.lookarg1list('units=', args)
        if customunits:
            dictunits = {}
...
def determine_customrange(args, inifile, nocustomrange=False):
    ...
    if rangefile and not nocustomrange:
        priorsrange, _, _ = io_tools.lookprior(io_tools.readfile(rangefile), needs_mq=False)
        ...
        customrange = io_tools.lookarg1list('range=', args)
        if customrange:
           dictrange = collections.OrderedDict()
...
def build_histograms(listwdir, include_nuisance=True, UseTex=False, use_derived=False, cfont='cmr10', dont_include_pars=[], args=None):
    ...
    if args:
        ...
        include_nuisance = not io_tools.lookarg('nonuisance', args)
        dont_include_pars = io_tools.lookarg1list('dont_include_pars', args) or dont_include_pars
    ...
    inifile = io_tools.get_inifile_name(os.path.join(listwdir[0], '..', '..'))
    modelname = io_tools.read1linestring(io_tools.readfile(os.path.join(listwdir[0], '..', '..', inifile)), 'NAME')
    priors, nuisance, _, tex, parnames, nparams, nchains = io_tools.load_chains_and_model_details(os.path.join(listwdir[0], '..', '..'))
    if include_nuisance:
        nuisance = []
    ...
    plotting_histograms_all = io_tools.fixed_length('Plotting distributions of all parameters...')
...
if __name__ == '__main__':
    ...
    ftime = time.time()
    print('Total time: %s' % io_tools.printtime(ftime - itime))
    print('')
