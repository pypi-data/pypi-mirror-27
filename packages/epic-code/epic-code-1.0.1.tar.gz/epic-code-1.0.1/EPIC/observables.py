...
import numpy as np
#import pdb
from EPIC.utils.statistics import LogNormalMode, LogNormalPDF, LogNormalCDF
from EPIC.utils.numbers import speedoflight, CL68
...
def growth_rate_sigma_eight(z, cosmology):
    ...
    OmegaDE0 = if_not_given(cosmology.params, cosmology.fixed, 'Od0', 0.686)
    #old best fit value:
...
def Eh(z, cosmology):
    ...
    if cosmology.model == 'lcdm':
        ...
        Omega_DE0 = if_not_given(cosmology.params, cosmology.fixed, 'Od0', 0.686)
        ...
        # for w constant:
        #if xi == 0:
        #return np.sqrt(h2 * ((1-Omega_DE0) * (1+z)**3 + Omega_DE0 * (1+z)**(3*(1 + w0))))
        #xi <> 0
        return np.sqrt(h2 * ((1-Omega_DE0) * np.exp(integrate.quad(lambda zbar: 3/(1+zbar)*(1 - xi * OmegaDE(zbar)/(1 - OmegaDE(zbar))), 0, z)[0]) + Omega_DE0 * (1+z)**(3*(1 + w0 + xi))))
