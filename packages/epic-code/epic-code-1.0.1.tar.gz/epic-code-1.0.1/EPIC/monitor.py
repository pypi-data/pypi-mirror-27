...
def plot_monitor_convergence(listwdirn, UseTex=False, png=False):
    ...
    from EPIC.utils.io_tools import read_model_name
    ...
    for i, wdirn in enumerate(listwdirn):
        ...
        modelname = read_model_name(os.path.join(wdirn, '..'))
...
if __name__ == '__main__':
    ...
    png = io_tools.lookarg('png', sys.argv)

    UseTex = io_tools.lookarg('usetex', sys.argv)

    cfont = io_tools.lookarg1('font=', sys.argv) or 'cmr10'
    ...
    npars = io_tools.load_chains_and_model_details(os.path.join(listworkingdir[0], '..'))[5]

    itime = time.time()

    from EPIC.utils.io_tools import fixed_length
    plotting_individual_parameters = fixed_length('Plotting variances and ratio for each parameter...')
    #pdb.set_trace()
    with click.progressbar(length=npars, show_pos=len(listworkingdir) == 1, empty_char='.', width=10, label=plotting_individual_parameters, info_sep='  |  ', show_eta=False) as bar:
        for wdirn in listworkingdir:
            _, _, _, tex, parnames, nparams, _ = io_tools.load_chains_and_model_details(os.path.join(wdirn, '..'))
