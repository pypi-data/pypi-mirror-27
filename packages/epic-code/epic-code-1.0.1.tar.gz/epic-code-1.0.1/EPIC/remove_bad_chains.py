...
def deletechain(wdir, N):
    ...
    io_tools.saveint(os.path.join(wdir, 'nchains.txt'), nchains-1)
...
def deletebadchains(wdir, accplims=[0.1,0.5], args=False):
    if args:
        ...
        accplims = io_tools.lookarg1list('accacc=', args, instance=float) or accplims
...
if __name__ == '__main__':
    wdir = sys.argv[1]
    N = io_tools.lookarg1list('N=', sys.argv, instance=int)
    if N:
        N.sort()
        for n in N[::-1]:
            deletechain(wdir, n)
    else:
        accplims = io_tools.lookarg1list('accacc=', sys.argv, instance=float)
        assert accplims
