...
def load_ini(wdir, inifile):
    ...
    lines = io_tools.readfile(os.path.join(wdir, inifile))

    model = io_tools.read1linestring(lines, 'MODEL')
    obsbles = io_tools.looktagstrings(lines, 'DATA')
    nBIC, labels, datapoints = load_data.load_data(obsbles)

    io_tools.saveint(os.path.join(wdir, 'nBIC.txt'), nBIC)
    ...
    priors, nuisance, mq, tex, parnames, nparams, nchains = io_tools.load_chains_and_model_details(wdir)
...
def MetropolisHastings(I, wdir, N, args, Q, S, list_last_state=None):
    ...
    chain_size = io_tools.readint(os.path.join(wdir, 'previous%i.txt' % (I+1)))
    X0 = lastline.strip('\n').split(',')
    ...
    if not isinstance(Xt, list):  # 1-D case
        Xt = [Xt,]
    lines_this_run = []
    accepted_moves = io_tools.readint(os.path.join(wdir, 'accepted_moves_%i.txt' % (I+1)))
    if lpXt == 0.:
    ...
    io_tools.saveint(os.path.join(wdir, 'accepted_moves_%i.txt' % (I+1)), accepted_moves)
    io_tools.saveint(os.path.join(wdir, 'previous%i.txt' % (I+1)), T)
...
def manage_swap(
    ...
    proposed_swaps = io_tools.readint(os.path.join(wdir, 'proposed_swaps.txt'))
    accepted_swaps = io_tools.readint(os.path.join(wdir, 'accepted_swaps.txt'))
    loops = 0
    ...
    io_tools.saveint(os.path.join(wdir, 'accepted_swaps.txt'), accepted_swaps)
    io_tools.saveint(os.path.join(wdir, 'proposed_swaps.txt'), proposed_swaps)
    if proposed_swaps > 0:
...
def advance_chain_MH(
    ...
    np.random.seed()
    lines_this_run = []
    acc = io_tools.readint(os.path.join(wdir, 'accepted_states_%i.txt' % (m+1)))

    try:
        T = io_tools.readint(os.path.join(wdir, 'total_steps_%i.txt' % (m+1)))
    except IOError:
        T = 0
    ...
    io_tools.saveint(os.path.join(wdir, 'total_steps_%i.txt' % (m+1)), T)
    chain_advanced[m].set()

    if m < nchains.value:
        #print('Chain %i finished' % (m+1))
        chain_completed.value = True
        io_tools.saveint(os.path.join(wdir, 'accepted_states_%i.txt' % (m+1)), acc)
...
def ParallelTempering(wdir, N, nchains, args, Q, S, ns, betas, adapt=0, free_random_walk=0):
    ...
    try: 
        ...
        proposed_states = io_tools.readint(os.path.join(wdir, 'proposed_states.txt'))
        proposed_states += n_iter_between_swaps.sum()
        io_tools.saveint(os.path.join(wdir, 'proposed_states.txt'), proposed_states)
...
def matrixplot(M, wdir):
    from EPIC.utils.math_functions import cov_to_corr
    fig, ax = plt.subplots()
    ax.imshow(cov_to_corr(M))
...
def MCMCmain(workingdir, Nsteps, step=20, lastfew_p=5, MCMClimit=np.inf, checkafter=7200, Fixed={}, plot_histograms=True, adaPT=0, args=None):
    ...
    if args:
        CustomFixed = io_tools.lookarg1list('Fixed=', args)
        if CustomFixed:
        ...
        adaPT = io_tools.lookarg1list('adapt=', args, instance=int) or adaPT
        ...
        if mode == 'MCMC':
            ...
            checkafter = io_tools.find_timestring_convert_to_seconds(args) or checkafter
            ...
            step = max(2, step)
            lastfew_p = io_tools.lookarg1('lastfew=', args, instance=int) or lastfew_p

            # MCMClimit, in case one wants to perform a fixed number of steps
            MCMClimit = io_tools.lookarg1('limit=', args, instance=int) or MCMClimit
            
            # tolerance for standard MCMC
            tol = io_tools.lookarg1('tol', args, instance=float, optask='Please specify the tolerance for convergence: ')

        elif mode == 'PT':
            MCMClimit = io_tools.lookarg1('limit=', args, instance=int, optask='Type the maximum number of steps: ')
    ...
    find_ini = io_tools.getfilelist(workingdir, 'ini')[0]
    ...
    try:
        counter = io_tools.readint(os.path.join(workingdir, 'counter.txt'))
    except IOError:
        counter = 0
    totaltime = 0
    try:
    ...
    if mode == 'MCMC':
        ...
        while (np.any(abs(hatRp[-hlast:]-1) > tol) and last_update_N < MCMClimit):# totaliterations < MCMClimit):
            ...
            LLC = []
            for i in range(nchains):
                accepted_moves = io_tools.readint(os.path.join(workingdir, 'accepted_moves_%i.txt' % (i+1)))
                total_moves = io_tools.readint(os.path.join(workingdir, 'previous%i.txt' % (i+1)))
                #if i == 0:
            ...
            #print('Iteration %i with %i steps, %i chains; in %s, on %s.' % (counter, real_Nsteps, nchains, io_tools.printtime(iterationtime), ctime()), end=' ')
            if real_Nsteps > 1 or (real_Nsteps + last_update_N) % Nsteps == 0:
                print('i %i, %i steps, %i ch; %s, %s.' % (counter, real_Nsteps + last_update_N, nchains, io_tools.printtime(iterationtime * (1 if real_Nsteps > 1 else Nsteps)), ctime()), end=' ')
            ...
            if nextcheck > 0.5 * iterationtime and last_update_N < MCMClimit:
                if real_Nsteps > 1 or last_update_N % Nsteps == 0:
                    print('Next: ~%s.' % io_tools.printtime(nextcheck))
                    #print('Next convergence check in ~%s.' % io_tools.printtime(nextcheck))
                    #print(S)
            else:
                if real_Nsteps > 1 or last_update_N % Nsteps == 0:
                    print('Checking now...')
                ...
                aftime = time()
                print('Total time for data analysis: %s.' % io_tools.printtime(aftime - aitime))

                totaliterations = n
                print('After %s and %i steps, with %i chains,' % (io_tools.printtime(cumulatedtime), totaliterations, nchains))
                timenow = ctime()
        ...
        ns = io_tools.readint(os.path.join(workingdir, 'ns.txt'))
        # parallel tempering
        try:
        ...
        while last_update_N < MCMClimit:
            ...
            print('i %i, %i of %i steps, %i ch; %s, ns %i, swap %s, accpt %s, %s.'
                    % (
                        ...
                        io_tools.printtime(iterationtime), ns, swap_ratio,
                        accept_ratio, ctime()
                        )
                    )
            if counter % 10 == 0:
        ...
        duration_end = time()
        io_tools.saveint(os.path.join(workingdir, 'counter.txt'), counter)

        print('Total elapsed time in Parallel tempering: %s' % io_tools.printtime(duration_end - duration_start))
        io_tools.saveint(os.path.join(workingdir, 'duration.txt'), duration_end - duration_start)
        ...
        plot_histograms = not io_tools.lookarg('dontdraw', args)
...
if __name__ == '__main__':
    ...
    Nsteps_user = io_tools.lookarg1('steps=', sys.argv, instance=int, optask='Please specify the number of steps in each passage in the MCMC loop: ')
    starting_point = io_tools.lookarg1list('starting_point=', sys.argv, instance=float)
    ...
    if os.path.isdir(sys.argv[1]):
        ...
        mode = io_tools.lookarg1('mode=', sys.argv) or 'MCMC'
