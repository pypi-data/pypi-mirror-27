...
from EPIC import MCMC_chains, drawhistograms
...
def read_chains(wdir, nchains, calculate_evidence=False, stop_at=False, ssd=False, showbar=True, args=None):
    if args:
        ...
        calculate_evidence = io_tools.lookarg('evidence', args) or calculate_evidence
...
def load_chains(wdir, nparams, nnuis, nchains, priors, parnames, tex, bchain=1, calculate_evidence=False, stop_at=False, burnin=None, png=False, args=None):
    ...
    if args:
        ...
        bchain = io_tools.lookarg1('bchain=', args, instance=int) or bchain
        ...
        plot_sequences = io_tools.lookarg('plot_sequences', args)
        ACF = io_tools.lookarg('ACF', args)
        calculate_evidence = io_tools.lookarg('evidence', args) or calculate_evidence or bchain != 1 or plot_sequences or ACF
    ...
    io_tools.pasta(wdir, 'n%ib%i' % (n, bchain))

    burnin = burnin or n//2
    maxlogposterior, bestfit = find_max_logposterior(nparams, f[bchain-1], n, burnin)
    AIC, BIC = Information_Criteria(n, parnames, bestfit, priors, nparams, nnuis, maxlogposterior, wdir, b=bchain)
    print('BIC: %.3f' % BIC)
    print('AIC: %.3f' % AIC)
...
def analyze_chains(wdir, combobins=[20], bchain=1, calculate_evidence=False, stop_at=False, wDE_list=None, use_derived = False, plot_sequences=False, ACF=False, THIN=1, usetex=False, burnin=None, png=False, SMOOTH=None, args=None):
    ...
    listbinsdict = io_tools.create_listbinsdict(os.path.join(wdir, 'n%ib%i' % (n, bchain)), combobins, binsfile, clear=False)
    ...
    if nderpars:
        io_tools.pasta(os.path.join(wdir, 'n%ib%i' % (n, bchain)), 'der_pars')
    ...
    io_tools.format_with_uncertainties(par_fits, os.path.join(wdir, 'n%ib%i' % (n, bchain)), 'format_par_fits.txt')
    if nderpars:
        der_par_fits = [st.distributions.norm.fit(chain) for chain in der_chains]
        io_tools.format_with_uncertainties(der_par_fits, os.path.join(wdir, 'n%ib%i' % (n, bchain)), 'format_der_par_fits.txt')
    ...
    with click.progressbar(length=numberhists, width=10, show_pos=False, empty_char='.', label=saving_histograms_label, info_sep='  |  ', show_eta=False) as bar:
        ...
        if nderpars:
            ...
            for binsdict in listbinsdict:
                io_tools.pasta(os.path.join(wdir, 'n%ib%i' % (n, bchain), 'der_pars'), "results%i" % binsdict['default'])
...
def initialize(command_arguments):
    ...
    bchain = io_tools.lookarg1('bchain=', command_arguments, instance=int) or 1
    ...
    calculate_R = io_tools.lookarg1list('calculate_R=', command_arguments)
    if calculate_R:
        F = [f[bchain-1],]
