...
def load_ini(inifile):
    lines = io_tools.readfile(inifile)
    priors, nuisance, mq = io_tools.lookprior(lines)
    tex = io_tools.looktagstrings(lines, 'TEX')
...
def register_chains_and_model_details(wdir, priors, nuisance, mq, tex, parnames, nparams, nchains):
    ...
    io_tools.saveint(os.path.join(wdir, 'nparams.txt'), nparams)
    io_tools.saveint(os.path.join(wdir, 'nchains.txt'), nchains)
...
def readargs(args, mode):
    prefix = mode
    cfullname = io_tools.lookarg1('cfullname', args)
    if cfullname:
        hora = '%s-%s' % (prefix, cfullname)
    else:
        hora = '%s-%s' % (prefix, io_tools.define_time())
    cname = io_tools.lookarg1('cname=', args)
    altfolder = io_tools.lookarg1('altdir=', args)
    if not altfolder:
        try:
    ...
    plotstates = not io_tools.lookarg('dontplotstates', args)

    inifile = io_tools.lookarg('.ini', args) or input('Type in the name of the \'.ini\' file: ')
    inifile = io_tools.assert_extension(inifile, 'ini')
    inidir = inifile[:-4]
    ...
    workingdir = io_tools.pasta(os.path.join(pardir,inidir), hora)
    shutil.copy2(inifile, workingdir)

    Nc = io_tools.lookarg1('chains=', args, optask='Please specify the number of chains: ', instance=int)
...
def write_PTchains_files(wdir, parnames, priors, Nc, betamax=10, betascale='log', args=None):
    if args:
        ...
        ns = io_tools.lookarg1('nswap=', args, instance=int, optask='Please specify after how many steps should propose a swap: ')

        # custom array of temperatures
        betamax = io_tools.lookarg1('betamax=', args, instance=int) or betamax
        betascale = io_tools.lookarg1('betascale=', args) or betascale

    io_tools.saveint(os.path.join(wdir, 'proposed_swaps.txt'), 0)
    io_tools.saveint(os.path.join(wdir, 'accepted_swaps.txt'), 0)
    io_tools.saveint(os.path.join(wdir, 'proposed_states.txt'), 0)
    for m in range(Nc):
        io_tools.saveint(os.path.join(wdir, 'accepted_states_%i.txt' % (m+1)), 0)
    io_tools.saveint(os.path.join(wdir, 'ns.txt'), ns)
...
def write_chains_files(workingdir, parnames, priors, Nc, starting_point=None):
    ...
    for i in range(Nc):
        io_tools.saveint(os.path.join(workingdir, 'accepted_moves_%i.txt' % (i+1)), 1)
