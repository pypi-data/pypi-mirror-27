...
from EPIC import observables
from EPIC.utils.numbers import log2pi #, GmumH
from EPIC.utils.io_tools import if_not_given
from EPIC.utils import statistics 
from EPIC.utils.bckg_cosmo import z_of_a
#import pdb
...
class Cosmology(object):
    def __init__(self, params, fixed, model, log10Ai=-5, size=30000):
        ...
        if 'darkSU2' in self.model:
            self.rk_densities = observables.integrate_densities(self,
                    log10Ai=log10Ai, size=size)
            if 'wnu0' in self.model:
                A, rho_Gamma, rho_phi0, rho_c = self.rk_densities 
                H = [observables.Hubble(a, np.array([rhoGamma, phi0, cdm]), self) \
                        for a, rhoGamma, phi0, cdm in zip(A, rho_Gamma,
                            rho_phi0, rho_c)]
            else:
                A, rho_Gamma, rho_phi0, rho_psi, rho_nu = self.rk_densities 
                H = [observables.Hubble(a, np.array([rhoGamma, phi0, psi, nu]), self) \
                        for a, rhoGamma, phi0, psi, nu in zip(A, rho_Gamma,
                        rho_phi0, rho_psi, rho_nu)]
            self.H_solution = interpolate.interp1d(z_of_a(A), np.array(H))

        elif self.model == 'LIclusters_wd':
            self.rk_DE = observables.integrate_rhod_wcons(self,
                    log10Ai=-0.5, size=size)
            A, rho_d = self.rk_DE
            H = [observables.Hubble_LIclusters(a, d, self) for a, d in zip(A, rho_d)]
            self.H_solution = interpolate.interp1d(z_of_a(A), np.array(H))
...
def simple(datapoints, function, cosmology):
    ...
    total_likelihood += - samplesize/2 * log2pi
...
def multipoints(datapoints, cosmology, function):
    ...
    if np.isnan(chi2):
        return -np.inf
    loglikelihood = - samplesize/2 * log2pi - sum([data.sigma for data in datapoints]) - chi2/2 
...
def matrixform(datapoints, observ, predict):
    ...
    try:
        ...
        return - datapoints.sample_size/2 * log2pi + 1/2 * datapoints.logdetICov - chi2/2
    except AttributeError:
        chi2 = np.dot( np.dot(X.transpose(), np.linalg.inv(datapoints.COV)), X )
        #return - chi2/2
        return - datapoints.sample_size/2 * log2pi - 1/2 * datapoints.logdetCov - chi2/2
...
def matrixform_nuisance(sample_size, observ, predict, COV, logdetCov):
    ...
    try:
        ...
        return -np.inf
    return - sample_size/2 * log2pi - 1/2 * logdetCov - chi2/2
...
def clusters_virial_ratio(datapoints, cosmology):
    ...
    ##    sup, kde = statistics.make_kde([num,])
    ...
    #    jx, jy = divmod(i, 5)
    #    print(cl.name)
    #    #sup, kde = statistics.make_kde([num,], thin=5)
    ...
    predict_tvr = observables.th_virial_ratio(cosmology) * np.ones(len(datapoints))
    ...
    return np.sum([np.log(float(statistics.make_kde([num,], x=predict, thin=1)[1])) for num, predict in zip(obs_tvr, predict_tvr)])
...
def sym_clusters_virial_ratio(datapoints, cosmology):
    ...
    return np.sum([np.log(1/tvr.std_dev * statistics.scipy_stats_standard_normal.pdf((predict-tvr.nominal_value)/tvr.std_dev)) for tvr, predict in zip(obs_tvr, predict_tvr)])

def JLA_binned_data(datapoints, cosmology):
    mupredict = observables.muthsimple(datapoints.z, cosmology).transpose()
    if np.any(np.isnan(mupredict)):
        return -np.inf
    muobs = datapoints.muB.transpose()
    Mpar = if_not_given(cosmology.params, cosmology.fixed, 'M', 0.0)
...
def JLA_full_SNe_data(datapoints, cosmology):
    mupredict = np.matrix(observables.muth(datapoints, cosmology)).T
    if np.any(np.isnan(mupredict)):
        return -np.inf
    alpha = if_not_given(cosmology.params, cosmology.fixed, 'alpha', 0.14)
    beta = if_not_given(cosmology.params, cosmology.fixed, 'beta', 3.1)
    A = np.matrix(datapoints.matrixA(alpha, beta)) 
    MBpar = if_not_given(cosmology.params, cosmology.fixed, 'MBpar', -19.02)
    deltaMB = if_not_given(cosmology.params, cosmology.fixed, 'deltaMB', -0.06)
...
def BAO_ratio(datapoints, cosmology):
    return simple(datapoints, observables.rBAO, cosmology)

def cosmicchrono(datapoints, cosmology):
    return simple(datapoints, observables.H, cosmology)

def f_sigma8(datapoints, cosmology):
    return simple(datapoints, observables.myfsigma8, cosmology)
...
def compute_logposterior(datapoints, params, fixed, priors, model, beta=1.):
    LogPrior = statistics.logpriordist(params, priors)
