...
from ..exc import DuplicateParameterWarning

from ..meta.container  import Container
from ..meta.definition import ParameterCollection, DataDefinition, Interception, MethodCall, DuplicateParameterDefinitionWarning
...
class XMLParser(ConfigParser):
    ...
    def parse(self, filepath : str):
        ...
        for container_node in root_node.children():
            ...
            except DuplicateParameterDefinitionWarning as e:
                raise DuplicateParameterWarning(
                    'The entity "{entity_id}" ({entity_class}) has duplicate definition for parameter "{parameter_name}".'.format(
                        entity_id      = container_node.attribute('id'),
                        entity_class   = container_node.attribute('class'),
