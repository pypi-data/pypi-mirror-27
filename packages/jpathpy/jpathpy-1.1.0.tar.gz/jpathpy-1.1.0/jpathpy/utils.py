...
class JMembers(object):
	'''
		List of JSON members which were selected by JPath query
	'''

	def __init__(self):
		'''
			Initializes the empty list of JSON members
		'''
		self._list = []
	...
	def __len__(self):
		'''
			Returns count of the JSON members in list
		'''
	...
	def __next__(self):
		'''
			Returns a next member from JSON members list
		'''
		self.__index += 1
	...
	def __add(self, *args):
		'''
			Adds a new member to the end of JSON members list
		'''
		if len(args) > 0:
			members = args[0] if type(args[0]) == type(list()) else [args[0]]
			for m in members:
				if not isinstance(m, JMember):
					raise TypeError('Can\'t add element to list of members. Expected JMember (not ' + stype(m) + ')')
				self._list.append(m)
				
	@staticmethod
	def create(*args):
		'''
			Creates a new list of JSON members.
			If argument isn\'t specified, creates the empty list.
			If argument is specified as an instance of JMember, it be added to the end of JSON members list.
			If argument is specified as list of an instances of JMember, all the instances be added to the end of JSON members list in that order in that they follow
		'''
	...
	def count(self):
		'''
			Count of JSON members in list. Readonly
		'''
		return len(self._list)
...
def disclosure_list(members):
	'''
		Returns the new instance of JMembers that will be created from list of JSON members specified as @members.
...
def filtering(filter, members):
	'''
		Returns the new instance of JMembers that will be created from list of JSON members specified as @members, but first it will be filtered.
...
def apply_func(func_name, params, members):
	...
	def call(func_name, params, value):
		...
		def length(value, *args):
			...
			return len(value) if type(value) == type(list()) or type(value) == type(dict()) else None
