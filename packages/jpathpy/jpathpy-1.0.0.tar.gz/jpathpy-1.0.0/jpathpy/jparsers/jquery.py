...
def jquery(jpath, jobj):
	...
	def p_jpath_root(p):
		...
		try:
			p[0] = utils.JMembers.create(utils.JMember(None, JDoc))
		except:
	...
	def p_jpath_filterlist(p):
		...
		for member in members:
			if type(member.value) == type(list()):
				...
				if len(filtered_members_from_list):
	...
	def p_jpath_filtered(p):
		...
		if (p[3] == '*'):
			...
			try:
				...
				for member in members:
					condition = jfexpr.jfexpression(p[3], member.value, JDoc)
					if condition:
