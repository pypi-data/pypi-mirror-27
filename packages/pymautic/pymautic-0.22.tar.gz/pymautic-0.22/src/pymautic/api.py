    ...
    def call_api(self, attrname):
        ...
        def custom_getattr(*args, **kwargs):
            ...
            if attrname.startswith("get_"):
                ...
                data = r.json()
                return data
            elif attrname.startswith("create_"):
                pieces = attrname.split("_")
                resource = pieces[1]

                url = self.url + '/api/{}/new'.format(resource)

                r = self.session.post(url, headers=self.headers, json=args[0])
                data = r.json()
                return data

            elif attrname.startswith("edit_"):
                pieces = attrname.split("_")
                resource = pieces[1]

                url = self.url + '/api/{}/edit'.format(resource)

                r = self.session.put(url, headers=self.headers, data=args[0])
                data = r.json()
                return data
