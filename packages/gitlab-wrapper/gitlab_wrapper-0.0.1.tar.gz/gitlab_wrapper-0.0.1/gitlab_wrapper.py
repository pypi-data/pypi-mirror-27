...
if __name__ == '__main__':
    wrapper = GitlabWrapper(url='http://192.168.56.103:27080', token='5sFk_EzB2cdwoCtwnL9T')
    # student = wrapper.get_or_create_user_login(email='itogo@mail.ru', name='Вася Студент')
    student = wrapper.get_or_create_user(email='studentkovamasha@ya.ru', name='Маша Студенткова')
    student_project = wrapper.get_or_create_user_project(user=student, repository_name='probe_repo')
    teacher = wrapper.get_or_create_user(email='uchetelevgennady@yandex.ru', name='Геннадий Учителев')
    wrapper.make_user_project_master(user=teacher, project=student_project)
    teacher_project = wrapper.get_or_create_user_project(user=teacher, repository_name='python_course')
    wrapper.copy_files_to_repository(
        base_project=teacher_project,
        files_path='module_01',
        target_project=student_project,
        autor=teacher.username,
    )
