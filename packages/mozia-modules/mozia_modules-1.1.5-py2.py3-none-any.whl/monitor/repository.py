...
class MonitorRepository(Repository):
    ...
    def get_platform_product(self, product_id):
        ...
        sql = """
           SELECT * FROM fashion_product.t_product_sku WHERE product_id=%s
           """
        with self.connection.cursor() as cursor:
            cursor.execute(sql, product_id)
    ...
    def set_platform_product_sizes_status(self, product_id, size, sku_status):
        sql = """
        UPDATE fashion_product.t_product_sku SET sku_status=%s,modify_time=unix_timestamp(),editor_name='crawler-for-im' WHERE product_id=%s AND size=%s
        """
    ...
    def set_platform_product_size_quantity(self, product_id, size, quantity):
        sql = """
        UPDATE fashion_product.t_product_sku SET quantity=%s,modify_time=unix_timestamp(),editor_name='crawler-for-im' WHERE product_id=%s AND size=%s
        """
