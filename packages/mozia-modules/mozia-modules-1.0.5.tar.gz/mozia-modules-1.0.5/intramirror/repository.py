...
def create_connection():
    return pymysql.connect(host='172.16.8.147', port=3306, user='dba', passwd='123456', db='intramirror',
...
class MonitorRepository(Repository):
    ...
    def set_platform_product_status(self, product_id, product_status):
        ...
        sql = """
        UPDATE t_product SET product_status=%s WHERE product_id=%s
        """
        with self.connection.cursor() as cursor:
    ...
    def set_platform_product_sizes_status(self, product_id, size, sku_status):
        sql = """
        UPDATE t_product_sku SET sku_status=%s WHERE product_id=%s AND size=%s
        """
    ...
    def set_platform_product_size_quantity(self, product_id, size, quantity):
        sql = """
        UPDATE t_product_sku SET quantity=%s WHERE product_id=%s AND size=%s
        """
