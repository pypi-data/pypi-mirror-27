...
from rest_framework import renderers, encoders
...
__AGGREGATE_KEYWORD = 'aggregate'
__GROUPBY_KEYWORD = 'group_by'
...
class AggregateRenderer(renderers.BaseRenderer):
    ...
    def render(self, data, media_type=None, renderer_context=None):
        ...
        if 'queryset' in data:
            ...
            try:
                self.build_result(data['queryset'], query_args)
    ...
    def process_query_params(self, request):
        ...
        query_args = {
            ...
            for key in (__AGGREGATE_KEYWORD, __GROUPBY_KEYWORD)
        }
        return query_args
    ...
    def build_result(self, qs, query_args):
        ...
        # process group by
        if query_args.get(__GROUPBY_KEYWORD):
            group_keys = []

            # special rules on how to handle fields
            for field in query_args[__GROUPBY_KEYWORD].keys():
        ...
        # process aggregates
        if query_args.get(__AGGREGATE_KEYWORD):
            aggs = {}
            custom_functions = {}

            for fn_name, fn_args in query_args[__AGGREGATE_KEYWORD].items():
            ...
            if aggs:
                qs = qs.annotate(**aggs) if query_args.get(__GROUPBY_KEYWORD) else qs.aggregate(**aggs)
