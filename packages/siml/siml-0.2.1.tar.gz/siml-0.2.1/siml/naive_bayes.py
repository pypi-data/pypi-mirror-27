    ...
    def classify_single_elem(self, X_elem):
        ...
        for label in self.labels:
            ...
            for ii in range(0,len(X_elem)):
              ...
              if X_elem[ii] in list(relative_feature_values.keys()):
...
class NaiveBayesText(NaiveBaseClass):
    ...
    def classify_single_elem(self, X_elem):
        ...
        for label in self.labels:
            ...
            for word in X_elem:
                if word in list(nb_dict_features.keys()):
...
####
X_train, Y_train, X_test, Y_test = ld.iphone()
X_train2, Y_train2, X_test2, Y_test2 = ld.iphone2()
...
# for ii in range(0,len(X_train)):
    # l_train.append((X_train[ii], Y_train[ii]))
# for ii in range(0,len(X_test)):
    # l_test.append((X_test[ii], Y_test[ii]))
# for ii in range(0,len(X_train)):
    # l_train.append((X_train2[ii], Y_train2[ii]))

    
# training_set_formatted = [(list_to_dict(element[0]), element[1]) for element in l_train]
# test_set_formatted = [(list_to_dict(element[0]), element[1]) for element in l_test]
# test_set_formatted2 = [(list_to_dict(element[0]), element[1]) for element in l_test2]
    
# print("training nltk")
# classifier = nltk.NaiveBayesClassifier.train(training_set_formatted)
# print("trained")
# print(nltk.classify.accuracy(classifier, test_set_formatted2))
# print("classified")
# print(nltk.classify.accuracy(classifier, test_set_formatted))
...
print("trained")
predicted_Y = nbc.classify(X_train2)
y_labels = set(Y_train2)
scores = []
for y_label in y_labels:
    f1 = f1_score(predicted_Y, Y_train2, y_label)
    scores.append(f1)
    msg = "F1-score for class '%s' is: %s" % (y_label, f1)
    print(msg.encode('utf-8'))
print("The average score is : %s" % str(np.mean(scores)))
...
# X_train2 = np.array([xi+[None]*(length-len(xi)) for xi in X_train])
# Y_train2 = np.array(Y_train)
# length = len(sorted(X_test,key=len, reverse=True)[0])
# X_test2 = np.array([xi+[None]*(length-len(xi)) for xi in X_test])
...
# print(clf.predict(X_test2[2:10]))
