...
def split_text(text):
    ...
    #st = re.split('(\d+|\s)',text)
    #cleaned_text =[]
    #for elem in st:
    #    if elem.strip(): cleaned_text.append(elem)
...
def iphone():
    datafile = '../datasets/Iphone/FinalCls.csv'
    Y_train, Y_test, X_train, X_test,  = [], [], [], []
    with open(datafile, 'r', encoding="utf8") as f:
        ...
        for line in lines[:no_training_examples]:
            ...
            label = line[1]
            Y_train.append(label)
            X_train.append(split_text(line[0]))
        for line in lines[no_training_examples:]:
            line = line.rstrip('\n').split('{')
            label = line[1]
            Y_test.append(label)
            X_test.append(split_text(line[0]))
    return X_train, Y_train, X_test, Y_test
    
def iphone2():
    datafile = '../datasets/Iphone/classifications1.csv'
    Y_train, Y_test, X_train, X_test,  = [], [], [], []
    with open(datafile, 'r', encoding="utf8") as f:
        lines = f.readlines()
        random.shuffle(lines)
        no_lines = len(lines)
        no_training_examples = int(0.95*no_lines)
        no_test_examples = no_lines - no_training_examples
        for line in lines[:no_training_examples]:
            line = line.rstrip('\n').split('{')
            label = line[1]
