...
if EXP:
    ...
    MIN_TOTAL_INT = 10 # minimun number of HiC pairs per contig to consider
    #MAX_INT_PER_LENGTH = 'auto' # maximun number of HiC pairs per length of contig
    MAX_INT_PER_LENGTH = 1 # maximun number of HiC pairs per length of contig
    MIN_COVERAGE = 0.8
    ITER = 50
...
#        self.overlap_graph = self.get_overlap_graph(overlap_graph_file)
    ...
    def assembleContigs_algorithm2(self):
        ...
        logging.debug("noise level set to {}".format(noise_level))
...
#        self.group = HiCAssembler.get_groups(self.hic.cut_intervals)
        ...
        self.PE = Scaffolds(self.hic.cut_intervals)
        if self.PE_scaffolds_file:
            self.add_PE_scaffolds(self.PE_scaffolds_file, self.PE)
...
#        self.group = HiCAssembler.get_groups(self.hic.cut_intervals)
        ...
        elapsed_time = time.time() - start_time
        logging.debug("time iterative_correction: {:.5f}".format(elapsed_time))
...
#                                                threshold=confident_threshold)
            ...
            logging.info("in total there are {} orphans".format(len(orphans)))
            small = [x for x in self.scaffolds.get_paths_length() if x <= merge_length]
            if len(small):
                logging.info("in total there are {} scaffolds smaller than {}. "
...
#                break
            ...
            if merge_length == prev_merge_length:
                merge_length *= 2
            if merge_length > 5e6:
                logging.info("Large merging value reached {}".format(merge_length))
                return
    ...
    def maskUnreliableRows(self, hic, astat_file, min_length=MIN_LENGTH,
        ...
        """
        logging.debug("filtering unreliable contigs")
        astat_dict = dict()
        ...
        repetitive = np.flatnonzero(astat < MAX_ASTAT)
        ...
        mask_s = set(mask)
        logging.debug("filtering paths with unreliable members")
        conn = [x for x in self.PE.get_all_paths() if len(x) > 1]
        for nodes in conn:
            ...
            if float(sum(length[overlap]))/(sum(length[nodes])) >= 0.75:
                ...
                mask_s.update(nodes)
                try:
                    logging.info("removing PE scaffold {} of size {} "
                                 "containing {} parts".format(
                            [self.PE.id2contig_name[x] for x in nodes],
                            length[nodes],
                            len(nodes)))
                except IndexError:
        ...
        mask = list(mask_s)
        rows_to_keep = cols_to_keep = np.delete(range(self.hic.matrix.shape[1]),
                                                mask)
        logging.info("Total: {}, small: {}, few inter: {}, low cover: {}, " \
        ...
        logging.info("removing {} ({}%) low quality regions from hic matrix " \
    ...
    def compute_distances(self, merge_length=1):
        ...
        contigs = HiCAssembler.flattenList(conn)
        ...
        logging.info("Mean contig length: {} sd{}".format(mean_contig_length,
                                                          sd_contig_length))
    ...
    def reduce_to_flanks_and_center(self, flank_length=20000):
        ...
        """
        logging.info("computing paths merge length {}".format(flank_length))
...
#            print "in {} out {} ".format(path, split_path)
        ...
        if len(reduce_paths) < 2:
            logging.info("Reduce paths to small {}. Returning".format(len(reduce_paths)))
            return None, None
        try:
            start_time = time.time()
            ...
            logging.info("Reduce matrix is empty. Returning".format(len(reduce_paths)))
            return None, None
...
#            print "edge exception {}".format(e)
        ...
        if debug and abs(u - v) != 1:
            ...
            logging.debug("[{}] an error joining {} and {} has been made\n {} \n{}"
...
#               fixed_paths.append((u,v))
    ...
    def assemble_super_contigs(self, G, paths, max_int, add_first_neighbors=True):
        ...
        """
        logging.info("Assembling contigs")
        ...
        while len(high_deg) > 0:
            ...
            if G.degree(node) > 3:
                ...
                if len(to_keep)>2:
                    ...
                    logging.debug("Found direct overlap for a neighbor of {}. "
                                  "Keeping neighbor {}".format(node, log_m))

                logging.debug("High degree node {} {} removed "
...
#                            import pdb;pdb.set_trace()
    ...
    #                                import pdb;pdb.set_trace()
            ...
            if self.matrix[edge[0],edge[1]] < 10:
                logging.debug('dropping interaction between {} {} ({}, {}) '
...
#            import pdb;pdb.set_trace()
    ...
    def add_PE_scaffolds(self, scaffolds_file, scaffolds, format='sga'):
        ...
        def get_contig_id(contig_name,  head, scaffolds):
            if contig_name in scaffolds.splitted_contigs:
                # need to decide if the link should
                # be made with the first or with the
                # last member of the group
                first_group = scaffolds.splitted_contigs[contig_name][0]
                last_group  = scaffolds.splitted_contigs[contig_name][-1]
        ...
        for line in scaffolds_file:
            ...
            for contig_info in path_raw:
                ...
                if debug:
                    ...
                    if abs(u-v) > 1:
                        logging.debug("WARNING: PE scaffolding wrong u,v "
                                      "{},{} in group {} {}, "
                                      "source {}, target {} \n{}".format(
                                u,v, 
...
class Scaffolds:
    ...
    def get_contig_graph(self, cut_intervals):
        ...
        prev_label = None
        i = 1
        self.splitted_contigs = defaultdict(list)
        for index in range(len(cut_intervals)):
            ...
            # Contigs with the same ID are splited
            # contigs.
            if prev_label == label[index]:
                ...
                if i == 1:
                    ...
                    self.splitted_contigs[label[index]].append(contig_id)
                i += 1
            else:
                i = 1
