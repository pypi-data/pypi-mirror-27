...
def trace (function, *args, **kwargs):
    ...
    logger.debug("fcall.start: %s(%s)", get_function_name(function), ", ".join(params_as_strings))
    try:
        before = os.times()[-1]
        retval = function(*args, **kwargs)
        after = os.times()[-1]
    except Exception as ex:
        extrathick.log_exception(logger, logging.DEBUG, "fcall", ex)
        raise
    else:
        logger.debug("fcall.elapsed: %f", after - before)
        # TODO maybe if retval is a generator or stream, wrap it in a tracer?
        logger.debug("fcall.retval: %a", retval)
        return retval
    finally:
        logger.debug("fcall.finish")
