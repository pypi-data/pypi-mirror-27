...
# @Last Modified time: 2017-10-30 16:34:29
...
class MongoService(DBService):
    ...
    def find_all(self, collection, query_sql, extract_fields={}):
        """alike the function find_one, actually call find_limit"""
        return self.find_limit(collection, query_sql, 0, extract_fields)

    def find_limit(self, collection, query_sql, limit, extract_fields={}):
        """alike find_one, actually call function find_limit_and_skip"""
        return self.find_limit_and_skip(collection, query_sql, limit, 0, extract_fields=extract_fields)

    def find_limit_and_skip(self, collection, query_sql, limit, skip, fetch_rate=5000, extract_fields={}, sort_fields={}):
        """alike find_limit, actually call function find"""
        return self.find(collection, query_sql, limit, skip, fetch_rate, extract_fields, sort_fields)

    def find(self, collection, query_sql, limit_count, skip_count=0, fetch_rate=5000, extract_fields={}, sort_fields={}):
    ...
    def replace_one(self, collection, query_sql, document):
        ...
        __replace = self.db[collection].replace_one(query_sql, document)
...
class MysqlService(DBService):
    ...
    @classmethod
    def update_sql_from_map(cls, table_name, update_target, update_dict):
        ...
        for k, v in update_target.items():
            footer.append('%s="%s"' % (k, v))

        return "%s%s where %s" % (header, ",".join(content), "and".join(footer))
