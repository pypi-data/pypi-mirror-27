...
class FieldRenderer(BaseRenderer):
    ...
    def wrap_widget(self, html):
        if isinstance(self.widget, CheckboxInput):
            checkbox_class = add_css_class('checkbox themed-checkbox', self.get_size_class())
            html = '<div class="{klass}">{content}</div>'.format(
                klass=checkbox_class,
                content=html,
            )
            # Add a span after the input tag for custom checkbox css
            html = html.replace('type="checkbox" />', 'type="checkbox" /><span></span>')
        return html
