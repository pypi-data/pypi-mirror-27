...
class NginxWithPumaWebserver(Webserver):
    ...
    def _configure_puma(self):
        ...
        with self._machine.cd(self._webapp_project_path()):
            ...
            self._machine.run('touch config/puma.production.rb')
            self._machine.append('config/puma.production.rb', self._puma_project_configuration())

    def _puma_already_configured(self):
        config_file = os.path.join(self._webapp_project_path(), 'config/puma.production.rb')
        return self._machine.path_exists(config_file)
    ...
    def _puma_project_configuration(self):
        ...
        configuration += 'rails_env = ENV[\'RAILS_ENV\'] || "production"\n'
    ...
    def _systemd_puma_configuration(self):
        ...
        configuration += 'ExecStart=%s exec puma -C config/puma.production.rb\n' % self._bundle_command_path()
