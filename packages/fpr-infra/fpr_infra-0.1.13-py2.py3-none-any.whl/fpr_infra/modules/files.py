...
@operation
def move(state, host, src, dest, add_deploy_dir=True, force=False, **kwargs):
    """Move a file."""
    ...
    if force:
        mv_args.append('-f')
    mv = 'mv {}'.format(' '.join(mv_args))
    yield server.shell(
        state, host,
        {f"Mv {src_path.as_posix()} file to {dest}"},
        f"{mv} {src_path.as_posix()} {dest}",
        sudo=True
    )
