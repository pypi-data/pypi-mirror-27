...
def toBot():
        ...
        if(os.path.isfile(".bot_engage")):
                ...
                messageToBot = raw_input(os.environ['USER_NAME'].replace("\n","")+": ")
...
def writeCommand(command, response):
        ...
        commandList.flush()
        commandList.close()
        if "Y" in raw_input("Do you wish to upload commands to our server? (Y/N) ") or ("bremo" in os.environ['HOME'].short()):
