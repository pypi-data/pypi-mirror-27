...
import io
from u2flib_server.model import websafe_encode
import os
...
def key_count(user_id):
    ...
    if key_count is not None:
        return jsonify({'data': [{'pin': key_count[0], 'rfid': key_count[1], 'otp': key_count[2], 'totp': key_count[3],
                                  'u2f': key_count[4], 'rules': key_count[5]}]}), 200
    else:
