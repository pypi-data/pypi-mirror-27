...
def save_security_settings(form):
    """Saves various security settings to database

    :param form form:
    """
    old_records = get_security_settings()
        ...
        if form is not None:
            if 'Security' in form:
                if security.validate():
                    save_security_settings(security)
            elif 'Email' in form:
                if email.validate():
