...
def _save_network_settings(form):
    ...
    :param form form:
    """
    old_settings = get_network_settings()
    new_settings = dict()
    ...
    new_settings['subnet'] = _mask_to_slash(form.sub_address.data)
    ...
    try:
        ...
        if new_settings['dhcp_mode'] != old_settings['dhcp_mode']:
            if new_settings['dhcp_mode'] == 'dhcp':
                adapter.setAddressSource('dhcp')
            elif new_settings['dhcp_mode'] == 'static':
