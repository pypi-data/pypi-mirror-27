...
def save_reader_settings(form):
    """Saves all changed reader settings to database

    :param form form:
    """
    old_settings = get_reader_settings()
...
def save_outputs_settings(form):
    """Saves the various outputs and their settings

    :param form form:

    """
    old_settings = get_outputs_settings()
    keys = set(old_settings.keys())
    new_settings = dict()
...
def reader_settings():
    ...
    if request.method == 'POST':
        ...
        if form is not None:
            if 'Reader' in form:
                if readers.validate():
                    save_reader_settings(readers)
            elif 'Outputs' in form:
                if outputs.validate():
                    save_outputs_settings(outputs)
    try:
