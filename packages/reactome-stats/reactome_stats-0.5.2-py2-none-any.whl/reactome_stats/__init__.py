...
'''
|===============================================|
| FUNCTION TEST: Hello Reactome Stats|
|===============================================|
'''
def hello():
  return("Hello, ReactomeStats!")
...
'''
|===============================================|
| FUNCTIONS ANALYSIS 1: Protein family and Reactome pathways |
|===============================================|

- reactome_stats.compareSources()
- reactome_stats.get_NpathwaysPerFamily()
- reactome_stats.barplot_proteinFamilies()
...
def get_NpathwaysPerFamily(tableReactome, tableUniProt, events = 'all', filterN = 50, toWrite = False):
  
  #filter reactome protein id per uniprot human reviewed proteins. 
  #per each family of proteins:
  # -count how many total unique proteins
  # -count how many unique proteins with reactome ID
  # -count how many unique proteins with reactome ID at reactome
  # -count how many unique reactome ID 

  tableReactomeReady = tableReactome

  tableUni = tableUniProt.loc[:,['DB_Object_ID','ProteinFamily','ReactomeID']] #subset of uniprot
  tableReactomeUniProt = tableReactomeReady[tableReactomeReady['DB_Object_ID'].isin(tableUni['DB_Object_ID'])]
  table_ru = tableReactomeUniProt.merge(tableUni, on = 'DB_Object_ID', how = 'inner')


  table_ru_global = None #table with counts
  listProteinFamily = table_ru['ProteinFamily'].unique().tolist()

  listUniqueProteinsCollector = []
  listUniqueProteinsReactomeCollector = []
  listUniqueProteinsReactomeUniprotCollector = []
  listUniqueReactomeIDCollector = []

  n_listUniqueProteinsCollector = []
  n_listUniqueProteinsReactomeCollector = []
  n_listUniqueProteinsReactomeUniprotCollector = []
  n_listUniqueReactomeIDCollector = []

  n_pathwaysPerFamily = []
  n_topPathwaysPerFamily = []
  n_bbePerFamily = []
  n_reactionPerFamily = []
  n_polymerisPerFamily = []
  n_depolymerisPerFamily = []
  n_failedReactPerFamily = []

  counter = len(listProteinFamily)

  for i in listProteinFamily:

    subTable_ru = table_ru[table_ru['ProteinFamily'].isin([i])]
    subTable_r = subTable_ru[~subTable_ru['ReactomeID_x'].isnull()]
    subTable_rr = subTable_ru[~subTable_ru['ReactomeID_y'].isin(['No ReactomeID'])]

    subTable_tp = subTable_ru[subTable_ru['p.schemaClass'].isin(['TopLevelPathway'])]
    subTable_p = subTable_ru[subTable_ru['p.schemaClass'].isin(['Pathway'])]
    subTable_bbe = subTable_ru[subTable_ru['rle.schemaClass'].isin(['BlackBoxEvent'])]
    subTable_react = subTable_ru[subTable_ru['rle.schemaClass'].isin(['Reaction'])]
    subTable_polym = subTable_ru[subTable_ru['rle.schemaClass'].isin(['Polymeristation'])]
    subTable_depolym = subTable_ru[subTable_ru['rle.schemaClass'].isin(['Depolymerisation'])]
    subTable_fail = subTable_ru[subTable_ru['rle.schemaClass'].isin(['FailedReaction'])]

    listUniqueProteins = subTable_ru['DB_Object_ID'].unique().tolist()
    listUniqueProteinsReactome =  subTable_r['DB_Object_ID'].unique().tolist()
    listUniqueProteinsReactomeUniprot =  subTable_rr['DB_Object_ID'].unique().tolist()
    listUniqueReactomeID = subTable_ru['ReactomeID_y'].unique().tolist()

    n_listUniqueProteins = len(listUniqueProteins)
    n_listUniqueProteinsReactome = len(listUniqueProteinsReactome)
    n_listUniqueProteinsReactomeUniprot = len(listUniqueProteinsReactomeUniprot)
    n_listUniqueReactomeID = len(listUniqueReactomeID)

    n_tp = len(subTable_tp['ReactomeID_x'].unique().tolist())
    n_p = len(subTable_p['ReactomeID_x'].unique().tolist())
    n_bbe = len(subTable_bbe['ReactomeID_x'].unique().tolist())
    n_react = len(subTable_react['ReactomeID_x'].unique().tolist())
    n_polym = len(subTable_polym['ReactomeID_x'].unique().tolist())
    n_depolym = len(subTable_depolym['ReactomeID_x'].unique().tolist())
    n_fail = len(subTable_fail['ReactomeID_x'].unique().tolist())

    listUniqueProteinsCollector.append(listUniqueProteins)
    listUniqueProteinsReactomeCollector.append(listUniqueProteinsReactome)
    listUniqueProteinsReactomeUniprotCollector.append(listUniqueProteinsReactomeUniprot)
    listUniqueReactomeIDCollector.append(listUniqueReactomeID)
    n_listUniqueProteinsCollector.append(int(n_listUniqueProteins))
    n_listUniqueProteinsReactomeCollector.append(int(n_listUniqueProteinsReactome))
    n_listUniqueProteinsReactomeUniprotCollector.append(int(n_listUniqueProteinsReactomeUniprot))
    n_listUniqueReactomeIDCollector.append(int(n_listUniqueReactomeID))

    n_pathwaysPerFamily.append(n_tp)
    n_topPathwaysPerFamily.append(n_p)
    n_bbePerFamily.append(n_bbe)
    n_reactionPerFamily.append(n_react)
    n_polymerisPerFamily.append(n_polym)
    n_depolymerisPerFamily.append(n_depolym)
    n_failedReactPerFamily.append(n_fail)

    counter = counter - 1

    if counter%500==0:

      print ("Processed "+str(counter)+" Protein families")

  table_ru_global = pd.DataFrame({'ProteinFamily':listProteinFamily,
                                  'n_listUniqueProteins':n_listUniqueProteinsCollector,
                                  'n_listUniqueProteinsReactome':n_listUniqueProteinsReactomeCollector,
                                  'n_listUniqueProteinsReactomeUniprot':n_listUniqueProteinsReactomeUniprotCollector,
                                  'n_listUniqueReactomeID':n_listUniqueReactomeIDCollector,
                                  'listUniqueProteins':listUniqueProteinsCollector,
                                  'listUniqueProteinsReactome':listUniqueProteinsReactomeCollector,
                                  'listUniqueProteinsReactomeUniprot':listUniqueProteinsReactomeUniprotCollector,
                                  'listUniqueReactomeID':listUniqueReactomeIDCollector, 
                                  'n_pathwaysPerFamily':n_pathwaysPerFamily, 
                                  'n_topPathwaysPerFamily':n_topPathwaysPerFamily, 
                                  'n_bbePerFamily':n_bbePerFamily, 
                                  'n_reactionPerFamily':n_reactionPerFamily, 
                                  'n_polymerisPerFamily':n_polymerisPerFamily, 
                                  'n_depolymerisPerFamily':n_depolymerisPerFamily, 
                                  'n_failedReactPerFamily':n_failedReactPerFamily})

  #print Barplot based on different columns:
  table_ru_globalSorted = table_ru_global.sort_values(by = 'n_listUniqueReactomeID', ascending = False).reset_index()
  print("....")
  barplot_proteinFamilies(table_ru_globalSorted, filterN = filterN)

  if toWrite == True:

    nameDf = 'rectomePathwaysEventCounts'
    folderPath = os.getcwd()
    pathToWrite = (folderPath+nameDf+".csv")  
    table_ru_globalSorted.to_csv(pathToWrite, index = False)
    print ("Table is already available at: "+pathToWrite) 

  return(table_ru_globalSorted)

def barplot_proteinFamilies (tableCounts, filterN):

  #BAR PLOT
  height = tableCounts['n_listUniqueReactomeID'].tolist()[0:filterN]
  bars_family = tableCounts['ProteinFamily'].tolist()[0:filterN]
  bars_comma = [x.replace('family','') for x in bars_family]
  bars = [x.replace(',','\n') for x in bars_comma]
  y_pos = np.arange(len(bars))

  plt.style.use('bmh')
  # Create bars
  plt.bar(y_pos, height,color = (0.8, 0.1, 0.1, 0.6))
...
def barStack_proteinFamiliesPathEvents (tableCounts, filterN):
  
  #stackBarplot pathways and events

  # Values of each group
  bars1 = tableCounts['n_pathwaysPerFamily'].tolist()[0:filterN]
  bars2 = tableCounts['n_topPathwaysPerFamily'].tolist()[0:filterN]
  bars3 = tableCounts['n_bbePerFamily'].tolist()[0:filterN]
  bars4 = tableCounts['n_reactionPerFamily'].tolist()[0:filterN]
  bars5 = tableCounts['n_polymerisPerFamily'].tolist()[0:filterN]
  bars6 = tableCounts['n_depolymerisPerFamily'].tolist()[0:filterN]
  bars7 = tableCounts['n_failedReactPerFamily'].tolist()[0:filterN]
  ...
  names_family = tableCounts["ProteinFamily"].tolist()[0:filterN]
  names_comma = [x.replace('family','') for x in names_family]
  names = [x.replace(',','\n') for x in names_comma]  
  barWidth = 1
  ...
  for name, hex in matplotlib.colors.cnames.items():
    #print(name, hex)
    sequenceColors.append(hex)

  plt.style.use('bmh')
  # Create brown bars
  plt.bar(r, bars1, color=sequenceColors[0] , edgecolor = 'white', width=barWidth, label = "Pathway")
  # Create green bars (middle), on top of the firs ones
  plt.bar(r, bars2, bottom=bars1, color=sequenceColors[5] , edgecolor='white', width=barWidth, label = "TopLevelPathway")
  # Create green bars (top)
  plt.bar(r, bars3, bottom=[bars1[i]+bars2[i] for i in range (0, filterN)], color=sequenceColors[10], edgecolor='white', width=barWidth, label = "BlackBoxEvent")
  # Create green bars (top)
  plt.bar(r, bars4, bottom=[bars1[i]+bars2[i]+bars3[i] for i in range (0, filterN)], color=sequenceColors[15], edgecolor='white', width=barWidth, label = "Reaction")
  # Create green bars (top)
  plt.bar(r, bars5, bottom=[bars1[i]+bars2[i]+bars3[i]+bars4[i] for i in range (0, filterN)], color=sequenceColors[20], edgecolor='white', width=barWidth, label = "Polymeristation")
  # Create green bars (top)
  plt.bar(r, bars6, bottom=[bars1[i]+bars2[i]+bars3[i]+bars4[i]+bars5[i] for i in range (0, filterN)], color=sequenceColors[25], edgecolor='white', width=barWidth, label = "Depolymerisation")
  # Create green bars (top)
  plt.bar(r, bars7, bottom=[bars1[i]+bars2[i]+bars3[i]+bars4[i]+bars5[i]+bars6[i] for i in range (0, filterN)], color=sequenceColors[30], edgecolor='white', width=barWidth, label = "FailedReaction")
   
  # Custom X axis
  plt.xticks(r, names, rotation = 90, size = 6)
  plt.xlabel("group")
  plt.legend()
  # Show graphic
  plt.show()
...
'''
|===============================================|
| FUNCTIONS SUMMATION: Protein family and Reactome pathways |
|===============================================|

- reactome_stats.summary

get a table to see f across all the analysis the results match properly
