...
check_url_list=[]
for url in urls:
    match=re.search(r"(([^\?&\^]*ip[^=]*=[^&]*)|([^\?&\^]*host[^=]=[^&]*)|([^\?&\^]*addr[^=]=[^&]*)|([^\?&\^]*cmd[^=]=[^&]*)|([^\?&\^]command[^=]=[^&]*))",url,re.I)
    if match:
...
def check(url):
    ...
    if "^" in url:
        ...
        data=url_list[1]
        if cookie!="":
            cmd='''cd %s && python2 commix.py -u "%s" --data "%s" --cookie "%s" -v 3 --batch | tee %s''' % (current_dir+"/commix",url,data,cookie,current_log_file)
            os.system(cmd)
        else:
            cmd='''cd %s && python2 commix.py -u "%s" --data "%s" -v 3 --batch | tee %s''' % (current_dir+"/commix",url,data,current_log_file)
            os.system(cmd)
    else:
        if cookie!="":
            cmd='''cd %s && python2 commix.py -u "%s" --cookie "%s" -v 3 --batch | tee %s''' % (current_dir+"/commix",url,cookie,current_log_file)
            os.system(cmd)
        else:
            cmd='''cd %s && python2 commix.py -u "%s" -v 3 --batch | tee %s''' % (current_dir+"/commix",url,current_log_file)
            os.system(cmd)
