...
if target[:4]!="http":
    ...
    if alive==0:
        ...
        pass
    return_string=""
    command="cd %s/hackUtils && python hackUtils.py -r %s > %s" % (current_dir,target,current_log_file)
    print(command)
    os.system(command)
    ...
    command="cd %s/joomla_exp && python joomla_exp.py %s > joomla_exp_log_tmp_file" % (current_dir,target+"/")
    print(command)
    os.system(command)
