...
class NumericalScheme(utils.ReprObject):
    ...
    def long_description(self, newlong_description):
        self._long_description = str(newlong_description)
    ...
    def equation(self):
        ...
        try:
            ...
            self._equation = self._default_equation
        return self._equation

    @equation.setter
    def equation(self, newequation):
        assert all([hasattr(newequation, a) for a in
                    ["derivative", "linear_factor", "nonlinear_addend",
                     "independent_addend", "variable"]]), \
            "equation does not seem to be instance of DerivativeEquation"
    ...
    def ignore_linear(self):
        ...
        try:
            ...
            self._ignore_linear = self._default_ignore_linear
    ...
    def max_timestep(self, time=None, variablevalue=None):
        ...
        try:
            ...
            except BaseException:  # estimating didn't work
                ts = self.fallback_max_timestep  # try fallback
            assert utils.is_numeric(ts)
            assert np.asarray(ts).size == 1
        except BaseException:  # neither estimate nor fallback were sensible
            ts = 1  # default
            raise
        return ts
    ...
    def integrate(self, time=None, until=None):
        ...
        if time is None:
            time = self.equation.variable.time
        current_max_timestep = self.max_timestep
        logger.debug("current maximum timestep is {}".format(
            current_max_timestep))
        if until is None:
            until = time + current_max_timestep
        logger.debug("integrating until time {}".format(until))
        time_now = time
        while time_now < until:
            logger.debug(("current time {} is smaller than until time {}"
            ...
            if time_left > current_max_timestep:  # full max_timestep fits
                ...
                timestep = time_left
            logger.debug(("integrate one step from time {} with "
                          "timestep {}").format(time_now, timestep))
            # integrate one step
            self.integrate_step(time=time_now, timestep=timestep)
            time_now += timestep
        logger.debug(("reached until time {}").format(time_now))
    ...
    def integrate_step(self, time=None, timestep=None):
        ...
        # logger.debug("{} tendency: {}".format(var.id,tend))
        # logger.debug("var() = {}".format(var()))
        # logger.debug("var({}) = {}".format(time,var(time)))
        # logger.debug("var.time = {}".format(var.time))
        # logger.debug("var(var.time) = {}".format(var(var.time)))
        new = var(time) + tend
...
class EulerImplicit(NumericalScheme):
    ...
    def step(self, time=None, timestep=None, tendency=True):
        ...
        assert np.all(
            nonlin == 0), ("nonlinear part of equation is not "
...
class LeapFrog(NumericalScheme):
    ...
    def _needed_timesteps_for_integration_step(self, timestep):
        # prev time and current time needed
        return np.array([-1, 0]) * timestep
...
class SetOfNumericalSchemes(utils.SetOfObjects):
    ...
    def plan(self):
        ...
        except BaseException:  # automatic planning didn't work
    ...
    def fallback_plan(self):
        ...
        try:
            ...
            self._fallback_plan = self._default_fallback_plan
    ...
    def assert_plan_valid(self, plan):
        ...
        for stage in plan:  # go over each stage
            ...
            if max(steps) != 1 or steps[-1] != 1:
                raise ValueError(
                    "The plan for {} does not end with a "
            ...
            logger.debug("{} needs the unified timesteps {}".format(
                                ...
                                msg = (
                                    ...
                                    "something like {}".format(
                                        ivid, needed_step, in_ivid, [
                                            in_ivid, [
                                                x for x in needed_steps_uni if x != 0]], ))
                                logger.debug(msg)
                                raise ValueError(msg)
                    else:  # we don't model this ivid
                        logger.debug(
                            "We do not simulate {} so we "
            ...
            logger.debug("We successfully integrated {} to {}".format(
                ivid, steps))
        logger.debug("Final test simulation: {}".format(simulation))
        logger.debug("### Plan test was successfull ###")
        return True
    ...
    def integrate(self, start_time, final_time):
        ...
        while current_time < final_time:
            ...
            for plan_step in plan:
                ...
                for ts in timesteps:  # loop over all timesteps
                    ...
                    logger.debug(
                        ("integrate scheme '{}' for equation '{}' until time {}").format(
                            scheme.description, scheme.equation.description, until_time))
                    scheme.integrate(
                        time=scheme_time,
                        until=until_time)
        ...
        logger.debug("end of integration")
