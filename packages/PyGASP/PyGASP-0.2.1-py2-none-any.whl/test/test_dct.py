import pygasp.dct as dct
import numpy as np
import scipy.fftpack
import pytest


def test_dct_equal_scipy_arange():
    for i in range(8, 11):
...
def test_idct_equal_scipy_arange():
    for i in range(8, 11):
        signal = np.arange(2**i, dtype=np.float64)
...
def test_dct2_equal_scipy_arange():
    for i in [100, 105]:
...
def test_idct2_equal_scipy_arange():
    for i in [100, 105]:
        signal = np.arange(i**2, dtype=np.float64).reshape(i, i)
