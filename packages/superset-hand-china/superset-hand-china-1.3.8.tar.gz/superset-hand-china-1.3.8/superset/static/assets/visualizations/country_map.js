...
function countryMapChart(slice, payload) {
  ...
  const sliceHeight = slice.height()-20;
  const sliceWidth = slice.width()-20;
  ...
  const clicked = function (d) {
    ...
    g.transition()
      .duration(750)
      .attr('transform', 'translate(' + sliceWidth / 2 + ',' + sliceHeight / 2 + ')scale(' + k + ')translate(' + -x + ',' + -y + ')');
    bigText.transition()
      .duration(750)
  ...
  d3.json(url, function (error, mapData) {
    ...
    let scale = 150;
    let offset = [sliceWidth / 2, sliceHeight / 2];
    ...
    const hscale = scale * sliceWidth / (bounds[1][0] - bounds[0][0]);
    const vscale = scale * sliceHeight / (bounds[1][1] - bounds[0][1]);
    scale = (hscale < vscale) ? hscale : vscale;
    const offsetWidth = sliceWidth - (bounds[0][0] + bounds[1][0]) / 2;
    const offsetHeigth = sliceHeight - (bounds[0][1] + bounds[1][1]) / 2;
