...
function horizonViz(slice, payload) {
  ...
  let container = slice.container;
  try {
    container.parents('.chart-container').css('max-height', slice.height()-20);
    container.parents('.chart-container').css('overflow', 'auto');
    container.parents('.chart-container').css('width', '99%');
  } catch (error) {
    $('#'+slice.containerId).css('height', slice.height()-20);
    $('#'+slice.containerId).css('overflow', 'auto');
    $('#'+slice.containerId).css('position', 'relative');
  }   
}
