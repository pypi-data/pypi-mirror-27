...
class Slice(Model, AuditMixinNullable, ImportMixin):
    ...
    def import_obj(cls, slc_to_import, import_time=None):
        ...
        if not slc_to_import.multi_language:
            slc_to_import.multi_language = '{}'
    ...
    @classmethod
    def override_slices(cls, override_dashboard, slice_id_dicts, dash_id_dicts):
        session = db.session
        slices = override_dashboard.slices
        for slc in slices:
            if not slc.multi_language:
                slc.multi_language = '{}'
            slc_params = json.loads(slc.params)
            if slc.viz_type == 'filter_box_combination':
                slc_ids = []
                for filter_slc_id in slc_params['filter_combination']:
                    filt_slc_id = int(filter_slc_id)
                    if filt_slc_id in slice_id_dicts:
                        slc_ids.append(slice_id_dicts[filt_slc_id])
                if len(slc_ids) > 0:
                    slc_params['filter_combination'] = slc_ids
                    slc.params = json.dumps(slc_params)
                    slc_to_override = None
                    filt_com_slc_id = int(slc.id)
                    if filt_com_slc_id in slice_id_dicts:
                        slc_to_override = session.query(Slice).filter_by(
                            id=slice_id_dicts[filt_com_slc_id]
                        ).first()
                    if slc_to_override:
                        slc_to_override.override(slc)
            elif slc.viz_type == 'ag_grid':
                is_override = False
                for slc_navigator in slc_params['navigator']:
                    slc_nav_id = None
                    dash_nav_id = None
                    if slc_navigator['navigate_type'] == 'slice':
                        if slc_navigator['slice_id']:
                            slc_nav_id = int(slc_navigator['slice_id'])
                            if slc_nav_id in slice_id_dicts:
                                is_override = True
                                slc_navigator['slice_id'] = slice_id_dicts[slc_nav_id]
                    elif slc_navigator['navigate_type'] == 'dashboard':
                        if slc_navigator['dashboard_id']:
                            dash_nav_id = int(slc_navigator['dashboard_id'])
                            if dash_nav_id in dash_id_dicts:
                                is_override = True
                                slc_navigator['dashboard_id'] = dash_id_dicts[dash_nav_id]
                if is_override == True:
                    slc.params = json.dumps(slc_params)
                    ag_slc_to_override = None
                    ag_grid_slc_id = int(slc.id)
                    if ag_grid_slc_id in slice_id_dicts:
                        ag_slc_to_override = session.query(Slice).filter_by(
                            id=slice_id_dicts[ag_grid_slc_id]
                        ).first()
                    if ag_slc_to_override:
                        ag_slc_to_override.override(slc)
        session.flush()
        return override_dashboard.id
...
class Dashboard(Model, AuditMixinNullable, ImportMixin):
    ...
    @classmethod
    def import_obj(cls, dashboard_to_import, slice_id_dicts, import_time=None):
        ...
        for slc in slices:
            ...
            if slc.id not in slice_id_dicts:
                slice_id_dicts[slc.id] = new_slc_id
        ...
        if not dashboard_to_import.multi_language:
            dashboard_to_import.multi_language = '{}'
        ...
        for dashboard_id in dashboard_ids:
            ...
            if copied_dashboard:
                ...
                for slc in copied_dashboard.slices:
                    slc_id = int(slc.id)
                    if slc_id not in slices_ids:
                        slices_ids.append(slc_id)
                    ...
                    if slc.viz_type == 'filter_box_combination':
                        for filter_id in slc_params['filter_combination']:
                            ...
                            if filter_slc:
                                ...
                                if filter_id not in slices_ids:
                                    slices_ids.append(filter_id)
                                if not filter_slc.multi_language:
                                    filter_slc.multi_language = '{}'
                        ...
                        for slc_navigator in slc_params['navigator']:
                            ...
                            if slc_navigator['navigate_type'] == 'slice':
                                if slc_navigator['slice_id']:
                                    slc_nav_id = int(slc_navigator['slice_id'])
                                    if slc_nav_id not in navigation_slice_ids:
                                        navigation_slice_ids.append(slc_nav_id)
                            elif slc_navigator['navigate_type'] == 'dashboard':
                                if slc_navigator['dashboard_id']:
                                    dash_nav_id = str(slc_navigator['dashboard_id'])
                                    if dash_nav_id not in dashboard_ids:
                                        dashboard_ids.append(dash_nav_id)

                for navigation_slice_id in navigation_slice_ids:
                    navigation_slice_id = int(navigation_slice_id)
                    if navigation_slice_id not in slices_ids:
                        nav_slc = (db.session.query(Slice).filter_by(id=navigation_slice_id).first())
                        if nav_slc:
                            datasource_ids.add((nav_slc.datasource_id, nav_slc.datasource_type))
                            # add extra params for the import
                            nav_slc.alter_params(
                                remote_id=navigation_slice_id,
                                datasource_name=nav_slc.datasource.name,
                                schema=nav_slc.datasource.name,
                                database_name=nav_slc.datasource.database.name,
                            )
                            if not nav_slc.multi_language:
                                nav_slc.multi_language = '{}'
                            navigation_slcs.append(nav_slc)

                copied_dashboard.alter_params(remote_id=dashboard_id)
                if not copied_dashboard.multi_language:
                    copied_dashboard.multi_language = '{}'
                ...
                for dashboard_id, dashboard_type in datasource_ids:
                    ...
                    if not eager_datasource.multi_language:
                        eager_datasource.multi_language = '{}'
                    eager_datasources.append(eager_datasource)
        
        # copied_dashboard.slices.extend(navigation_slcs)
        # copied_dashboard.slices.extend(filter_slcs)
        extra_slcs.extend(navigation_slcs)
        extra_slcs.extend(filter_slcs)
        return pickle.dumps({
            'extra_slices': extra_slcs,
