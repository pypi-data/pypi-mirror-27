...
class DatabaseView(SupersetModelView, DeleteMixin):  # noqa
    ...
    show_columns = [
        ...
        'changed_by',
        'changed_on',
        'expose_in_sqllab', 'allow_run_async', 'allow_run_sync', 'allow_ctas', 'allow_dml', 'force_ctas_schema',
        'impersonate_user', 'verbose_name'
    ]
...
class SliceModelView(SupersetModelView, DeleteMixin, SupersetFilter):  # noqa
    ...
    def pre_update(self, obj):
        ...
        if number_of_existing_slices > 1:
            if obj.subtitle:
                slc_name_sub = obj.slice_name + '[' + obj.subtitle + ']'
                raise Exception(__("Slice Name [ {} ] already exists, Please change slice name to save!").format(slc_name_sub))
            else:
...
class Superset(BaseSupersetView):
    ...
    def explore_json(self, datasource_type, datasource_id):
        ...
        vizT = ['ag_grid','echarts_bar','echarts_bar_waterfall','echarts_bar_h','echarts_area_stack',
                    ...
                    'echarts_big_number','big_number_viz','echarts_radar_map','echarts_pie_h_g',
                    'table','echarts_bar_progress','echarts_bubble','china_city_map','echarts_china_map'
                    ,'echarts_china_city_map_migration','echarts_quadrant']
        # vizT = []
        ...
        if payload['form_data']['viz_type'] in vizT:
            if 'data' in payload:
                if  datasource_type == 'table':
                    ...
                    if payload['form_data']['viz_type'] == 'echarts_bubble':
                        for data in payload['data']:
                            for i in range(0,len(data['values'])):
                                ...
                                for key in val.keys():
                                    for metric in metrics:
                                        if key == metric.metric_name:
                                            v[metric.nick_name] = val[key]
                                    for column in columns:
                                        if key == column.column_name:
                                            v[column.nick_name] = val[key]
                                    v['shape'] = val['shape']
                                    v['size'] = val['size']
                                    v['x'] = val['x']
                                    v['y'] = val['y']
    ...
    def import_dashboards(self):
        ...
        if request.method == 'POST' and f:
            ...
            data = pickle.load(f)
            # TODO: validation dashboard name and slice name
            slice_names = []
            dashboards_names = []
            for dashboard in data['dashboards']:
                number_of_existing_dashboard = db.session.query(sa.func.count('*')).filter(
                    models.Dashboard.dashboard_title == dashboard.dashboard_title
                ).scalar()
                # dashboard name is already exists in the session
                if number_of_existing_dashboard > 0:
                    if dashboard.dashboard_title not in dashboards_names:
                        dashboards_names.append(dashboard.dashboard_title)
                
                import_slices = dashboard.slices
                for slc in import_slices:
                    number_of_existing_slice = db.session.query(sa.func.count('*')).filter(
                        models.Slice.slice_name == slc.slice_name,
                        models.Slice.subtitle == slc.subtitle
                    ).scalar()
                    # slice name is already exists in the session
                    if number_of_existing_slice > 0:
                        slice_name_sub = None
                        if slc.subtitle:
                            slice_name_sub = slc.slice_name + '[' + slc.subtitle + ']'
                        else:
                            slice_name_sub = slc.slice_name
                        if slice_name_sub:
                            if slice_name_sub not in slice_names:
                                slice_names.append(slice_name_sub)
                
            if len(dashboards_names) > 0:
                dash_names = ', '.join(dashboards_names)
                flash(__("Dashboard Name [ {} ] already exists, Please change dashboard name to import!").format(dash_names),"danger")
                if len(slice_names) > 0:
                    sli_names = ', '.join(slice_names)
                    flash(__("Slice Name [ {} ] already exists, Please change slice name to import!").format(sli_names),"danger")
                return redirect('/dashboardmodelview/list/')
            else:
                if len(slice_names) > 0:
                    sli_names = ', '.join(slice_names)
            ...
            dash_id_dicts = {}
            for dashboard in data['dashboards']:
                old_dash_id = int(dashboard.id)
                new_dash_id = models.Dashboard.import_obj(
                    dashboard, slice_id_dicts, import_time=current_tt)
                if old_dash_id not in dash_id_dicts:
                    dash_id_dicts[old_dash_id] = new_dash_id
            db.session.commit()
            if 'extra_slices' in data:
                for extra_slice in data['extra_slices']:
                    old_slice_id = int(extra_slice.id)
                    new_slice_id = models.Slice.import_obj(
                        extra_slice, import_time=current_tt)
                    if old_slice_id not in slice_id_dicts:
                        slice_id_dicts[old_slice_id] = new_slice_id
                db.session.commit()
            for dashboard in data['dashboards']:
                models.Slice.override_slices(dashboard, slice_id_dicts, dash_id_dicts)
            db.session.commit()
            return redirect('/dashboardmodelview/list/')
    ...
    def explore(self, datasource_type, datasource_id):
        ...
        hideNav = request.args.get("hideNav") == "true"
        bootstrap_data = {
    ...
    def save_dash(self, dashboard_id):
        ...
        multi_language = eval(dash.multi_language)
        if multi_language.__contains__('en'):
            if dash.nick_name == multi_language['en']:
                data['multi_language'] = '{"en":"'+data["dashboard_title"]+'","zh":"'+multi_language["zh"]+'"}'
                data['dashboard_title'] = dash.dashboard_title
        if multi_language.__contains__('zh'):
            if dash.nick_name == multi_language['zh']:
                data['multi_language'] = '{"en":"'+multi_language["en"]+'","zh":"'+data["dashboard_title"]+'"}'
                data['dashboard_title'] = dash.dashboard_title
        else:
            data['multi_language'] = '{}'      
        self._set_dash_metadata(dash, data)
    ...
    def _set_dash_metadata(dashboard, data):
        ...
        dashboard.multi_language = data['multi_language']
    ...
    def sqllab(self):
        ...
        return self.render_template(
            'superset/basic.html',
            entry='sqllab',
            hideNav=request.args.get("hideNav"),
