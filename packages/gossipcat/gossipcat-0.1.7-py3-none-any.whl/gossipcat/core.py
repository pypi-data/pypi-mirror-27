...
def glimpse(data, target):
    ...
    Prints a general infomation of the dataset and plot the
    distribution of target value.

    Args:
        data: A dataset you wanna glimpse.
        target: The target variable in your dataset; limited
            to binary.
    """
    print('\nInformation:')
    print(data.info())
...
def features_dup(data, n_head=5000, print_dup=False):
    ...
    Args:
        data: A dataset you wanna check and return duplicated 
            columns names.
        n_head: First n_head rows to be checked; default 5000.
        print_dup: Whether print duplicated columns names.
...
def corr_pairs(data, gamma=0.99999):
    """
...
def features_new(data, ls, target, auc_score=0.75, silent=False):
    """
    ...
    for index, value in enumerate(ls):
        temp = data[ls[index][0]] - data[ls[index][1]]
        if len(temp.unique()) > 1:
            ...
            if auc > auc_score:
                ...
                new['-'.join(value)] = data[ls[index][0]] - data[ls[index][1]]
    return new
