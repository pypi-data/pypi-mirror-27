...
def count_blue_pixels(image, canvas):
    ...
    for x in range(image.width):
        for y in range(image.height):
            ...
            sv = s + v
            if (h >= 100 and h < 110 and (sv > 270)) or\
                (h >= 110 and h < 120 and (sv > 260)) or\
                (h >= 120 and h < 125 and (sv > 250)) or\
                (h >= 125 and h < 130 and (sv > 255)) or\
                (h >= 130 and h < 135 and (sv > 270)) or\
                (h >= 135 and h < 140 and (sv > 275)) or\
                (h >= 140 and h < 145 and (sv > 280)) or\
                (h >= 145 and h < 150 and (sv > 285)) or\
                (h >= 150 and h < 155 and (sv > 305)) or\
                (h >= 155 and h < 160 and (sv > 310)) or\
                (h >= 160 and h < 165 and (sv > 325)) or\
                (h >= 165 and h < 170 and (sv > 330)) or\
                (h >= 170 and h < 175 and (sv > 335)) or\
                (h >= 175 and h < 180 and (sv > 340)) or\
                (h >= 180 and h < 175 and (sv > 345)):
...
def get_all(sky_image_path, is_daytime, alert_recipients, image_save_to_dir):
    """Returns image with overlaid clouds detected and the cloud coverage percentage
       as a float. Returns original image and None if no clouds detected.
    ...
    alert_recipients    - filepath to .txt file containing email addresses of recipients of waterspout alerts.
    ...
    if is_daytime:
        ...
        if len(cloud_rects) == 0 and len(waterspout_rects) == 0:
            return sky_image, None
        ...
        #clouds were detected, use upper_left to get skycolor and call count_sky_pixels
            ...
            count = count_sky_pixels(q, sky_image_HSV, sky, canvas)
            
            #if waterspout detected, overlay box on detected waterspout and send email notification
            if len(waterspout_rects) > 0:
                red = (255,255,255)
                waterspout_im = OverlayBoxes(sky_image_HSV, fromWHtoXY(waterspout_rects), fill=None, outline=red)
                waterspout_im = waterspout_im.convert("RGB")
                waterspout_im.save(image_save_to_dir + "temp_waterspout.jpg")
                waterspoutalert.send_message(image_save_to_dir + "temp_waterspout.jpg", alert_recipients)
