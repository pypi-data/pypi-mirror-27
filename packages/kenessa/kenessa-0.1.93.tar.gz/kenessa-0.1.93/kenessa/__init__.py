...
class Kenessa:
    ...
    def get_district(self, province):
        ...
        query = "SELECT District.id,District.name \
                ...
                WHERE Province.id IN ({seq}) OR Province.name IN ({seq})".format(seq=','.join(['?'] * len(province)))
    ...
    def get_sector(self, district):
        ...
        query = "SELECT Sector.id,Sector.name \
                ...
                WHERE District.id IN ({seq}) OR District.name IN ({seq})".format(seq=','.join(['?'] * len(district)))
    ...
    def get_all_from_village_id(self, village):
        ...
        id_cell = {'cell_id': m_cell.group()}
        output.append(id_cell)
        # id sector
        m_sector = re.search("\d{4}", str(list(id_village[0])[0]))
        id_sector = {"sector_id": m_sector.group()}
        output.append(id_sector)
        # id district
        m_district = re.search("\d{2}", str(list(id_village[0])[0]))
        id_district = {"district_id": m_district.group()}
        output.append(id_district)
        # id province
        m_province = re.search("\d{1}", str(list(id_village[0])[0]))
        id_province = {"province_id": m_province.group()}
        output.append(id_province)
