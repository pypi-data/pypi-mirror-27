...
def HybridCE(preds,labs,weighting,seq_len,FP_weighting_):      
   ...
   previous_ce=tf.map_fn(lambda x: -tf.reduce_sum(CSFunctions.cross_entropy_peak_dif_form(x[0],x[1],x[2],x[3],x[4]), reduction_indices=[1]), (preds[:,1:seq_len+1],labs[:,1:seq_len+1],preds[:,:seq_len],labs[:,:seq_len],FP_weighting_),dtype=(tf.float32))
   future_ce=tf.map_fn(lambda x: -tf.reduce_sum(CSFunctions.cross_entropy_peak_dif_form(x[0],x[1],x[2],x[3],x[4]), reduction_indices=[1]), (preds[:,1:seq_len+1],labs[:,1:seq_len+1],preds[:,2:seq_len+2],labs[:,2:seq_len+2],FP_weighting_),dtype=(tf.float32))
