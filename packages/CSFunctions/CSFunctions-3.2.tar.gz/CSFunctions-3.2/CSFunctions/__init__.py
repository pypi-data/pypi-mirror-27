...
def cross_entropy_peak_dif_form(pred1,lab1,pred2,lab2,eps=np.finfo(float).eps):
    out=((tf.abs(lab1-lab2)*(tf.log(tf.abs(pred1-pred2))+eps))+((1-tf.abs(lab1-lab2))*(tf.log(1-tf.abs(pred1-pred2)+eps))))
    return out 

def cross_entropy_peak_dif_form_TP(pred1,lab1,pred2,lab2,eps=np.finfo(float).eps):
    out=tf.abs(lab1-lab2)*(tf.log(tf.abs(pred1-pred2)+eps))
    return out

def cross_entropy_peak_dif_form_FP(pred1,lab1,pred2,lab2,eps=np.finfo(float).eps):
    out=(1-tf.abs(lab1-lab2))*(tf.log(1-tf.abs(pred1-pred2)+eps))
    return out
...
def HybridCETP(preds,labs,weighting,seq_len):
   stand_ce=tf.map_fn(lambda x: -tf.reduce_sum(CSFunctions.cross_entropy_form(x[0],x[1]), reduction_indices=[1]),(preds[:,1:seq_len+1],labs[:,1:seq_len+1]),dtype=(tf.float32))    
   previous_ce=tf.map_fn(lambda x: -tf.reduce_sum(CSFunctions.cross_entropy_peak_dif_form_TP(x[0],x[1],x[2],x[3]), reduction_indices=[1]), (preds[:,1:seq_len+1],labs[:,1:seq_len+1],preds[:,:seq_len],labs[:,:seq_len]),dtype=(tf.float32))
   future_ce=tf.map_fn(lambda x: -tf.reduce_sum(CSFunctions.cross_entropy_peak_dif_form_TP(x[0],x[1],x[2],x[3]), reduction_indices=[1]), (preds[:,1:seq_len+1],labs[:,1:seq_len+1],preds[:,2:seq_len+2],labs[:,2:seq_len+2]),dtype=(tf.float32))
   cost=tf.reduce_mean(tf.add(tf.reshape(stand_ce,[-1]),(weighting*tf.add(tf.reshape(future_ce,[-1]),tf.reshape(previous_ce,[-1])))))
   return cost

def HybridCEFP(preds,labs,weighting,seq_len):
   stand_ce=tf.map_fn(lambda x: -tf.reduce_sum(CSFunctions.cross_entropy_form(x[0],x[1]), reduction_indices=[1]),(preds[:,1:seq_len+1],labs[:,1:seq_len+1]),dtype=(tf.float32))    
   previous_ce=tf.map_fn(lambda x: -tf.reduce_sum(CSFunctions.cross_entropy_peak_dif_form_FP(x[0],x[1],x[2],x[3]), reduction_indices=[1]), (preds[:,1:seq_len+1],labs[:,1:seq_len+1],preds[:,:seq_len],labs[:,:seq_len]),dtype=(tf.float32))
   future_ce=tf.map_fn(lambda x: -tf.reduce_sum(CSFunctions.cross_entropy_peak_dif_form_FP(x[0],x[1],x[2],x[3]), reduction_indices=[1]), (preds[:,1:seq_len+1],labs[:,1:seq_len+1],preds[:,2:seq_len+2],labs[:,2:seq_len+2]),dtype=(tf.float32))
   cost=tf.reduce_mean(tf.add(tf.reshape(stand_ce,[-1]),(weighting*tf.add(tf.reshape(future_ce,[-1]),tf.reshape(previous_ce,[-1])))))
   return cost
...
def NewHybridCE(preds,labs,weighting,seq_len):
   ...
   return cost,stand_ce+previous_ce+future_ce
