...
class SettingsEntry:
    ...
    def __attrs_post_init__(self):
        self.apiMapping = self.create_apimapping(self.apiMapping)
        self.settings = self.create_settings(self.settings)

    def create_settings(self, x):
        if x is not None:
            return {y["titleTextID"] : SettingsEntry(**y, device=self.device) for y in x}

    def create_apimapping(self, x):
        if x is not None:
            return ApiMapping(**x)

    async def get_value(self):
        """Returns current value for this setting."""
        _LOGGER.debug("Getting value: %s", self.apiMapping)
        res = await self.device.get_setting(self.apiMapping.service,
    ...
    async def set_value(self, value):
        ...
        res = await self.device.set_setting(self.apiMapping.service,
                                            self.apiMapping.setApi['name'],
                                            self.apiMapping.target,
                                            value)
    ...
    apiMapping = attr.ib(default=None)
