...
async def traverse_settings(dev, module, depth=0):
    """Goes through all """
    for setting in module.settings.values():
        if setting.is_directory:
            print("%s%s (%s)" % (depth * ' ', setting.title, setting.titleTextID))
            return await traverse_settings(dev, setting, depth + 2)
        else:
            print_settings([await setting.get_value()], depth=depth)
...
@click.argument("value", required=False)
@coro
async def settings(dev: Protocol, service, target, value):
    """Print or set settings.

    Calling without parameters will list all available settings.
    Passing `service` will show only settings from that service.
    Passing `target` will return only that setting.
    Passing `value` allows setting a new value for the give setting.
    """
    settings_tree = await dev.get_settings()

    if service is not None and (value is None):
        if service not in settings_tree:
            click.echo("No settings for service '%s'" % service)
            return
        if target:
            pass
        module = settings_tree[service]
        await traverse_settings(dev, module)
        return

    for module in settings_tree.values():
        await traverse_settings(dev, module)
