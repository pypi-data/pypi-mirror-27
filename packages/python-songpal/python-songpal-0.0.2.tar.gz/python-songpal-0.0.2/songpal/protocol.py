...
class Protocol:
    ...
    async def get_settings(self) -> Dict[str, SettingsEntry]:
        settings = await self.request_settings()
        return {x["usage"]: SettingsEntry(**x, device=self) for x in settings["settings"]}
    ...
    async def set_setting(self, service: str, method: str, target: str, value: str):
        params = {"settings": [{"target": target, "value": value}]}
