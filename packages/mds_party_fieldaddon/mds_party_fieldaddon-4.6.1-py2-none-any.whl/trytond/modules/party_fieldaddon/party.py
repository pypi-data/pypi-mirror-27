...
class Party(ModelSQL, ModelView):
    ...
    def get_pfacomposedname(cls, parties, names):
        ...
        qu1 = tab_party.select(tab_party.id,
            Concat2(
                Case(
                    # False or Null if: tab_party.name in [Null, '']
                    (CharLength(tab_party.name) > 0, tab_party.name),
                    else_ = Concat2(u'[', tab_party.code, ']')
                ),
                Case(
                    (CharLength(tab_party.pfafirstname) > 0,
                    Concat2(u', ', tab_party.pfafirstname)),
                    else_ = None
                ),
                Case(
                    ((CharLength(tab_party.pfacompany) > 0) |
                     (CharLength(tab_party.pfadepartment) > 0),
                        Concat2(
                            u' [',
                            tab_party.pfacompany,
                            Case(
                                ((CharLength(tab_party.pfacompany) > 0) & 
                                 (CharLength(tab_party.pfadepartment) > 0), 
                                u', '
