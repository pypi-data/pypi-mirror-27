...
def getUnixTime(low, high):
    """
        Returns unixtime, including microseconds
    """
    seconds = float(high) * 2 ** 32 + low
    return seconds * 1e-7 - 11644473600

def parse_record(raw_record, opts):
    """
        parses data from a raw MFT record
            # record = {
            #       'f1': '\x00\x00',
            #       'seq': 1, 
            #       'lsn': 5.0929133824e-314, 
            #       'attr_off': 56, 
            #       'next_attrid': 7, 
            #       'alloc_sizef': 1024, 
            #       'recordnum': 0, 
            ...
            #               'par_ref': 5, 
            #               'real_fsize': 16384, 
            #               'mtime': <analyzemft.mftutils.WindowsTime instance at 0x1016dc128>, 
            #               'atime': <analyzemft.mftutils.WindowsTime instance at 0x1016dc1b8>, 
            #               'crtime': <analyzemft.mftutils.WindowsTime instance at 0x1016dc0e0>, 
            #               'name': '$MFT', 
            #               'ctime': <analyzemft.mftutils.WindowsTime instance at 0x1016dc170>, 
            #               'par_seq': 5, 
            #               'nlen': 4, 
            #               'flags': 3e-323, 
            #               'alloc_fsize': 16384, 
            ...
            #               'sec_id': 256, 
            #               'quota': 0.0, 
            #               'mtime': <analyzemft.mftutils.WindowsTime instance at 0x1016d2f38>, 
            #               'atime': <analyzemft.mftutils.WindowsTime instance at 0x1016dc050>, 
            #               'crtime': <analyzemft.mftutils.WindowsTime instance at 0x1016d2dd0>, 
            #               'maxver': 0, 
            #               'ver': 0, 
            #               'ctime': <analyzemft.mftutils.WindowsTime instance at 0x1016d2cf8>, 
            #               'class_id': 0, 
            #               'usn': 0.0, 
            #               'dos': 6, 
            #               'own_id': 0
    ...
    if opts.debug or opts.verbose > 2:
        print >> sys.stderr, '-->Record number: %d\n\tMagic: %s Attribute offset: %d Flags: %s Size:%d' % (
            record_number,
            record['magic'],
            record['attr_off'],
    ...
    if record['magic'] == 0x44414142:
        if opts.debug or opts.verbose > 2:
            print >> sys.stderr, "BAAD MFT Record"
        record['baad'] = True
        return record

    if record['magic'] != 0x454c4946:
        if opts.debug or opts.verbose > 2:
            print >> sys.stderr, "Corrupt MFT Record"
        record['corrupt'] = True
        return record
    ...
    while read_ptr < 1024:
        ...
        if opts.debug or opts.verbose > 2:
            print >> sys.stderr, "Attribute type: %x Length: %d Res: %x" % (atr_record['type'], atr_record['len'], atr_record['res'])

        if atr_record['type'] == 0x10:  # Standard Information
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "Stardard Information:\n++Type: %s Length: %d Resident: %s Name Len:%d Name Offset: %d" % (
            ...
            si_record = decode_si_attribute(raw_record[read_ptr + atr_record['soff']:], opts.localtz)
            record['si'] = si_record
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "++CRTime: %s\n++MTime: %s\n++ATime: %s\n++EntryTime: %s" % (
            ...
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "Attribute list"
            if atr_record['res'] == 0:
                al_record = decode_attribute_list(raw_record[read_ptr + atr_record['soff']:], record)
                record['al'] = al_record
                if opts.debug or opts.verbose > 2:
                    print >> sys.stderr, "Name: %s" % (al_record['name'])
            else:
                if opts.debug or opts.verbose > 2:
                    print >> sys.stderr, "Non-resident Attribute List?"
                record['al'] = None

        elif atr_record['type'] == 0x30:  # File name
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "File name record"
            fn_record = decode_fn_attribute(raw_record[read_ptr + atr_record['soff']:], opts.localtz, record)
            record['fn', record['fncnt']] = fn_record
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "Name: %s (%d)" % (fn_record['name'], record['fncnt'])
            record['fncnt'] += 1
            if fn_record['crtime'] != 0:
                if opts.debug or opts.verbose > 2:
                    print >> sys.stderr, "\tCRTime: %s MTime: %s ATime: %s EntryTime: %s" % (
            ...
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "Object ID"

        elif atr_record['type'] == 0x50:  # Security descriptor
            record['sd'] = True
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "Security descriptor"
            ...
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "Volume info attribute"
            volume_info_record = decode_volume_info(raw_record[read_ptr + atr_record['soff']:], opts)
            ...
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "Property set"

        elif atr_record['type'] == 0x100:  # Logged utility stream
            record['loggedutility'] = True
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "Logged utility stream"

        else:
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "Found an unknown attribute"

        if atr_record['len'] > 0:
            read_ptr = read_ptr + atr_record['len']
        else:
            if opts.debug or opts.verbose > 2:
                print >> sys.stderr, "ATRrecord->len < 0, exiting loop"
            break
...
def mft_to_csv(record, ret_header, opts):
    ...
    if record['fncnt'] > 0 and 'si' in record:
        filename_buffer = [
            record['filename'],
            opts.date_formatter(record['si']['crtime'].dtstr),
            opts.date_formatter(record['si']['mtime'].dtstr),
            opts.date_formatter(record['si']['atime'].dtstr),
            opts.date_formatter(record['si']['ctime'].dtstr),
            opts.date_formatter(record['fn', 0]['crtime'].dtstr),
            opts.date_formatter(record['fn', 0]['mtime'].dtstr),
            opts.date_formatter(record['fn', 0]['atime'].dtstr),
            opts.date_formatter(record['fn', 0]['ctime'].dtstr),
        ]
    elif 'si' in record:
        filename_buffer = [
            'NoFNRecord',
...
def decode_si_attribute(s, localtz):
    d = {
        'crtime': getUnixTime(struct.unpack("<L", s[:4])[0], struct.unpack("<L", s[4:8])[0]),
        'mtime': getUnixTime(struct.unpack("<L", s[8:12])[0], struct.unpack("<L", s[12:16])[0]),
        'ctime': getUnixTime(struct.unpack("<L", s[16:20])[0], struct.unpack("<L", s[20:24])[0]),
        'atime': getUnixTime(struct.unpack("<L", s[24:28])[0], struct.unpack("<L", s[28:32])[0]),
...
def decode_fn_attribute(s, localtz, _):
    ...
    d = {
        ...
        'crtime': getUnixTime(struct.unpack("<L", s[8:12])[0], struct.unpack("<L", s[12:16])[0]),
        'mtime': getUnixTime(struct.unpack("<L", s[16:20])[0], struct.unpack("<L", s[20:24])[0]),
        'ctime': getUnixTime(struct.unpack("<L", s[24:28])[0], struct.unpack("<L", s[28:32])[0]),
        'atime': getUnixTime(struct.unpack("<L", s[32:36])[0], struct.unpack("<L", s[36:40])[0]),
...
def decode_volume_info(s, opts):
    ...
    if opts.debug or opts.verbose > 2:
        print >> sys.stderr, "+Volume Info"
        print >> sys.stderr, "++F1%d" % d['f1']
        print >> sys.stderr, "++Major Version: %d" % d['maj_ver']
        print >> sys.stderr, "++Minor Version: %d" % d['min_ver']
        print >> sys.stderr, "++Flags: %d" % d['flags']
        print >> sys.stderr, "++F2: %d" % d['f2']
