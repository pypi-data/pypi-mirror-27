...
def get_l_values(modes) :
    nm = modes.shape[0]
    if(nm > 0):
        lval = [int(modes[0,1])]
        for i in range(1,nm):
            if(modes[i,1] != modes[i-1,1]):
                        lval.append(int(modes[i,1]))
    else:
        lval = []
...
class SeismicConstraints:
    ...
    def __init__(self,file='',types=['nu'],matching = 'frequency',data=None):
        self.file = file
        self.types = types
        if (file != ''):
...
def surface_effects(nu,Model,Params,Settings):
	if( Settings['modes'] != None ):
		...
		if(surface_effects != None ): # adding surface effects
