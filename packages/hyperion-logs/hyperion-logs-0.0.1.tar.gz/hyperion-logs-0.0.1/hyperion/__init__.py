...
def get_logger(stream_name='test_stream',log_group='test_group',default_level = 'INFO'):
    """
    ...
    try:
        ...
        handler.create_session(profile_name='093937234853-GR_GG_COF_AWS_CoreAdmin')
        handler.log_group = log_group
        handler.create_log_group()
        handler.stream_name = stream_name
        log = logging.getLogger(__name__)
        log.setLevel(logging_levels[default_level.lower()])
        log.addHandler(handler)
        print('Logger sending logs to {0} inside log group {1}.'.format(stream_name,log_group))
        return log
    except Exception as e:
        print ('unable to create logger because of {}'.format(e))
...
class CloudWatchLogHandler(handler_base_class):
    ...
    def create_session(self,profile_name=None):
        try:
            ...
            self.role = self.sts_client.assume_role(RoleARN='arn:aws:iam::093937234853:role/dms-cloudwatch-logs-role',RoleSession='LogSession')
