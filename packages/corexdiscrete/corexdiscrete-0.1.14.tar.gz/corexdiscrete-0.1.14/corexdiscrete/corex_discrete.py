...
class CorexDiscrete(UnsupervisedLearnerPrimitiveBase[Input, Output, CorexDiscrete_Params, CorexDiscrete_Hyperparams]):  #(Primitive):
    ...
    def _get_feature_names(self):
        return ['CorexDisc_'+ str(i) for i in range(self.n_hidden)]from sklearn import preprocessing
#import primitive
import sys
#sys.path.append('corexdiscrete/')
import corexdiscrete.corexdiscrete.corex as corex_disc
...
class CorexDiscrete(UnsupervisedLearnerPrimitiveBase[Input, Output, CorexDiscrete_Params, CorexDiscrete_HyperParams]):  #(Primitive):
    ...
    def produce(self, *, inputs : Sequence[Input], timeout : float = None, iterations : int = None) -> CallResult[Output]: 
    ...
    def _fit_transform(self, inputs : Sequence[Input], timeout : float = None, iterations : int = None) -> CallResult[Output]:
        self.columns = list(inputs)
        if len(self.columns) == 1:
            return inputs.values
        X_ = inputs[self.columns].values
