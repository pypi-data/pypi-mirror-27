...
#         ax=fig.gca(projection='3d')
        ...
        ax.set_zlabel('EField(N/C)')
        
        fig.colorbar(surf,shrink=0.5,aspect=5)
      

        plt.subplot(132)
        plt.title("x-axis[x co-ordinate(m)], y-axis[y co-ordinate(m)], Plot- Intensity of EF at(x,y)")
#         circle1=plt.Circle((0.5,0.5),0.1,color='r',linewidth=3,facecolor='red')
#         ax.add_patch(circle1)
        
        plt.pcolormesh(xdist, ydist, matrix)
#         ax.add_patch(circle1)
        im=plt.pcolormesh(xdist, ydist, matrix)
        cb=plt.colorbar(im)
        plt.show()
