...
class OlymKeywords(object):
    ...
    def select_frame_by_index(self,locator,index):
        ...
        logger.info("Selecting frame '%s'." % locator)
        if locator.startswith("tag"):
            elements = self.find_element(locator,False,True)
            if index < 0: 
                index = index + elements.length

            element = elements[int(index)]
        else:
            element = self.find_element(locator,True,True)
...
if __name__ == '__main__':
    ...
    test=OlymKeywords().clear_document(dir1)
