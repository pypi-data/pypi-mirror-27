...
def get_tables():
    ...
    def describe_table(connection, tableName):
        ...
        df = pd.read_sql('describe ' + tableName, connection)[['col_name','data_type']]
        partition_info = []
        try:
            # drop partitionning information
            partition_information_row = df.loc[df['col_name'] == '# Partition Information'].index[0]
            partition_information_rows = df.iloc[partition_information_row:]
            partition_info = list(map(lambda x : x[0],partition_information_rows.to_dict(orient='split')['data']))[2:]
            app.logger.error('Partition info: ' + str(partition_info))
            df = df.drop(partition_information_rows.index)
        except:
            pass

        json_rows = dict(df.to_dict(orient='split')['data'])

        return {'name':tableName,'columns':json_rows,'partitionColumns':partition_info}
