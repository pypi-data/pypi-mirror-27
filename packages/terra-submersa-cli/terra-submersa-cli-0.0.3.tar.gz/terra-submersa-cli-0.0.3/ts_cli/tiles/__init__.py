...
class CLITiles:
    """
    ts-tiles command.
    each cmd_xyz command, parsing out cmmand line is associated with a exec_cmd, doing the actual execution
    """
    ...
    def exec_list(self, rest_cli, output):
    ...
    def cmd_add(rest_url, image, properties):
        ...
        try:
            rest_cli = RestCli(rest_url)
            out = CLITiles().exec_add(rest_cli, image, properties)
            click.echo(out)
        except (CannotParseJsonException, ResponseError) as e:
            click.echo('ERROR: ' + str(e), err=True)
            sys.exit(1)

    def exec_add_sensys(self, rest_cli, image, txt, id, title, description, copyright):
        params = {}
        if not id is None:
            params['id'] = id
        ...
        return rest_cli.post_json('api/sensys2d',
                                  files={
    ...
    def cmd_add_sensys(rest_url, image, txt, id, title, description, copyright):
        ...
        try:
            rest_cli = RestCli(rest_url)
            resp = CLITiles().exec_add_sensys(rest_cli, image, txt, id, title, description, copyright)
            click.echo(resp)
        except (CannotParseJsonException, ResponseError) as e:
            click.echo('ERROR: ' + str(e), err=True)
            sys.exit(1)

    def exec_remove(self, rest_cli, id):
        return rest_cli.delete_json('api/system/' + id)
    ...
    def cmd_delete(rest_url, id):
        ...
        try:
            rest_cli = RestCli(rest_url)
            resp = CLITiles().exec_remove(rest_cli, id)
            click.echo(resp)
        except (CannotParseJsonException, ResponseError) as e:
            click.echo('ERROR: ' + str(e), err=True)
            sys.exit(1)

    def exec_zip(self, rest_cli, id, output):
        return rest_cli.get_file('zip/system/' + id, output)
    ...
    def cmd_zip(rest_url, id, output):
        ...
        try:
            rest_cli = RestCli(rest_url)
            resp = CLITiles().exec_zip(rest_cli, id, output)
            click.echo(resp)
        except (CannotParseJsonException, ResponseError) as e:
            click.echo('ERROR: ' + str(e), err=True)
            sys.exit(1)

    def exec_unzip(self, rest_cli, input):
        return rest_cli.post_json('unzip/system', files={'archive': open(input, 'rb')})
    ...
    def cmd_unzip(rest_url, input):
        ...
        try:
            rest_cli = RestCli(rest_url)
            resp = CLITiles().exec_unzip(rest_cli, input)
            click.echo(resp)
        except (CannotParseJsonException, ResponseError) as e:
            click.echo('ERROR: ' + str(e), err=True)
            sys.exit(1)
