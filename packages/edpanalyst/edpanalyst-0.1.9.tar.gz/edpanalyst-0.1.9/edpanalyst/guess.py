...
def _guess_given_vals(col_name, data):
    ...
    if all(_is_like_list_of_numbers(x) for x in data.dropna()):
...
def _guess_real_stat_type_and_reason(data):
    ...
    """Guesses either realAdditive or realMultiplicative for `data`."""
    ...
    reason = 'uniform cor. %.3f, log-uniform cor. %.3f' % (quantiles_cor,
                                                           log_quantiles_cor)
    if log_quantiles_cor > quantiles_cor and quantiles_cor < 0.95:
...
def _most_likely_sequence_def(col_name, data):
    # type: (str, pd.Series) -> ColumnDefinition
    ...
    length = max(len(x) for x in non_null)
    if length == 0:
        return ColumnDefinition(col_name, 'void', 'All sequences are empty.')

    # Make a set of all the first non-None elements of the sequences.
    nulls_dropped = [[e for e in seq if e is not None] for seq in non_null]
    first_elements = set(seq[0] for seq in nulls_dropped if len(seq) > 0)
    ...
    # an array with every non-missing number in any sequence in `data`
    nums = pd.Series((e for seq in nulls_dropped for e in seq),
                     dtype=np.float64)
