...
STAT_TYPES = [
    ...
    'proportion', 'magnitude', 'sequence', 'date', 'void'
]
    ...
    def __init__(
        ...
        if stat_type == 'sequence':
            ...
            if self._transform not in ('identity', 'log'):
                raise SchemaValueError('%r is not a valid transform' %
                                       (transform,))
        else:
            if transform:
                raise SchemaValueError(
                    '`transform` is only valid for sequences.')
            if length:
                raise SchemaValueError('`length` is only valid for sequences.')
