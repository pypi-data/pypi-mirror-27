...
def _date_to_days(v, col_name):  # type: (Any, str) -> int
    """Converts a date-like object, `v`, to days since the epoch."""
    if (not isinstance(v, pd.Timestamp) and
...
class Session(object):
    ...
    def popmod(self, pmid):  # type: (str) -> population_model.PopulationModel
        """Returns the PopulationModel corresponding to `pmid`."""
        try:
            return population_model.PopulationModel(pmid, self._client)
        except NoSuchGeneratorError:
...
class Population(object):
    ...
    @property
    def models(self):  # type: () -> List[population_model.PopulationModel]
        return [
            population_model.PopulationModel(model['id'], self._client)
    ...
    def _find_latest(self, models):
        ...
        if len(built) > 0:
            ...
            return population_model.PopulationModel(latest['id'], self._client)
        else:
            return None
    ...
    def latest(self,
               wait=70):  # type: (int) -> population_model.PopulationModel
    ...
    def build_model(
        ...
        return population_model.PopulationModel(resp.json()['id'],
                                                self._client)
