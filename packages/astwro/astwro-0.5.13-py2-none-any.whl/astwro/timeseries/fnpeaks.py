...
from astwro.pydaophot import Runner
...
class FNpeaks(Runner):
    ...
    def __init__(self, dir=None, lc=None, start_freq=0.0, end_freq=40.0, step_freq=0.0001, periodogram=False):
        # type: ([str,object], [str], float, float, float, bool) -> FNpeaks
        """
        :param [str] dir:           pathname or TmpDir object - working directory,
                                    if None temp dir will be used and deleted on destructor
        :param [str] lc:            2D Nx2 array-like with light curve. Row of array: (julian day, magnitude)
                                    or filename of at least two column text file with the same data
        :param float start_freq:    start frequency to scan (1/day)
        :param float end_freq:      end frequency to scan (1/day)
        :param float step_freq:     frequency step
        :param bool periodogram:    whether to generate periodogram data
        """
    ...
    def ALlstar(self, image_file=None, psf_file='i.psf', stars='i.ap', profile_photometry_file='i.als',
