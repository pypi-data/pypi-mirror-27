...
def requestCube(ip_connect, name_bbdd, name_cube, col_cube, med_cube, request_data, ifDate, dimension_filters, database_cube_dict, total_data=False, where_tupla=('Date', 'Id', 'Id'), request_date_previous=False, complete_name=False, format_cube=False, n_c=[], time_zone="", redis=""):
    ...
    date_to_balance = request_data['var_settings']['CUBE_CHANGE_DATE'] if 'CUBE_CHANGE_DATE' in request_data['var_settings'] else None
    message_channel = request_data['var_settings']['USER_MESSAGE_CHANNEL'] if 'USER_MESSAGE_CHANNEL' in request_data['var_settings'] else None
    cube = CUBE(limit_sim_exe=limit_sim_exe, limit_time=limit_time, version=request_data['var_settings']['VERSION'], date_to_balance=date_to_balance, message_channel=message_channel)
    mdx_class = MDX()
    ...
    if ifDate:
        if not request_date_previous:
            where[where_tupla] = (request_data['from_Date'], request_data['to_Date'])
        else:
            where[where_tupla] = (request_data['from_Date_previous'], request_data['to_Date_previous'])
...
def requestCubeFilters(ip_connect, name_bbdd, name_cube, col_cube, med_cube, request_data, ifDate, dimension_filters, database_cube_dict=False, where_tupla=('Date', 'Id', 'Id'), n_c=[], time_zone="", redis=""):
    ...
    if request_data[request_data['types']] != 'NULL':
        ...
        exclude_rows = {}
    if ifDate:
        where[('Date', 'Id', 'Id')] = (request_data['from_Date'], request_data['to_Date'])
