...
def create_docker_image(score_file, dependencies, service_name, verb, target_runtime, context,
    ...
    try:
        ...
        if target_runtime is None:
            raise CLIError('Target runtime must be provided. Valid runtimes are {}'.format(SUPPORTED_RUNTIMES.keys()))
        
        is_known_runtime = target_runtime in SUPPORTED_RUNTIMES.keys() or target_runtime in NINJA_RUNTIMES
        if not is_known_runtime:
            raise CLIError('{} is not a valid runtime. Valid runtimes are: {}'.format(target_runtime, SUPPORTED_RUNTIMES.keys()))
