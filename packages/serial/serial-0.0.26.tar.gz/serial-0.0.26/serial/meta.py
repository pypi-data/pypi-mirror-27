...
import serial
from serial.utilities import qualified_name, properties_values, UNIDENTIFIED
...
def pointer(o, p=UNIDENTIFIED):
    # type: (serial.model.Model, Optional[str]) -> Optional[str]
    if p is not UNIDENTIFIED:
        if not isinstance(p, str):
            raise TypeError(
                '`xp` must be a `str`, not %s.' % repr(p)
            )
        o._pointer = p
    return o._pointer


def url(o, u=UNIDENTIFIED):
    # type: (serial.model.Model, Optional[str]) -> Optional[str]
    if u is not UNIDENTIFIED:
        if not isinstance(u, str):
            raise TypeError(
                '`xp` must be a `str`, not %s.' % repr(u)
            )
        o._url = u
    return o._url
