...
class SleepManager(object):
    ...
    def myfunc(self):
        with SleepManager(sleep_time, self, interval=1, notify=lambda x: x.alive) as sm:
            ...
            ....
    """
    def __init__(self, sleep_time, instance, interval=1, notify=lambda x: x.alive, immediately=False):
        """
    ...
    def __enter__(self):
        while not self.immediately and time.time() - self.enter_time < self.sleep_time and self.notify(self.instance):
...
class ExceptContext(object):
    ...
    """
    def __init__(self, exception, func_name=None,
                 errback=lambda func_name, *args: traceback.print_exception(*args),
                 finalback=lambda:None):
        """
