...
def do_backup(cfg: config.Configuration, db: database.Database) -> None:
   ...
   n_threads= 20
   deletion_tracker= db.get_deletion_tracker()
...
def do_one_file(direntry, run_name, archivedirs_to_use, cfg, db, index_file):
   """
...
def do_one_file_no_retry(run_name, path, stat, cfg, db, archivedirs_to_use, index_file):
         ...
         with synchronisation.global_data_lock(checksum):
            if db.have_checksum(checksum):
               ...
               with archivedirs_to_use.get_returnable() as archive_dir:
                  backup_path= cfg.backup_path_abs(archive_dir, checksum)
