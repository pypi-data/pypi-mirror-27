    ...
    def __init__(self, span, levels=None, rd=None, modifier=False, **rd_args):
        ...
        if rd is not None:
            ...
            for x in ["seasons", "quarters"]:
                if x in rd_args:
                    del rd_args[x]
            if isinstance(rd_args.get("days"), float):
                # currently only halfs are supported
                rd_args["hours"] = rd_args.get("hours", 0) + 12
                rd_args["days"] = int(rd_args["days"])
                self.levels.add(Units.HOUR)
            if isinstance(rd_args.get("months"), float):
                # currently only halfs are supported
                rd_args["days"] = rd_args.get("days", 0) + 15
                rd_args["months"] = int(rd_args["months"])
                self.levels.add(Units.DAY)
            if isinstance(rd_args.get("years"), float):
                rd_args["days"] = rd_args.get("days", 0) + 182
