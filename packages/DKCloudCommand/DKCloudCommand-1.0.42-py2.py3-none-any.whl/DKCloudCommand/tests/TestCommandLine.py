...
class TestCommandLine(BaseTestCloud):
    ...
    def test_user_info(self):
        ...
        while index < len(splitted_output):
            if stage == 1:
                if 'Name:' in splitted_output[index] and '+dk@datakitchen.io' in splitted_output[index]: stage += 1
                index += 1
                continue
            if stage == 2:
                if 'Email:' in splitted_output[index] and '+dk@datakitchen.io' in splitted_output[index]: stage += 1
                index += 1
                continue
            if stage == 3:
    ...
    def test_config_list(self):
        ...
        while index < len(splitted_output):
            if stage == 1:
                ...
                continue
            if stage == 2:
                if 'Username:' in splitted_output[index] and '+dk@datakitchen.io' in splitted_output[index]: stage += 1
                index += 1
                continue
            if stage == 3:
                if 'Password:' in splitted_output[index]: stage += 1
                index += 1
                continue
            if stage == 4:
    ...
    def test_create_recipe(self):
        ...
        test_kitchen = self._add_my_guid(test_kitchen)
        recipe_name = 'unit-test-my-recipe-%05d-' % random.randint(0, 10000)
        runner = CliRunner()
        ...
        DKFileUtils.write_file(file_path, contents.replace('[YOUR EMAIL HERE]', 'alex+dk@datakitchen.io'))
        contents = DKFileUtils.read_file(file_path)
        self.assertTrue('alex+dk@datakitchen.io' in contents)
        ...
        self.assertTrue(0 == result.exit_code)
        self.assertTrue('succeeded' in result.output)
        self.assertTrue('alex+dk@datakitchen.io' in result.output)
        ...
        contents = DKFileUtils.read_file(file_path)
        DKFileUtils.write_file(file_path, contents.replace('alex+dk@datakitchen.io', 'blah+dk@datakitchen.io'))
        contents = DKFileUtils.read_file(file_path)
        self.assertTrue('blah+dk@datakitchen.io' in contents)
        self.assertTrue('[YOUR EMAIL HERE]' not in contents)
        self.assertTrue('alex+dk@datakitchen.io' not in contents)
        ...
        contents = DKFileUtils.read_file(file_path)
        self.assertTrue('blah+dk@datakitchen.io' not in contents)
        self.assertTrue('[YOUR EMAIL HERE]' not in contents)
        self.assertTrue('alex+dk@datakitchen.io' in contents)
        ...
        file_path = os.path.join(kitchen_dir, file_name)
        contents = DKFileUtils.read_file(file_path)
        ...
        self.assertTrue(0 == result.exit_code)
        self.assertTrue('Updating the settings of Kitchen' in result.output)
        self.assertTrue('succeeded' in result.output)
