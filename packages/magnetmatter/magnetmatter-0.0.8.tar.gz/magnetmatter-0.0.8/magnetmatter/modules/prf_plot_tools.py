...
def extract_out_file():
    """ returns the refined parameter from FP .out file as a dictionary
    keys:
        FP notation of refined parameters
    values:
        value and corresponding error.
    """
...
def calculate_size(wavelength = "", phases = "", refined_par = ""):
    """ returns a dict containing ab- and c-size for each phase calculated from Y-cos and L-size.

    requires two dictionaries:
        first:
            key = index, value = string of phasename
        second:
            key = refined par, value = tuple(value, error)
    returns:
...
def find_significant_figure(string):
    """ returns index for the decimal.Decimal module for significant figures """
    try:
        string = str(string)
        if string[0] == "-":
            string = string[1:]
...
@TraceCalls()
def prepare_refinement_result(phases = "", refined_par = "", size_info = "", frac_info = "", separation = "", spacegroups = ""):
    """
    ...
    inputs:
        phases:
            dict with phasenames
        refined_par:
            dict with FullProf named parameters with values and errors
        size_info:
            dict with calculated ab and c crystallite sizes from Lorentzian line broadening and Lorenzian anisotropic parameter
        frac_info:
            dict with phase fractions, values and errors.
    return:
        string formatted to be printed on canvas, non-phase parameters on the top,
        then each phase is listed with corresponding parameters.
    ...
    for phase_key in phases.keys():
        insert_text[phase_key] = ["{:16.15}{} {}".format(spacegroups[phase_key], separation, phases[phase_key])]

    """ subfunction to prepare text for insertion """
    def insert_text2(newline, name, separation, value, error):
        ...
        inputs:
            ...
            separation: old parameter that was used for interactive plotting mode to separate name and value. Does interactive mode does probably not work any longer
        ...
        returns:
            ...
            e.g. value = 8.357975 and error = 0.00049791916 for refined parameter Cell_A_ph1_pat is shown as
        ...
        if (name == "R wp / chi2"):
            return "{}{:16.15}{} {:3.3} / {:3.3}".format(newline, name, separation, value, error )

        string = round2significant_error(value, error, 13) # 13 corresponce to showing eight characters of str(value).
        result = "{}{:16.15}{} {:14.13}{}".format(newline, name, separation, string, unit)
        return result
    ...
    for key, (value, error) in refined_par.items():
        ...
        if not phase_pattern:
            if key == "R wp / chi2":
                insert_text["0"].append( insert_text2("",key, separation, value, error) )
            else:
                insert_text["0"].append( insert_text2("\n",key, separation, value, error) )
            continue
