...
def install(pi=None):
    ...
    if stdsocket._realsocket is socket:
        raise Exception("Still installed")
            ...
            if isinstance(referrer1, types.MethodType):
                for referrer2 in gc.get_referrers(referrer1):
                    if isinstance(referrer2, _socketobject_new):
                        setattr(referrer2, methodName, types.MethodType(newClassMethod, referrer1.__self__, referrer1.__self__.__class__))
...
class _fakesocket(asyncore_dispatcher):
    ...
    def __init__(self, realSocket):
        ...
        if not isinstance(realSocket, _realsocket_old):
            raise Exception("An invalid socket passed to fakesocket %s" % realSocket.__class__)
...
if False:
    def dump_socket_stack_traces():
        import traceback
        for skt in list(asyncore.socket_map.values()):
            for k, v in list(skt.__dict__.items()):
...
if __name__ == '__main__':
    ...
    def TestMonkeyPatchUrllib(uri):
        ...
        install()
        try:
            import urllib.request, urllib.parse, urllib.error  # must occur after monkey-patching!
