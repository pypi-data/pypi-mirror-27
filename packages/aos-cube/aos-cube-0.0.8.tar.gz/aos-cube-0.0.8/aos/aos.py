...
class Repo(object):
    ...
    def fromurl(cls, url, path=None):
        ...
        if m_local:
            ...
            name = os.path.basename(path or m_repo_url.group(2)[:-4])
            repo.name = name if name != 'AliOS-Things' else '.' + name
            repo.path = os.path.abspath(path or os.path.join(os.getcwd(), repo.name))
...
def copy_directory(src_dir, dst_dir):
    if not os.path.exists(src_dir) or os.path.exists(dst_dir): return
    try:
...
def get_target_code(project_path, os_code_path, target):
    pd = Program(project_path)
    
    with cd(os_code_path):
        _run_make(['-f build/Makefile', target, '>' + os.path.join(project_path, '.logcat.txt')])
    
    #copyp os default dir
    for d in default_dir_list:
        src = os.path.join(os_code_path, d)
    ...
    with open(os.path.join(os_code_path, 'out', target, 'config.mk'), 'r') as f:
        for line in f:
    ...
    for sm in sdk_makefiles:
        sm = sm.strip()
        if sm:
            copy_dir_dict[os.path.abspath(os.path.dirname(os.path.join(os_code_path, sm)))] = os.path.abspath(os.path.dirname(os.path.join(project_path, 'os', sm)))
    ...
    for sp in sdk_prebuild_libs:
        sp = sp.strip()
        if sp:
            copy_dir_dict[os.path.abspath(os.path.dirname(os.path.join(os_code_path, sp)))] = os.path.abspath(os.path.dirname(os.path.join(project_path, 'os', sp)))
    ...
    for si in sdk_includes:
        si = si.strip()
        if si:
            copy_dir_dict[os.path.abspath(os.path.join(os_code_path, si))] = os.path.abspath(os.path.join(project_path, 'os', si))

    #all component dir
    components = config_dict['COMPONENTS'].split(' ')
    for co in components:
        #component dir
        src_dir = os.path.abspath(os.path.join(os_code_path, config_dict[co + '_LOCATION']))
    ...
    app = config_dict['APP']
    copy_dir_dict[os.path.abspath(os.path.join(os_code_path, config_dict[app + '_LOCATION']))] = os.path.abspath(os.path.join(project_path, app))
...
def new(target, path='.', scm='git', program=False, component=False, aoslib=False, create_only=False, depth=None, protocol=None):
    ...
    action("Creating new %s \"%s\" (%s)" % (d_type, target.split('@')[0], scm))
    p = Program(d_path)
    if d_type == 'program':
        ...
        p.path = cwd_root
        p.set_root()
        p.set_cfg('os_code_path', os.path.join(p.path, '.AliOS-Things'))
        d = ''
        ...
        if d == 'aos':
           get_target_code(d_path, p.get_cfg('os_code_path'), target)
...
def make():
    ...
    if os.path.isfile(os.path.join(os.getcwd(), Cfg.file)):
        ...
        with cd(os_path):
            _run_make(['-f build/Makefile', make_args, app_path, out_path])   
    else:
        #aos source code
