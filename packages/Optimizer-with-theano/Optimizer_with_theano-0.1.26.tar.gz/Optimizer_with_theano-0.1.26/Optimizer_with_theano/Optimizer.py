...
from .Modify import Flatten, Reshape, Batchnorm
...
from .CapsNet import Capsule_layer
...
class optimizer:
    ...
    def set_data(self, 
                 ...
                 input_shape=None,
                 is_shuffle=True):
        if type(x_arr) == tuple or type(x_arr) == list:
            x_arr = np.zeros(x_arr).astype(theano.config.floatX)
        else:
            x_arr = x_arr.astype(theano.config.floatX)
            
        if type(y_arr) == tuple or type(y_arr) == list:
            y_arr = np.zeros(y_arr).astype(theano.config.floatX)
        else:
...
#        self.n_data = self.n_row
    ...
    def dense(self, 
        ...
        layer = Dense_layer(obj, 
                            ...
                            activation=act,
                            is_train=is_train,
                            func_to_variable=func_to_variable,
                            name=name
                           )
        obj   = layer.update()
        return obj
    ...
    def CapsNet(self, 
        ...
        layer = Capsule_layer(obj, 
                             ...
                             func_to_variable=func_to_variable,
                             name=name,
                             num_vector1=num_vector1,
                             num_vector2=num_vector2,
...
#        return obj
    ...
    def loss_binary_cross_entropy(self, axis=None):
        obj = self.copy()
        obj = binary_cross_entropy(obj, axis)
        return obj
    ...
    def opt_RMSProp(self, alpha=0.001, gamma=0.9, ep=1e-8, input_grads=[]):
        ...
        obj.updatelst = []
        params = obj.layer_info.get_params() + input_grads
        #rlst = [theano.shared(np.random.rand(*x.get_value().shape).astype(theano.config.floatX)) for x in params]
