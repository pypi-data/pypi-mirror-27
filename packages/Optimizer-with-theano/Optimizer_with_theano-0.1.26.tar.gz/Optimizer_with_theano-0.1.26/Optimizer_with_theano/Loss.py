...
def binary_cross_entropy(obj, axis=None):
    obj.out = T.clip(obj.out, _EPSILON, 1.0 - _EPSILON)
    obj.loss =  nnet.binary_crossentropy(obj.out, obj.y).mean(axis)
    
    # classification accuracy
    diff = obj.y - obj.out
    #obj.train_acc = (T.eq(obj.out,obj.y).sum().astype(theano.config.floatX) / obj.n_batch)
    #obj.valid_acc = (T.eq(obj.out,obj.y).sum().astype(theano.config.floatX) / obj.x_test_arr.shape[0])
    obj.train_acc = 1 - ((diff**2).sum() / ((obj.y.flatten() - obj.y.mean())**2).sum(axis))
    obj.valid_acc = 1 - ((diff**2).sum() / ((obj.y.flatten() - obj.y.mean())**2).sum(axis))
