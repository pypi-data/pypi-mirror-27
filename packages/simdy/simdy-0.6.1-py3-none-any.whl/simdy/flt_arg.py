...
class Float32Arg(Argument):
    def __init__(self, name=None, value=float32()):
        super(Float32Arg, self).__init__(name)
        if not isinstance(value, float32):
            raise TypeError("float32 type expected got", value)
        self._value = value
    ...
    def can_operate_with_memory(self, cgen, arg, op):
        return isinstance(arg, Float32Arg) and op in ('+', '-', '/', '*', '>', '<', '==', '<=', '>=', '!=')
    ...
    def cond_with_mem(self, cgen, reg1, op, arg, jump_label, satisfied):
        cons = self._cond_operators(satisfied)
        if cgen.cpu.AVX or cgen.cpu.AVX512F:
            code = 'vucomiss %s, dword [%s]\n' % (reg1, arg.name)
        else:
            code = 'ucomiss %s, dword[%s]\n' % (reg1, arg.name)
        code += "%s %s\n" % (cons[op], jump_label)
        return code

    def cond_with_const(self, cgen, reg1, value, op, jump_label, satisfied):
        arg = Float32Arg(value=float32(value))
    ...
    def arith_with_memory(self, cgen, reg, op, arg):
        ...
        code = cgen.gen.arith_f32(reg, op, name=arg.name, dst_reg=xmm3)
        dest_reg = reg if xmm3 is None else xmm3
        return code, dest_reg, Float32Arg

    def arith_with_arg(self, cgen, reg, arg2, reg2, op):
        if cgen.can_destruct(reg):
            code = cgen.gen.arith_f32(reg, op, xmm2=reg2)
            dest_reg = reg
        elif cgen.can_destruct(reg2) and op in ('+', '*'):
            code = cgen.gen.arith_f32(reg2, op, xmm2=reg)
            dest_reg = reg2
        else:
            xmm3 = cgen.register('xmm')
            code = cgen.gen.arith_f32(reg, op, xmm2=reg2, dst_reg=xmm3)
            dest_reg = xmm3
        return code, dest_reg, Float32Arg
    ...
    def fma_cmd(self, cgen, arg2, op, reverse, arg3, reg1, reg2, reg3=None):
        ...
        if cgen.cpu.FMA:
            ...
            if cgen.can_destruct(reg1):
                code = cgen.gen.fma_f32_213(reg1, reg2, op, negate, xmm3=reg3, name=arg3.name)
                dest_reg = reg1
            elif cgen.can_destruct(reg2):
                code = cgen.gen.fma_f32_213(reg2, reg1, op, negate, xmm3=reg3, name=arg3.name)
                dest_reg = reg2
            elif reg3 is not None and cgen.can_destruct(reg3):
                code = cgen.gen.fma_f32_231(reg3, reg1, op, negate, xmm3=reg2)
                dest_reg = reg3
            else:
                xmm4 = cgen.register('xmm')
                code = cgen.gen.move_reg(xmm4, reg1)
                code += cgen.gen.fma_f32_213(xmm4, reg2, op, negate, xmm3=reg3, name=arg3.name)
                dest_reg = xmm4
        else:
    ...
    def set_array_item(self, addr, value):
        if not isinstance(value, float):
            raise TypeError("Expected %s got %s" % (float32, type(value)))
        x86.SetFloat(addr, value, 0)

    def get_array_item(self, addr):
        return float32(x86.GetFloat(addr, 0, 0))
    ...
    def load_item_from_array(self, cgen, ptr_reg):
        xmm = cgen.register('xmm')
        code = cgen.gen.load_f32(xmm, ptr_reg=ptr_reg)
        return code, xmm, Float32Arg

    def store_item_to_array(self, cgen, ptr_reg, xmm):
        return cgen.gen.store_f32(xmm, ptr_reg=ptr_reg)


float32.arg_class = Float32Arg

register_arg_factory(Float32Arg, lambda: Float32Arg())
