...
class EChartsPlugin(BaseAdminPlugin):
    ...
    def block_results_top(self, context, nodes):
        ...
        nodes.append(loader.render_to_string('xadmin/blocks/model_list.results_top.echarts.html',
...
class EChartsView(ListAdminView):
    ...
    def get_echarts_datas(self, echarts_type=FLOT_TYPE_DEFAULT):
        ...
        if echarts_type == ECHARTS_TYPE_LINE:
            echart = Line(title=title, subtitle=subtitle, **x_option)
            ...
            echart = Bar(title=title, subtitle=subtitle, **x_option)
            for i, yfname in enumerate(self.y_fields):
                e_label = force_unicode(label_for_field(yfname, self.model, model_admin=self))
                e_attrs = []
                e_values = []
            ...
            echart = Pie(title=title, subtitle=subtitle, **x_option)
            for i, yfname in enumerate(self.y_fields):
                e_label = force_unicode(label_for_field(yfname, self.model, model_admin=self))
                e_attrs = []
                e_values = []
            ...
            echart = Liquid(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_LINE3D:
            echart = Line3D(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_GAUGE:
            echart = Gauge(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_GEO:
            echart = Geo(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_GRAPH:
            echart = Graph(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_RADAR:
            echart = Radar(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_SCATTER:
            echart = Scatter(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_WORDCLOUD:
            echart = WordCloud(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_FUNNEL:
            echart = Funnel(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_MAP:
            echart = Map(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_PARALLEL:
            echart = Parallel(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_POLAR:
            echart = Polar(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_HEATMAP:
            echart = HeatMap(title=title, subtitle=subtitle, **x_option)

        elif echarts_type == ECHARTS_TYPE_TREEMAP:
            echart = TreeMap(title=title, subtitle=subtitle, **x_option)
