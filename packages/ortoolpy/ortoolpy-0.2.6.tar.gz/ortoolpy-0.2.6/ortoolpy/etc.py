...
def tsp(nodes, dist=None):
    ...
    for _,r in a.query('NodeI!=0 & NodeJ!=0').iterrows():
        i,j,vij,vji = r[['NodeI','NodeJ','VarIJ','VarJI']]
        m += u[i]+1 -(n-1)*(1-vij) + (n-3)*vji <= u[j] #持ち上げポテンシャル制約(MTZ)
    for _,r in a.query('NodeI==0').iterrows():
        j,v0j,vj0 = r[['NodeJ','VarIJ','VarJI']]
        m += 1+(1-v0j) +(n-3)*vj0 <= u[j]  #持ち上げ下界制約
    for _,r in a.query('NodeJ==0').iterrows():
        i,vi0,v0i = r[['NodeI','VarIJ','VarJI']]
        m += u[i] <=(n-1)-(1-vi0)-(n-3)*v0i #持ち上げ上界制約
    m.solve()
    a['ValIJ'] = a.VarIJ.apply(value)
    a['ValJI'] = a.VarJI.apply(value)
    dc = dict(np.r_[a[a.ValIJ>0.5][['NodeI','NodeJ']].values,
                    a[a.ValJI>0.5][['NodeJ','NodeI']].values])
