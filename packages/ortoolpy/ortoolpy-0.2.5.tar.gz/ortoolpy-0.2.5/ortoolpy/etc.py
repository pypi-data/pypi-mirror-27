...
def tsp2(pos):
    ...
    a = pd.DataFrame([(i,j,np.linalg.norm(pos[i]-pos[j]))
        for i in range(n) for j in range(i+1,n)], columns=['Node1','Node2','Dist'])
    a['Var1to2'] = addbinvars(len(a))
    a['Var2to1'] = addbinvars(len(a))
    m = LpProblem()
    u = [0]+addvars(n-1)
    m += lpDot(a.Dist, a.Var1to2+a.Var2to1)
    g1 = a.groupby('Node1').groups
    g2 = a.groupby('Node2').groups
    for i in range(n):
        idx1,idx2 = g1.get(i,[]),g2.get(i,[])
        m += lpSum(chain(a.Var1to2[idx1], a.Var2to1[idx2])) == 1
        m += lpSum(chain(a.Var1to2[idx2], a.Var2to1[idx1])) == 1
    for _,r in a[a.Node1>0].iterrows(): # 持ち上げポテンシャル制約
        m += u[r.Node1]+1 - (n-1)*(1-r.Var1to2) + (n-3)*r.Var2to1 <= u[r.Node2]
    for _,r in a[a.Node1==0].iterrows():
        m += 1 + (1-r.Var1to2) + (n-3)*r.Var2to1 <= u[r.Node2] # 持ち上げ下界制約
        m += u[r.Node2] <= n-1 + (1-r.Var2to1) + (n-3)*r.Var1to2 # 持ち上げ上界制約
    m.solve()
    a['Val1to2'] = a.Var1to2.apply(value)
    a['Val2to1'] = a.Var2to1.apply(value)
    dc = dict(np.r_[a[a.Val1to2>0.5][['Node1','Node2']].values,
                    a[a.Val2to1>0.5][['Node2','Node1']].values])
