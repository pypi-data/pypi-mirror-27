...
class CMFModel:
    ...
    def create_boundary_conditions(self, cmf_project):
        ...
        # Helper functions
        def set_inlet(boundary_condition_):

            # Get the correct cell and layer
            if int(boundary_condition_['layer']) == 0:
                cell_layer = cmf_project.cells[int(boundary_condition_['cell'])].surfacewater
            else:
                cell_layer = cmf_project.cells[int(boundary_condition_['cell'])].layers[int(boundary_condition_['layer'])]
            ...
            if len(boundary_condition_['flux']) > 1:
                inlet.flux = cmf.timeseries(begin=datetime(2017, 1, 1), step=timedelta(hours=1))
                for in_flux in boundary_condition_['flux']:
                    inlet.flux.add(float(in_flux))
            else:
                inlet.flux = boundary_condition_['flux']

        def set_outlet(boundary_condition_, index):
            x, y, z = boundary_condition_['location'].split(' ')
            outlet = cmf_project.NewOutlet('outlet_' + str(index), float(x), float(y), float(z))
            cell = cmf_project.cells[int(boundary_condition_['cell'])]
            ...
            if boundary_condition_['layer'] == 'all':
                for l in cell.layers:
                    ...
                    cmf.Darcy(l, outlet, FlowWidth=boundary_condition_['flow_width'])
                cmf.KinematicSurfaceRunoff(cell.surfacewater, outlet, boundary_condition_['flow_width'])
            elif boundary_condition_['layer'] == 0:
                cmf.KinematicSurfaceRunoff(cell.surfacewater, outlet, boundary_condition_['flow_width'])
            else:
                layer = cell.layers[int(boundary_condition_['layer'])]
                cmf.Darcy(layer, outlet, FlowWidth=boundary_condition_['flow_width'])

        def set_boundary_condition(boundary_condition, bc_index):
            if boundary_condition['type'] == 'inlet':
                set_inlet(boundary_condition)
            elif boundary_condition['type'] == 'outlet':
                set_outlet(boundary_condition, bc_index)
            else:
        ...
        for boundary_condition_index in range(0, len(self.boundary_dict)):
            set_boundary_condition(self.boundary_dict[int(boundary_condition_index)], boundary_condition_index)
