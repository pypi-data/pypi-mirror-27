...
class MongoTeacher(Mongo):
    ...
    def create_increment_stage(self):
        categories = self.get_new_categories()
        selected_collection = self.connection["CheckModels"]
        for cat in categories:
            name = cat["name"]
            enabled = cat["enabled"]
            if enabled == 1:
                selected_collection.update_one({},{"$set":{name:0}})
    ...
    def create_bias_models(self):
        ...
        for cat in categories:
            ...
            enabled = cat["enabled"]
            if enabled == 1:
                selected_collection.update_one({},{"$set":{name:1}})
        # result = selected_collection.find_one({})
        # if result is None:
        #
        #     tmp = {
    ...
    def create_bias_stage(self):
        ...
        for cat in categories:
            ...
            enabled = cat["enabled"]
            if enabled == 1:
                selected_collection.update_one({},{"$set":{name:10}})
