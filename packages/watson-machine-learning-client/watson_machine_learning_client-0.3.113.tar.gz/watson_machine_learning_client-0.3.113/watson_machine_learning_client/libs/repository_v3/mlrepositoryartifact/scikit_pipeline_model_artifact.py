...
class ScikitPipelineModelArtifact(ModelArtifact):
    ...
    def __init__(self, scikit_pipeline_model, training_features=None, training_target=None, feature_names=None,
        ...
        if is_xgboost:
            ...
            if self.meta.prop(MetaNames.LABEL_FIELD ) is None:
                self.meta.merge(
                    MetaProps({MetaNames.LABEL_FIELD : "Not Applicable for XGBoost" })
                )
            ...
            if self.meta.prop(MetaNames.LABEL_FIELD ) is None:
                self.meta.merge(
                    MetaProps({MetaNames.LABEL_FIELD : "Not Applicable for sklearn" })
                  )
