...
class ZoneLayout(object):
    ...
    def addZoneAt(self, location, _bypassSymmetry=False):
        ...
        if not _bypassSymmetry and self.symmetryEnforced:
            if location != ~location:
                self.addZoneAt(~location, _bypassSymmetry=True)
    ...
    def connectZone(self, startLocation, direction, _bypassSymmetry=False):
        '''
        ...
        if not _bypassSymmetry and self.symmetryEnforced:
            self.connectZone(~startLocation, ~direction, _bypassSymmetry=True)
    ...
    def createMapLayout(self, layoutDatabase):
        '''
        ...
        for location in locations:
            ...
            result.addZone(topRow, topCol, startTeamIndex)
...
class MapLayout(object):
    ...
    def addZone(self, topRow, topCol, startTeamIndex):
        '''
        ...
        zone = ZoneDef(len(self.zones), startTeamIndex, x, y)
        self.zones.add(zone)
    ...
    def dumpState(self):
        ...
        for i, zone in enumerate(zones):
            ...
            zoneInfo.append([zone.initialOwnerIndex, topRow, topCol])
    ...
    def fromDumpedState(layoutDatabase, data):
        ...
        for zoneInfo in zoneData:
            initialOwnerIndex, topRow, topCol = zoneInfo
            result.addZone(topRow, topCol, initialOwnerIndex)
