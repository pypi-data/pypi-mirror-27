...
URL_LIVE_STREAM_SESSION_API = "https://my.canary.is/api/watchlive/{}/session"
URL_LIVE_STREAM_SEND_API = "https://my.canary.is/api/watchlive/{}/send"
URL_LIVE_STREAM_API = "https://my.canary.is/api/watchlive/34051/{}/stream.m3u8"
...
class Api:
    ...
    def get_live_stream_url(self, device_uuid):
        sessionId = self._call_api(
            "post",
            URL_LIVE_STREAM_SESSION_API.format(device_uuid),
            json={
        ...
        message = self._call_api(
            "post",
            URL_LIVE_STREAM_SEND_API.format(device_uuid),
            json={
                "sessionId": sessionId
            }).json()["success"]

        return URL_LIVE_STREAM_API.format(sessionId)

    def _call_api(self, method, url, **kwargs):
        _LOGGER.debug("About to call %s with %s", url, kwargs or {})
