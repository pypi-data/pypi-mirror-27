        ...
        def decorator(function):
            def top(function):
                def wrapper(*args,**kwargs):
                    if permissions != None:
                        if (self.permissions.has_key(args[1].message.from_user.id) and permissions < self.permissions[args[1].message.from_user.id]) or (self.permissions.has_key(args[1].message.from_user.id)==False and permissions < self.default_perms):
