...
class VCFMendelCheck(object):
    ...
    pattern_key = ['ok', 'gt_c', 'gt_f', 'gt_m', 'sex']
    sample_key = ['trio']

    Var = namedtuple('Var', var_key)
    Pattern = namedtuple('Pattern', pattern_key)

    def __init__(self, vcf, pedfile, regions=None):
        # VCF
        self._ped = ped = PedFile(pedfile)
        self.trios = list(ped.iter_trios())
        if regions:
        ...
        self.trio_samples = [trio for trio in self.trios if all(sid in sample_set for sid in trio)]
        logging.debug(self.trios)
        logging.debug(sample_set)

    def __iter__(self):
        sample_idxs = self.sample_idxs
        trio_samples = self.trio_samples
        Var = self.Var
        ...
        for row in self._it:
            ...
            for c, f, m in trio_samples:
                ...
                pattern = Pattern(int(ok), dat_c.gt_str, dat_f.gt_str, dat_m.gt_str, sex)
...
def vcf_mendel_check(args):
    """
    """
    checker = VCFMendelCheck(args.vcf, args.pedigree, regions=args.regions)
    logging.info('Trios in ped: %s (%s)', len(checker.trios), args.pedigree)
    ...
    if args.stats:
        ...
        keys = checker.var_key + checker.pattern_key + ['trio']
        print (*keys, sep='\t')
        for v, pat, trio in checker:
            trio = ','.join(trio)
            print (*(v + pat + (trio,)), sep='\t')
