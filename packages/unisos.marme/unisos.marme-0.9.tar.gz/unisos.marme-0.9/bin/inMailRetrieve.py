...
class examples(icm.Iif):
    ...
    def iif(self,
        ...
        icm.iifExampleMenuChapter('*Config File Creation Facility IIFs*')

        iifAction = " -i offlineimaprcUpdate"
        iifArgs = ""
        menuLine = """{iifAction} {iifArgs}""".format(iifAction=iifAction, iifArgs=iifArgs)
        icm.iifExampleMenuItem(menuLine, verbosity='none')
        icm.iifExampleMenuItem(menuLine, verbosity='full')

        iifAction = " -i offlineimaprcStdout" ; iifArgs = ""
        menuLine = """{iifAction} {iifArgs}""".format(iifAction=iifAction, iifArgs=iifArgs)
        icm.iifExampleMenuItem(menuLine, verbosity='none')
        icm.iifExampleMenuItem(menuLine, verbosity='full')
        ...
        icm.iifExampleMenuChapter('*Operation Facility IIFs -- (offlineimapRun)*')

        iifAction = " -i offlineimapRun" ; iifArgs = ""
        menuLine = """{iifAction} {iifArgs}""".format(iifAction=iifAction, iifArgs=iifArgs)
        icm.iifExampleMenuItem(menuLine, verbosity='none')
        icm.iifExampleMenuItem(menuLine, verbosity='full')
...
"""
*  [[elisp:(org-cycle)][| ]]  [[elisp:(blee:ppmm:org-mode-toggle)][Nat]] [[elisp:(beginning-of-buffer)][Top]] [[elisp:(delete-other-windows)][(1)]] || Class-IIF    ::  offlineimapRun    [[elisp:(org-cycle)][| ]]
"""
class offlineimapRun(icm.Iif):
    """."""
...
"""
*  [[elisp:(org-cycle)][| ]]  [[elisp:(blee:ppmm:org-mode-toggle)][Nat]] [[elisp:(beginning-of-buffer)][Top]] [[elisp:(delete-other-windows)][(1)]] || Class-IIF    ::  offlineimaprcUpdate -- Create/Update Config File   [[elisp:(org-cycle)][| ]]
"""    
class offlineimaprcUpdate(icm.Iif):
...
####+END:
        ...
        outcome = offlineimaprcStdout().iif(
            interactive=False,
...
"""
*  [[elisp:(org-cycle)][| ]]  [[elisp:(blee:ppmm:org-mode-toggle)][Nat]] [[elisp:(beginning-of-buffer)][Top]] [[elisp:(delete-other-windows)][(1)]] || Class-IIF    ::  offlineimaprcStdout    [[elisp:(org-cycle)][| ]]
"""
class offlineimaprcStdout(icm.Iif):
    """Show Config File. Applies results of inMailAcctAccessParsGet() to offlineimaprcTemplate()."""

    iifParamsMandatory = []
    iifParamsOptional = ['inMailAcct', 'mboxesList', 'ssl'] 
    iifArgsLen = {'Min': 0, 'Max': 0,}
...
####+END:
        ...
        outcome = marmeAcctsLib.inMailAcctParsGet().iif(
            interactive=False,
            inMailAcct=inMailAcct,
            parTypes=['access'],
        )
