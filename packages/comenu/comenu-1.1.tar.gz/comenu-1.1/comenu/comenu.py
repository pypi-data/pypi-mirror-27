...
def main():
    ...
    # Check arguments
    parser = argparse.ArgumentParser(prog='comenu',
...
def display_help():
    ...
    puts(' crtl+c | q  - Quit comenu')
    puts(' k (up)      - Move your selection up')
    puts(' j (down)    - Move your selection down')
    puts(' h           - Show help menu')
    puts(' c           - Create new command')
    puts(' d           - Delete command')
    puts(' e           - Edit command')
    puts(' + (plus)    - Move command up')
    puts(' - (minus)   - Move command down')
...
def command_create():
    ...
    call(['clear'])
    puts(colored.cyan('Create new command entry'))
    puts('')
    ...
    config['targets'].append({'command': command, 'name': name, 'friendly': friendly, 'options': options})
    ...
    puts('')
    puts('New command added')
    time.sleep(TRANSITION_DELAY_TIME)


def command_edit(selected_target):
    global targets, config_name

    call(['clear'])
    puts(colored.cyan('Editing command %s' % targets[selected_target]['name']))
    puts('')

    target = targets[selected_target]

    while True:
        name = input_prefill('Name: ', target['name'])
        if name is not '':
            break

    friendly = input_prefill('Description: ', target['friendly'])
    command = input_prefill('Command: ',  target['command'])
    ...
    config['targets'].insert(selected_target,
                             {'command': command, 'name': name, 'friendly': friendly, })
...
def command_delete(selected_target):
    global targets, config_name

    call(['clear'])
    puts(colored.red('Delete command entry for %s' % targets[selected_target]['name']))
    puts('')

    while True:
        response = input('Are you sure you want to delete this command [yes|NO]: ').lower()
...
def command_move_up(selected_target):
    global config_name
...
def command_move_down(selected_target):
    global config_name
...
def display_menu():
    ...
    while True:
        ...
        if num_targets <= 0:
            puts(colored.red('Whoops, you don\'t have any commands defined in your config!'))
            puts('')
            puts('Press "c" to create a new command')
        else:
            ...
            for index, target in enumerate(targets):
                if target['friendly']:
                    desc = target['name'].ljust(longest_name) + ' | ' + target['friendly']
                else:
                    desc = target['name'].ljust(longest_name)
                target['desc'] = desc
        ...
        # Check if key is a number
            ...
            args = [command] + target.get('options', []) + [target['name']]
            command_array = command.split(' ')

            command = command_array[0]
            args = command_array
            try:
                 # After this line, shell will replace the python process
            ...
            command_delete(selected_target)

        elif key == 'e' and num_targets > 0:
            command_edit(selected_target)

        elif key == '-' and num_targets > 0:
            if selected_target < num_targets:
                command_move_down(selected_target)
                selected_target += 1

        elif key == '+' and num_targets > 0:
            if selected_target > 0:
                command_move_up(selected_target)
                selected_target -= 1
