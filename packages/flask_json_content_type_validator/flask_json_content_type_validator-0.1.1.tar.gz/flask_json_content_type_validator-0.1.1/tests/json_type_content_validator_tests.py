...
class JsonTypeContentValidatorTests(unittest.TestCase):
    ...
    def test_dummy_resource_should_return_data_given_content_type_contains_more_data_then_just_application_json(self):
        # given
        url = '/echo-resource'
        data = dict(key='value')
        ...
        # when
        result = self.app.get(
            url, data=json.dumps(data), headers={'Content-Type':
                                                 'application/json;'
                                                 'chartset=utf-8'})
