...
def interceptor(rn_environ, *args, **kwargs):
    ...
    if conf.profile_http_header_enabled:
        text = ''
        items = ['host', 'connection','cache_control', 'user_agent',
                'upgrade_insecure_requests',
                'accept', 'accept_encoding', 'accept_language',
                'cookie']
        for key in items:
            if environ.get('HTTP_{}'.format(key.upper())):
                text += '{}={}\n'.format(key, environ.get('HTTP_{}'.format(key.upper())))
