...
# Deprecated due to HKEX Website Update , Dec 2017
# def getBoardLot():
...
def getIndex_yahoo(index, year=3, begdate='', enddate=''):
    if (enddate == ''):
        enddate = datetime.now()
        begdate = enddate - relativedelta(years=year)
    else:
    ...
    if (index == 'SP500'):
        ...
        print('Err: Unsupported Index')
        return
    df = web.DataReader(series_name, 'yahoo', begdate, enddate)
    df = df[['Adj Close']]
    df.columns = [index]
    return df
...
def getETFinfo(mkt='', processes=10):
    if (mkt == 'US' or mkt == 'us'):
        ...
        #     res.append(df)
        #     info = pd.concat(res)
        print ('HKEX Website Change, Pending Implementation')
        return
    else:
        print('Market must be US or HK')
        return
