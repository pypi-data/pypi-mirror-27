...
def ensemble_learning(work_dir, df, sales_file_proc, ocm_file, time_tag, sale_tag, sales_mapping_file, feature_file, province):
    ...
    if not os.path.exists(time_tag):
        os.mkdir(time_tag)
    df = pd.read_excel(sales_mapping_file)
...
def data_visualization(work_dir, time_tag, sale_tag):
    ...
    plt.close()
    # 箱线图-散点图解释商圈需要调整的原因
    adjustment_interpreter.interpreter(time_tag, sale_tag)
...
def main(work_dir, province, sales_file_ori, sales_file_proc, ocm_file, feature_file):
    ...
    for sale_tag in ['整体平均千箱', '高价平均千箱']:
        tag = "%s_" % sale_tag + time_tag
        df = pd.DataFrame({sale_tag: sale_data[sale_tag]})
        sales_mapping_file = "%s商圈OCM_%s.xlsx" % (province, sale_tag)
        if not os.path.exists(sales_mapping_file):
            ...
            bsp.sum_mutil_dealer(sales_file_ori)
        ...
        data_visualization(work_dir, tag, sale_tag)
        AnalyzeGrading.analyze_grading(tag, '%s_综合.xlsx' % tag, '客户形态对应的商圈等级', '预估销量', '整体预估销量')
