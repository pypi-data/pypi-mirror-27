...
def interpreter(time_tag, sale_tag, province="湖北"):
    ...
    try:
        df_grading = pd.read_excel("%s各商圈总销量.xlsx" % province, usecols=[0, 1], index_col=0)
    ...
    df_ocm = pd.read_excel('湖北商圈OCM_%s.xlsx' % sale_tag, index_col=0)
    df_ocm.fillna(0, inplace=True) ;df_ocm.to_excel('tsss.xlsx')
    ...
    for index, i in enumerate(param):
        ...
        df_maintain_proc2 = df_maintain_proc.loc[:, ['index', '参数', i]]
    ...
    df_maintain_proc.set_index("index", inplace=True)
...
if __name__ == "__main__":
    time_tag = '整体平均千箱_2017-12-04-10h26m'
    sale_tag = '整体平均千箱'
    interpreter(time_tag, sale_tag)
