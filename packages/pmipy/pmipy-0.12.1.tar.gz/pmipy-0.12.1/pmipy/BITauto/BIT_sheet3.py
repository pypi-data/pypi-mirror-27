...
# 提取问卷数据
## 打开问卷信息文件，该数据已经清洗过 
df = pd.read_excel('MR002-170802Y-德克士Brand Image Tracking（W45-W48）（11月）数据v2.xlsx')
Base = len(df) # Base即为行数，也就是问卷数
## 提取问卷题目编号信息文件，如未来题目有变动，需更新此文件信息
df_qa = pd.read_excel('qa-Olivia.xlsx', index_col=0)
## 获取配额/权重
df_weight = pd.read_excel('新配额分布.xlsx', index_col=0)
...
# 提取QB6
def extract_data3(qtag, qtag_list):
    # 多选题，将各选项纵向叠加
    for index, i in enumerate(qtag_list):
        df1 = df.loc[:, list_essential1]
        try:
    ...
    # 使用透视表转换数据格式, 注意，其他就用其他栏统计的结果得到extract_data2的统计方式可借鉴此方法
    df_city = pd.pivot_table(df2,index=[city_tag], columns=[qtag], values=[CODE], aggfunc=np.size)
    df_city = df_city['CODE'].loc[:, advertising_list]
    df_sex = pd.pivot_table(df2,index=[sex_tag], columns=[qtag], values=[CODE], aggfunc=np.size)
    df_sex = df_sex['CODE'].loc[:, advertising_list]
    df_age = pd.pivot_table(df2,index=[age_tag], columns=[qtag], values=[CODE], aggfunc=np.size)
    df_age = df_age['CODE'].loc[:, advertising_list]
    df_rsjd = pd.pivot_table(df2,index=[rsjd_tag], columns=[qtag], values=[CODE], aggfunc=np.size)
    df_rsjd = df_rsjd['CODE'].loc[:, advertising_list]
    ...
    df_period1 = df_city
    df_period2 = df_period1.apply(lambda x: x * df_weight.iloc[:, 0])
    total_list = list(df_period2.sum() / Base1)
    total_list.insert(0, Base1)

    # 处理城市、性别、年龄段和人生阶段
    df_city = df_city.apply(lambda x: x / city_base1)
    df_city.insert(0, 'base', city_base1)
    df_sex = df_sex.apply(lambda x: x / sex_base1)
    df_sex.insert(0, 'base', sex_base1)
    df_age = df_age.apply(lambda x: x / age_base1)
    df_age.insert(0, 'base', age_base1)
    df_rsjd = df_rsjd.apply(lambda x: x / rsjd_base1)
    df_rsjd.insert(0, 'base', rsjd_base1)
    
    return total_list, df_city.T, df_sex.T, df_age.T, df_rsjd.T

# 从excel中提取含显著性标记的数字矩阵, col_num指填入列的
def extract_digit(sht, row_num, col_num, vertical_length, data_list):
...
def period_proc1(qtag, sht, row_num, col_num_starting, vertical_length, total_list):
    # 获取各周期数据
    ...
    if os.path.exists(period_file):
        ...
        df_total_percent, total_temp = extract_digit(sht, row_num, col_num_starting, vertical_length, total_list)
...
# 处理城市、性别、年龄段和人生阶段
def csar_proc1(qtag, sht, row_num, col_num_starting, vertical_length, df_city, df_sex, df_age, df_rsjd):
    # 按Excel顺序填入城市信息
    ...
    for index, column in enumerate(df_city_proc.columns):
        ...
        df_digit_percent, base_temp = extract_digit(sht, row_num, col_num2, vertical_length, df_city_proc[column])
        # 先写入base数据
    ...
    for column in df_sex_proc.columns:
        ...
        col_num2 = col_num_starting + period_num * (index + 1)
        df_digit_percent, base_temp = extract_digit(sht, row_num, col_num2, vertical_length, df_sex_proc[column])
        # 先写入base数据
        sht.range(row_num, col_num2).value = base_temp
        # 标记显著性数值，并写入sheet <2.外食情况>
        mark_significance(df_digit_percent, base_temp, sht, row_num, col_num2)
    ...
    for column in df_age_proc.columns:
        index += 1  # 使用col_num2跟踪移动列
        col_num2 = col_num_starting + period_num * (index + 1)
        df_digit_percent, base_temp = extract_digit(sht, row_num, col_num2, vertical_length, df_age_proc[column])
        # 先写入base数据
        sht.range(row_num, col_num2).value = base_temp
        # 标记显著性数值，并写入sheet <2.外食情况>
        mark_significance(df_digit_percent, base_temp, sht, row_num, col_num2)
    ...
    for column in df_rsjd_proc.columns:
        index += 1  # 使用col_num2跟踪移动列
        col_num2 = col_num_starting + period_num * (index + 1)
        df_digit_percent, base_temp = extract_digit(sht, row_num, col_num2, vertical_length, df_rsjd_proc[column])
        # 先写入base数据
        sht.range(row_num, col_num2).value = base_temp
        # 标记显著性数值，并写入sheet <2.外食情况>
        mark_significance(df_digit_percent, base_temp, sht, row_num, col_num2)
...
def processing_type2(qtag, sht, row_num, col_num_starting, vertical_length, brand_list=brand_list):
    qtag_list = list(df_qa.loc[qtag].dropna())
    total_list, df_city, df_sex, df_age, df_rsjd = extract_data2(qtag, qtag_list, brand_list)

    # 处理并填入"周期"信息
    period_proc1(qtag, sht, row_num, col_num_starting, vertical_length, total_list)
    # 处理城市、性别、年龄段和人生阶段
    csar_proc1(qtag, sht, row_num, col_num_starting, vertical_length, df_city, df_sex, df_age, df_rsjd)

def processing_type3(qtag, sht, row_num, col_num_starting):
    qtag_list = list(df_qa.loc[qtag].dropna())
    vertical_length = 10
    total_list, df_city, df_sex, df_age, df_rsjd = extract_data3(qtag, qtag_list)
...
def BIT_automation(filepath):
    ...
    # 读取<3.品牌与广告知名度> sheets
    sht3 = wb.sheets['3.品牌与广告知名度']
    # 将QB1数据填入<3.品牌与广告知名度> sheets
    processing_type2('QB1', sht3, 6, 4, 7)  # 成功！
    processing_type2('QB2', sht3, 21, 4, 7)
    processing_type2('QB4', sht3, 36, 4, 7)  # 成功！
    processing_type2('QB3', sht3, 51, 4, 7)  # 成功！
    processing_type2('QB3.1', sht3, 66, 4, 7)  # 成功！
    processing_type2('QB5', sht3, 82, 4, 7)  # 成功！
    processing_type3('QB6.1', sht3, 97, 4, 10)
