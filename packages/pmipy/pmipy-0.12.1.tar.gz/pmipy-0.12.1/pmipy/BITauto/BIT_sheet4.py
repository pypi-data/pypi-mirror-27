...
# 用于QB8-14
def period_proc2(qtag, sht, row_num, col_num_starting, vertical_length, total_list):
    # 获取各周期数据
    period_file = '%s_period_data.xlsx' % qtag
    if os.path.exists(period_file):
        df_total = pd.read_excel(period_file, usecols=period_num-2)
        ...
        df_total_percent, total_temp = extract_digit(sht, row_num-1, col_num_starting, vertical_length, total_list)
    # 改掉total_temp
...
# 用于QB8-
def csar_proc2(qtag, sht, row_num, col_num_starting, vertical_length, df_city, df_sex, df_age, df_rsjd):
    # 修改row_num
    row_num = row_num - 1
    complete_city_list = ['成都', '哈尔滨', '西安', '郑州', '福州', '杭州']
    df_city_proc = dimension_proc(df_city, complete_city_list, vertical_length)
...
def processing_type4(qtag, sht, row_num, col_num_starting, vertical_length, brand_list):
    qtag_list = list(df_qa.loc[qtag].dropna())
    total_list, df_city, df_sex, df_age, df_rsjd = extract_data2(qtag, qtag_list, brand_list)

    # 处理并填入"周期"信息
    period_proc2(qtag, sht, row_num, col_num_starting, vertical_length, total_list)
    # 处理城市、性别、年龄段和人生阶段
    csar_proc2(qtag, sht, row_num, col_num_starting, vertical_length, df_city, df_sex, df_age, df_rsjd)
...
def BIT_automation(filepath):
    ...
    row_num = 6
    processing_type2('QB7' , sht4, row_num, 4, 6, brand_list2)
    for i in range(1, 8):
        qtag = 'QB' + str(7 + i)
        row_num = 7 + i * 6
        processing_type4(qtag, sht4, row_num, 4, 6, brand_list2)
