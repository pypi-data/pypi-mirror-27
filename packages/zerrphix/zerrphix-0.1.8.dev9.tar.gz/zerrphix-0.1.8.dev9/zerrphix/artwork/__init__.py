                    ...
                    if _type == 'text':
                        if item[_type].has_key('@special'):
                            if hasattr(self.specials, item[_type]['@special']) or item[_type]['@special'] in ['season',
                                ...
                                if not isinstance(special_text, basestring):
                                ...
                                if text:
                                    ...
                                    if item[_type].has_key('@font_size'):
                                        if item[_type]['@font_type'] == 'truetype':
                                            font = ImageFont.truetype(os.path.join(template_folder,
                                                                                   item[_type]['@font']),
                                        ...
                                        if '@use_dotdotdot' in item[_type]:
                                            if item[_type]['@use_dotdotdot'].lower() == 'true':
                                                use_dotdotdot = True
                                        text_line(ImageDraw.Draw(img), int(item[_type]['@x']), int(item[_type]['@y']),
                                                  text, font,
                                                  ...
                                                  int(item[_type]['@width']) if item[_type].has_key('@width') else None,
                                        ...
                                        lines, line_spacing, font = text_box(ImageDraw.Draw(img),
                                                                             text,
                                                                             font_file_path,
                                                                             int(item[_type]['@width']),
                                                                             int(item[_type]['@height']),
                                                                             int(item[_type]['@font_min_size']),
                                                                             int(item[_type]['@font_max_size']),
                                                                             int(item[_type]['@x']),
                                                                             int(item[_type]['@y']),
                                                                             int(item[_type]['@line_spacing']),
                                                                             item[_type]['@split_char'] if item[
                                                                                 _type].has_key('@split_char') else ' ',
                                                                             int(item[_type]['@use_sentences']) if item[
                                                                                 _type].has_key(
                                        ...
                                        if isinstance(lines, list):
                                            ...
                                            for line in lines:
                                                ...
                                                ImageDraw.Draw(img).text(
                                                    (int(item[_type]['@x']), y_minus_y_line_font_offset), line,
                            ...
                            if text:
                                ...
                                if item[_type]['@font_type'] == 'truetype':
                                    font = ImageFont.truetype(
                                        os.path.join(template_folder, item[_type]['@font']),
                                        int(item[_type]['@font_size']))
                                    text_line(ImageDraw.Draw(img), int(item[_type]['@x']), int(item[_type]['@y']), text,
                                              font,
                                              item[_type]['@colour'] if item[_type].has_key('@colour') else '#FFF',
                                              item[_type]['@halign'] if item[_type].has_key('@halign') else '',
                                              item[_type]['@valign'] if item[_type].has_key('@valign') else '',
                        ...
                        if item[_type].has_key('@special'):
                            ...
                            if item[_type]['@special'] in ['backdrop', 'poster', 'banner', 'collection_poster',
                                ...
                                if temp_image is not None:
                                    img.paste(temp_image, (int(item[_type]['@x']), int(item[_type]['@y'])),
                                        ...
                                        if os.path.isfile(score_image_file_path):
                                            ...
                                            if item[_type].has_key('@alpha_composite') and item[_type][
                                                ...
                                                img.paste(temp_image,
                                                          (int(item[_type]['@x']), int(item[_type]['@y'])),
                                                          temp_image.convert('RGBA'))
                                                log.debug('''img paste complete for item[_type]: {0}'''.format(
                                                    item[_type]))
                                                temp_image.close()
