...
class XDCCDownloadManager(object):
    ...
    @staticmethod
    def prepare_directory(directory: str, series_name: str, season: int) \
            -> Tuple[str, str]:
        """
        ...
        MetaDataManager.generate_media_directory(
            destination_directory, media_type="tv_series"
        )

        season_directory = \
            os.path.join(destination_directory, "Season " + str(season))
    ...
    def get_max_season_and_episode_number(directory) -> Tuple[int, int]:
        ...
        if not MetaDataManager.is_media_directory(directory, "tv_series"):
            ...
            seasons = list(filter(
                lambda x: x.startswith("Season "), os.listdir(directory)
            ))
            ...
            if len(seasons) == 0:
                ...
                max_season = \
                    max(seasons, key=lambda x: int(x.split("Season ")[1]))
                max_episodes = 0
    ...
    @staticmethod
    def auto_rename(renaming_scheme: GenericScheme, starting_episode: int,
                    packs: List[XDCCPack]) -> None:
        """
        ...
        season_number = \
            int(os.path.basename(season_directory).rsplit("Season ", 1)[1])
    ...
    @staticmethod
    def get_preliminary_renaming_results(renaming_scheme: GenericScheme,
                                         starting_episode: int,
                                         packs: List[XDCCPack],
                                         season_number: int, series_name: str)\
            -> List[str]:
        """
