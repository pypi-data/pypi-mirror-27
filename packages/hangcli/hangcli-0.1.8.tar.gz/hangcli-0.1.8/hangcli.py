...
def react(project):
	...
	reactSrcWebpackconfig = '/usr/local/hangcli/webpack.config.js'
	babelRc = '/usr/local/hangcli/.babelrc'
	indexJS = '/usr/local/hangcli/index.js'
	indexHtml = '/usr/local/hangcli/index.html'
	...
	# 将index.js放入到src/index.js中，其中src我们需要新建
	copyfile(indexJS, os.getcwd()+ '/'+ project + '/' + 'src' + '/' + 'index.js')
	# 将index.html放入到dist/index.html中，其中dist我们需要新建
	copyfile(indexJS, os.getcwd()+ '/'+ project + '/' + 'dist' + '/' + 'index.html')
