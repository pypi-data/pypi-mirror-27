...
def get_bilan_or_CExploit_params(orga, year, file_type):
    ...
    dico = { 'target_dir':target_dir,
        ...
        'tex_parameters' : 'data_parameters',
        'document_tmpl' : 'Bilan_CExploit_tmpl',
        'year' : year,
        'file_type' : file_type,
    }
...
def write_pdf(params, unicode_TeX):
    ...
    parameters_file = os.path.join(tmp_dir, '%(tex_parameters)s.tex' % params)
    open(parameters_file, 'w').write(unicode_TeX)
    open(os.path.join(tmp_dir, '%(base_file)s.tex' % params), 'w').write(
            '''\\include{%(tex_parameters)s}
               \\include{%(document_tmpl)s}''' % params )
...
\\multicolumn{3}{P{3.5in}|}{\\centering{\\textbf{PASSIF}}}\\\\\\hline

Descriptif & Brut & \\scriptsize{Amortissement/ Dépréciations} &
Net & Exercice %(prev_year)s&
Descriptif & Valeur & Exercice %(prev_year)s\\\\\\hline\n''' % {
        'prev_year' : self._year - 1, }
        else:
            return '''
        |P{2.555in}|P{0.8in}|P{0.8in}|P{2.555in}|P{0.8in}|P{0.8in}|}
\\hline
\\multicolumn{3}{|P{4.155in}|}{\\centering{\\textbf{CHARGES (hors taxes)}}} &
\\multicolumn{3}{P{4.155in}|}{\\centering{\\textbf{PRODUITS (hors taxes)}}}\\\\\\hline
Descriptif & Exercice %(year)s & Exercice %(prev_year)s&
Descriptif & Exercice %(year)s & Exercice %(prev_year)s\\\\\\hline\n''' % {
                    'year' : self._year,
                    'prev_year' : self._year - 1, }
...
\\companySiren\\ R.C.S. \\companyRCS \\par \\bigskip
}
        ''' % { 'year' : self._year,
    ...
    def add_header(self):
        return '''%(company_header)s
    \\newcommand{\\tableau}{
    \\begin{supertabular}{%(structure_tableau)s\n\n''' % {
    'company_header' : self.add_header_company(),
    ...
    def gen_pdf(self):
        ...
        param_gen = get_bilan_or_CExploit_params(
                self._orga, self._year, self._doc_name.replace(' ', '_'))
