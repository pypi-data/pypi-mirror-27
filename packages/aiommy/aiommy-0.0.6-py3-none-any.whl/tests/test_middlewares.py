...
class LoggingMiddlewareTestCase(MiddlewareTestCase):
    ...
    @unittest_run_loop
    async def test_response_empty_message(self):
        message = 'msg'

        async def handler(request):
            return web.HTTPServiceUnavailable(text=message)

        async def handler_raiser(request):
            raise web.HTTPServiceUnavailable(text=message)

        for h in [handler, handler_raiser]:
            middleware = logging_middeleware_factory(self.logger)
