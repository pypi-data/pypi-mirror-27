...
class MotorClient(CommandRunner):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.insert_index = 0
        # do frames get saved and uploaded? Turns on when the command "start_frames" is
        # received. This is separate from the canbus run.
    ...
    def check_motor(self, update_command=True, status="Ready"):
        ...
        start_tag = len(self.card.frames_list)-1
        ...
        state = {"status": status, "model": "ADP-055-18",
                 ...
                 'emcy_msgs': self.card.emcy_msgs[-1] if len(self.card.emcy_msgs) > 0
                 else "No emergency messages detected"}
    ...
    def run_canbus(self):
        ...
        for frame_to_write in frames_to_write:
            ...
            if "expression" in frame_to_write.keys():
                ...
                xmit_frame = CanFrame(id=frame_to_write["id"], data=frame_to_write["data"])
                output_frames.append(deepcopy(xmit_frame))
                self.card.xmit(xmit_frame, timeout=self.card.timeout, append=False)
    ...
    def run_motor_trace(self):
        ...
        logger.info("Motor setup is: " +
                    str(self.setup["id"]) + " pos is: " +
                    str(self.card.property_getter("actual_position"))+" "+str(test_order["relative_move"]))
        ...
        transmitter.results_dict["target_reached"] = self.card.trace_target_reached
