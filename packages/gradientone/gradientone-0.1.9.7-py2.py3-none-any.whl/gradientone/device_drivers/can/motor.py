    ...
    def handle_sdo(self, in_frame):
        ...
        def send_toggle(in_frame, width, out_data):
            ...
            while not (0x580 <= in_frame.id < 0x600 and in_frame.data[0] < 0x40):
                ...
                in_frame = self.recv(timeout=self.timeout)
                if in_frame is None:
                    gateway_helpers.logger.error("Response frame not received within timeout. Check your CAN device.")
...
if __name__ == "__main__":
    ...
    for i in range(1000):
        ...
        start = time()
        data = card.do_trace(10000, 2000, .50, properties=['actual_load_position', 'actual_motor_velocity'], poll=False, relative=True)
        end = time()
        ...
        if len(target_reached) == 0:
            target_reached = [0]
        print(end-start, target_reached[-1])