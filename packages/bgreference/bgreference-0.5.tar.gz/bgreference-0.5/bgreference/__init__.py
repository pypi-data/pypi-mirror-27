...
def _get_dataset(build_name, release=bgdata.LATEST):
    global REF_PATHS, REF_MMAPS

    if build_name not in REF_PATHS:
        REF_PATHS[build_name] = bgdata.get_path('datasets', 'genomereference', build_name, build=release)
        REF_MMAPS[build_name] = {}

    return REF_PATHS[build_name]


def _get_mmap(build_name, sequence_name, release=bgdata.LATEST):
...
def refseq(build_name, sequence_name, start, size=1, release=bgdata.LATEST):
