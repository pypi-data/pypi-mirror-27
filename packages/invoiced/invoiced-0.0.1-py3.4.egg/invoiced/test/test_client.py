...
class TestClient(unittest.TestCase):
    ...
    def test_get_request(self):
        def request_callback(request):
            # verify headers
            self.assertEqual(request.headers['authorization'], "Basic dGVzdDo=")
            self.assertEqual(request.headers['content-type'], "application/json")
            self.assertEqual(request.headers['user-agent'], "Invoiced Python/" + invoiced.VERSION)
        ...
        responses.add_callback(responses.GET,
          'https://api.invoiced.com/invoices?filter[levels]=work&test=property',
