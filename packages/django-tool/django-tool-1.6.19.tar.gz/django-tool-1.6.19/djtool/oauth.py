...
class ClientOauth(Common):
    ...
    def application(self):
        response = self.requests.get('%s/api/v1/application/?client_id=%s&env' % (settings.ACCOUNT_API_HOST, self.client_id, settings.ENVIRONMENT), headers={'X-CSRFToken': self.requests.cookies.get('csrftoken'), 'TOKEN': self.get_token()}).json()
        return response.get('data', []) if response.get('code') == 20000 else []
