...
class ClientOauth(Common):
    ...
    def application(self):
        response = self.requests.get('%s/api/v1/application/?client_id=%s' % (settings.ACCOUNT_API_HOST, self.client_id), headers={'X-CSRFToken': self.requests.cookies.get('csrftoken'), 'TOKEN': self.get_token()}).json()
        return response.get('data', []) if response.get('code') == 20000 else []
    ...
    def _update_user_info(self, user, uuid, data):
        response = self.requests.put('%s/api/v1/%s/%s/?client_id=%s' % (settings.ACCOUNT_API_HOST, user, uuid, self.client_id), data=data, herders={'TOKEN': self.get_token()}).json()
        return response.get('data', {}) if response.get('code') == 20000 else {}
