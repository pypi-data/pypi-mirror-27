...
def download_all(comic_name, time = datetime.datetime.now()):
    """
    :type time: datetime.datetime
    :param time: the datetime.datetime object
    :type comic_name: str
    :param comic_name: the comic to download
    :return: Nothing
    :rtype: None
    """
    save_name = save_structure(time) + '.jpg'
    try:
        while True:
            get_picture(comic_name, dt = time, save_name=save_name)
            time = time - one_day
            save_name = save_structure(time) + '.jpg'
    except InvalidDate:
        print('Complete.')