...
# Here's a an inexhaustive list of some of the dependencies that might arise if you use my library (you only need the ones that you use, so don't worry if you dont have them all):
# gtts_token
# pygame
# cv2
# TestOSC
...
__known_editors=['vim','emacs','suplemon','atom','xo','sublime']# NONE of these names should intersect any methods or varables in the r module or else they will be overwritten! FYI: 'xo' stands for 'exofrills', I haven't used it much though. I don't really use console based editors much...
for __editor in __known_editors:
    exec("""
def X(file_or_object):
    edit(file_or_object,editor_command='X')""".replace('X',__editor))
del __known_editors,__editor# This is just a setup section to create methods for us, so get rid of the leftovers. __known_editors and __editor are assumed to be unused anywhere else in our current namespace!
#endregion
...
import rp.rp_ptpython.prompt_style as ps
Đ_python_input_eventloop = None#Singleton for python_input
def python_input(scope,header='',enable_ptpython=True):
