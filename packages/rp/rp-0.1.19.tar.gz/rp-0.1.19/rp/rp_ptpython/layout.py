...
from .filters import HasSignature, ShowSidebar, ShowSignature, ShowDocstring,ShowRealtimeInput,ShowVarSpace,ShowVarSpaceAndShowRealtimeInput,ShowVarSpaceOrShowRealtimeInput
...
def create_layout(python_input,
    ...
    def title_fill(title,fill=' '):
        import rp.r_iterm_comm
        rp.r_iterm_comm.python_input_buffers[title]=title
        try:
            return VSplit([
            Window(height=D.exact(1),content=FillControl(fill,token=Token.Window.TIItleV2)),
            Window(BufferControl(buffer_name=title,lexer=SimpleLexer(default_token=Token.Window.TIItleV2),),wrap_lines=False,height=D.exact(1),width=D.exact(len(title))),
            Window(height=D.exact(1),content=FillControl(fill,token=Token.Window.TIItleV2)),
                    ])
        except:
                              ...
                              VSplit([
                                  ConditionalContainer(
                                      ...
                                      HSplit([
                                          ...
                                          Window(BufferControl(buffer_name='realtime_display',lexer=SimpleLexer(default_token=Token.Docstring),),wrap_lines=True,height=D(max=12))
                                      ]),
                                  ...
                                  ConditionalContainer(
                                      ...
                                      HSplit([
                                          title_fill("VARS"),
                                          Window(BufferControl(buffer_name='vars',lexer=SimpleLexer(default_token=Token.Docstring),),wrap_lines=True,height=D(max=12))
                                      ]),
