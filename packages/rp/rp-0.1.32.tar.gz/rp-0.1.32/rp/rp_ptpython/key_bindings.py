...
def load_python_bindings(python_input):
    ...
    def _(event):# Spacebar
        ...
        try:
            function_comma_flag=function_comma_flag and after_line.startswith(")")
    ...
    def _(event):
        ...
        for t in selection_tuples:# Handle arrow-keys on selection by putting the cursor on beginning or end of selection
            buffer._set_cursor_position(t[0])
            buffer.exit_selection()
        else:
            buffer.cursor_left()
    ...
    def thing2(char):
        ...
        def _(event,filter=has_selection):# Parenthesis completion
            ...
            if not surround(buffer,char,char):
                ...
                if after.startswith(char):
                    buffer.cursor_right()
                elif not before and not after or before and after and before[-1]in'(=!#%&*+,-./:;<>^|~' and after[0]in')=!#%&*+,-./ :;<>^|~' or before and after and before[-1]+after[0] in 2*char:
            ...
            def beginswithany(a,*b):
                for x in b:
                    if a.startswith(x):
                        return True
                return False
            if beginswithany(current_line,'for ','def ','lambda ','while ','with ','if ','except ','try ') or not text or text.split('\n')[0] in ["):",']',')','}',':']:# Presumably at the end of def( a,b,c,d,e^): where ^ is cursor
