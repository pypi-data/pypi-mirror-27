...
def load_python_bindings(python_input):
    ...
    def _(event):# Spacebar
        ...
        from r import space_split,is_namespaceable
        import r_iterm_comm
        split=space_split(before_line)
        from r import printed
        try:
            if not after_line and all(is_namespaceable(x) for x in split) and len(split)==2 and split[0]=='def':
                ...
                buffer.cursor_right(count=1)
            elif before_line and before_line[-1]=='('and after_line[0]==')':
                buffer.delete_before_cursor(count=1)
                buffer.cursor_right(count=1)
                buffer.delete_before_cursor(count=1)
                buffer.insert_text(' ')
                ...
                buffer.insert_text(',')
            elif split and not before_line.endswith(" ") and  (split[-1]) in r_iterm_comm.globa and callable(r_iterm_comm.globa[split[-1]]):
                buffer.insert_text('()')
                buffer.cursor_left(count=1)
            elif split and not before_line.endswith(" ") and split[-1] in r_iterm_comm.globa and hasattr(r_iterm_comm.globa[split[-1]],'__getitem__'):
                buffer.insert_text('[]')
                buffer.cursor_left(count=1)
            else:
                buffer.insert_text(' ')
