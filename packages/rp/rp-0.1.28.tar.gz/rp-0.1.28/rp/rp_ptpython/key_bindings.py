...
def load_python_bindings(python_input):
    ...
    @handle(Keys.ControlC)# On mac this is alt+z
    def _(event):
        buffer=event.cli.current_buffer
    ...
    def surround(buffer,begin,end):
        ...
        for t in selection_tuples:
            ...
            state=prompt_toolkit.selection.SelectionState
            selectoin=state(max(t) + 2,type=doc.selection.type)
            buffer.document=Document(text[:t[0]] + begin +text[t[0]:t[1]] + end +text[t[1]:],min(t),state())
            buffer._set_cursor_position(min(t))
            # buffer.document.selection=selectoin
            # buffer.document.selection.original_cursor_position=0
            # buffer=Document(text[:t[0]]+begin+text[t[0]:t[1]]+end+text[t[1]:],doc.cursor_position,doc.selection)
        return bool(selection_tuples)
    ...
    def _(event):
        ...
        for whocares in range(4):
            try:
                if buffer.document.current_line.startswith(' '):
                    buffer.cursor_left()
