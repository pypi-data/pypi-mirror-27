...
import rp_ptpython.prompt_style as ps
Đ_python_input_eventloop = None#Singleton for python_input
def python_input(scope=globals,header=''):
    try:
        from prompt_toolkit.shortcuts import create_eventloop
        from rp_ptpython.python_input import PythonCommandLineInterface,PythonInput as Pyin
        global Đ_python_input_eventloop
...
def pseudo_terminal(*dicts,get_user_input=python_input,modifier=None,style=pseudo_terminal_style()):
    ...
    import r_iterm_comm# Used to talk to ptpython
    ...
    try:
        while True:
            try:
                ...
                try:
                    user_message=get_user_input(header=style.header)
                    allow_keyboard_interrupt_return=False
                except KeyboardInterrupt:
                    if allow_keyboard_interrupt_return:
                        fansi_print("Caught repeated KeyboardInterrupt ⟹ RETURN",'cyan','bold')
