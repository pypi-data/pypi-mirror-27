...
def escape_char_callback(spawn):
    ...
    for retry_number in range(
            spawn.settings.ESCAPE_CHAR_CALLBACK_PAUSE_CHECK_RETRIES):
        sleep(spawn.settings.ESCAPE_CHAR_CALLBACK_PAUSE_SEC)
    ...
    sleep(spawn.settings.ESCAPE_CHAR_CALLBACK_PRE_SENDLINE_PAUSE_SEC)
    spawn.sendline()
...
def password_handler(spawn, context, session):
    ...
    if session.get('tacacs_login') == 1:
        ...
        if '-l' in spawn_command_list or re.search(r'\S+@\S+', spawn_command):
            if 'tacacs_password' in context:
                spawn.sendline(context['tacacs_password'])
                session['tacacs_login'] = 0
                return
    else:
...
def bad_password_handler(spawn):
    ...
    """
    raise Exception('Bad Password send to device %s' % (str(spawn),))
...
def incorrect_login_handler(spawn, session):
    ...
    if session['incorrect_login_attempts'] <=3:
        ...
        raise Exception('Login failure, either wrong username or password')
