...
from unicon.plugins.nso.utils import NsoUtils
...
class Command(BaseService):
    """ Service to execute a single command on the NSO CLI.
    ...
    Example:
        .. code-block:: python
              ...
              output = nso.command('show services')
    ...
    def call_service(self, command,
        ...
        con.log.debug("+ plugin nso +++ command '%s' +++" % command)
        ...
        patterns = NsoPatterns()
        dialog = Dialog()
        if reply:
            dialog += reply
        dialog.append(Statement(pattern=patterns.cisco_or_juniper_prompt))
        dialog.append(Statement(pattern=patterns.cisco_or_juniper_config_prompt))
...
class Configure(BaseService):
    """ Service to configure NSO with list of `commands`.
    ...
    Example:
        .. code-block:: python
              ...
              output = nso.configure()
              output = nso.configure('services sw-init-l3vpn foo \
              ...
              output = nso.configure(cmds, timeout=120)
    ...
    def call_service(self, command=[],
        ...
        con.log.debug('+ plugin nso +++ configure state %s +++' % sm.current_state)
        ...
        try:
            for cmd in command:
                ...
                if self.result:
                    ...
                    output = re.sub(r"^\r\n", "", output, 1)
                    output = output.replace(PROMPT_PREFIX, "")
...
class Execute(BaseService):
    """ Execute Service implementation for NSO
    ...
    def call_service(self, command=[],
        ...
        for cmd in command:
            con.log.debug("+ plugin nso +++ execute command '%s' +++" % cmd)
            self.result = con.command(cmd,
                reply=reply,
                error_pattern=error_pattern,
                timeout=timeout)
...
class CliStyle(BaseService):
    ...
    def call_service(self, style, *args, **kwargs):
        ...
        con.log.debug('+ plugin nso +++ cli_style current state %s +++' % sm.current_state)
        ...
        con.log.debug('+ plugin nso +++ cli_style new state %s +++' % sm.current_state)
        self.result = True
