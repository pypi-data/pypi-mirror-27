...
class NsoStateMachine(StateMachine):
    ...
    def detect_state(self, connection, timeout=30):
        ...
        if result is None:
            dialog = Dialog()
            dialog.append(Statement(pattern=patterns.cj_prompt))
            dialog.append(Statement(pattern=patterns.cj_config_prompt))
            connection.sendline()
            try:
                result = dialog.process(connection.spawn, timeout=timeout)
            except Exception as err:
                raise SubCommandFailure("Command execution failed", err)

        prompt = result.match_output.split()[-1]
        connection.log.debug("+ plugin nso +++ prompt %s +++" % prompt)

        state_match = False
        for state in self.states:
            connection.log.debug("+ plugin nso +++ checking pattern '%s' against prompt: '%s'" % (state.pattern, prompt))
        ...
        connection.log.debug("+ plugin nso +++ detected state %s +++" % self.current_state)
