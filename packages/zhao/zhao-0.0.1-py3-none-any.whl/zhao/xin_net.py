...
class MiWiFi(object):
    ...
    if MIWIFI.adsl_is_offline:
        MIWIFI.adsl_reconnect()
    """
    ...
    @property
    def adsl_is_online(self):
        """ADSL 是否在线
        """
        test_urls = ('http://www.163.com',
                     'http://www.baidu.com',
                     'http://www.sina.com.cn')
        return not all(requests.head(url).status_code == 302 for url in test_urls)

    @property
    def adsl_is_offline(self):
        """ADSL 是否掉线
        """
        return not self.adsl_is_online

    def _do(self, action):
        url = f'http://miwifi.com/cgi-bin/luci/;stok={self.token}/api/xqnetwork/{action}'
        return self.session.get(url).json()

    def adsl_disconnect(self):
        """断开 ADSL
        """
        try:
    ...
    def adsl_reconnect(self):
        """重新连接 ADSL
        """
        if self.adsl_disconnect():
            sleep(10)
            if self.adsl_connect():
                sleep(20)
                return self.adsl_is_online
