...
Convert it to whether int of float, and try again.")
 ...
 entries.'.format(len(self.Seq)))
    ...
    def first_digits(self, digs, inform=True, Z_conf_level=None, high_Z='pos',
        ...
        if str(Z_conf_level) not in list(confs.keys()):
...
records < {2} after preparation.".format(len(self), len(self) - len(temp),
            ...
            if Z_conf_level is not None:
                _inform_(df, high_Z=high_Z, conf=confs[str(Z_conf_level)])
