...
class TaylorAndFrancisOnline(GettableDOI):
    # -- constants --
    JOURNAL_URL = 'tandfonline.com'
    JOURNAL_STR = 'Talor & Francis Online'
    DOI_KEY = 'tandfonline.com/doi/'
    ...
    def __get_raw_doi(self, *, url: str)->str or None:
        ...
        # e.g. "http://www.tandfonline.com/doi/abs/10.1080/0097840X.1987.9936798"
        ...
        if self.__decision_include_keyword(keyword=self.DOI_KEY, text=url):
            ...
            for char in url:
                ...
                count += 1
            doi = doi.replace('abs/', '')
            # 10.1080/0097840X.1987.9936798
            # print(doi)
            return doi
        else:
            print("含まれない！？ scrapingを用いた検索処理は未実装です。")
            print('NoneDOI From Talor & Francis Online ({link})'.format(link=url))
            return None
