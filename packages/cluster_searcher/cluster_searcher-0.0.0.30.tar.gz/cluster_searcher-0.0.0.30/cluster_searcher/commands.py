...
@command()
def load_kegg( file_name: MOptional[Filename[EXT.KEGG]] = None, permit: Optional[List[str]] = None ):
    """
    ...
    """
    if not file_name:
        file_name = state.settings.last_kegg
    ...
    state.kegg_file = kegg.KeggFile( file_name, permit )
    ...
    state.settings.last_kegg = file_name
...
def write_report( silent: bool = False,
                  ...
                  combine: bool = False,
                  report: MOptional[Filename[EXT.REPORT]] = None,
    ...
    if report == "default":
        if using:
            ...
            report = file_helper.replace_extension( state.kegg_file.file_name, ".report" if not combine else ".report_c" )
    ...
    try:
        ...
        # - CLUSTERS file
        report_file = file_writing.open_write( report, "report", overwrite_all )
...
@command()
def write_all_to_report( directories: List[str], silent: bool = True, combine: bool = False, all: bool = False ):
    """
    Translates all .xml (kgml) files in the specified directory into .report files.
    **Existing files are overwritten**
    
    :param directories:   Directory to process 
    :param silent:      Passed to `write_report`. Note the default is different. 
    :param all:         Passed to `write_report`. Note the default is different.
    ...
    for file_name in MCMD.iterate( file_names, "Iterating files" ):
        ...
        write_report( silent = silent, combine = combine, overwrite = True, all = all )
        write_udm( report.EResolveColour.BLAST, overwrite = True )
        write_udm( report.EResolveColour.COMBINED, overwrite = True )
        write_udm( report.EResolveColour.TEXT, overwrite = True )


@command()
def write_all_to_udm( directory: Dirname ):
    """
    ...
    **Existing files are overwritten**
    ...
    for file_name in MCMD.iterate( file_names, "Iterating files" ):
        ...
        write_udm( colour = report.EResolveColour.BLAST, read = file_name, overwrite = True )
        write_udm( colour = report.EResolveColour.COMBINED, read = file_name, overwrite = True )
        write_udm( colour = report.EResolveColour.TEXT, read = file_name, overwrite = True )
