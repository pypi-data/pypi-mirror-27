...
#if defined(DEBUG) && DEBUG
	    PyErr_WarnEx(NULL,  "pfunc before callback\n", 1);
#endif
	    try{
...
#if defined(DEBUG) && DEBUG
	    PyErr_WarnEx(NULL, "pfunc after callback:\n", 1);
#endif
	    Py_XDECREF(result);
	    Py_XDECREF(arg);
...
#define MoveValuesToBuffer(DBRBUFTYPE, DBRTYPE, FORMAT, PBUF)        \
	...
	for(cur=0; cur<count; cur++){\
	  ...
	  ptr[cur]=(DBRTYPE) buf;\
          Py_XDECREF(item); /* added by NY, 2018.1.3 */ \
	}\
   }
...
static PyObject *Py_ca_put(PyObject *self, PyObject *args)
{
  ...
  if (count == 1){
    ...
    case DBR_INT:/* note that MoveValuesToBuffer is Macro */
      MoveValuesToBuffer(dbr_long_t, dbr_int_t, "i",pbuf); 
      break;
    case DBR_FLOAT:
      MoveValuesToBuffer(dbr_double_t, dbr_float_t, "d",pbuf);
      break;
    case DBR_ENUM:
      MoveValuesToBuffer(dbr_long_t, dbr_enum_t, "l",pbuf);
      break;
    case DBR_CHAR:
      MoveValuesToBuffer(dbr_char_t, dbr_char_t, "b", pbuf);
      break;
    case DBR_LONG:
      MoveValuesToBuffer(dbr_long_t, dbr_long_t, "l", pbuf);
      break;
    case DBR_DOUBLE:
      MoveValuesToBuffer(dbr_double_t, dbr_double_t, "d", pbuf);
      break;
    default:
    ...
    if(pbuf != NULL){
      //PyErr_WarnEx(NULL, "releasing buffer for array input", 0);
      free(pbuf);
    }
