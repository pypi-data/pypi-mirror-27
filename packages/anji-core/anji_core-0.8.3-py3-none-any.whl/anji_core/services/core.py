...
class ServiceControlTask(RegularTask):
    ...
    def execute(self):
        target_service = self.shared.bot.manager_env['services'].get_by_technical_name(self.service_technical_name, self.shared.bot.sudo)
        message_body = ''
        message_type = MessageType.none
        target_service_status = target_service.get_status()
        if target_service_status in [ServicesStatus.absent, ServicesStatus.unknown]:
            message_body = f'Service {target_service.name} not valid'
            message_type = MessageType.error
        else:
            if self.control_signal == 'restart' and target_service_status != ServicesStatus.restarting:
                target_service.restart()
                message_body = f'Service {target_service.name} was restarted'
                message_type = MessageType.info
            elif self.control_signal == 'start' and target_service_status != ServicesStatus.running:
                target_service.start()
                message_body = f'Service {target_service.name} was started'
                message_type = MessageType.info
            elif self.control_signal == 'stop' and target_service_status != ServicesStatus.stopped:
                target_service.stop()
...
class AbstractService(SecureModel):
    ...
    status = EnumField(ServicesStatus, default=ServicesStatus.unknown, description='Service status', displayed=False)
    ...
    def describe_status(self):
        if self.status in [ServicesStatus.stopped, ServicesStatus.absent, ServicesStatus.unknown]:
            return MessageType.error
        elif self.status in [ServicesStatus.restarting]:
