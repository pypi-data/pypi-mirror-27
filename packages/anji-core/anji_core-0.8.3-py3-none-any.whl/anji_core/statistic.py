...
from anji_orm import register, Model, IntField, DatetimeField, FloatField, DictField, query, ListField
...
class TimeSeriesMixin(object):
    ...
    def root_mean_square(self):
        ...
        model_query = self.build_query(self.additional_statistic_filter(self.build_similarity_dict()))
        if weight_field:
        ...
        with register.pool.connect() as conn:
            model_stats = model_query.run(conn)
    ...
    @classmethod
    def additional_statistic_filter(cls, current_query_dict: Dict) -> Dict:
...
class TimeSeriesCompressModel(Model, TimeSeriesMixin):
    ...
    def get_data(cls, original_model_record: Union[Model, TimeSeriesMixin]):
        with register.pool.connect() as conn:
            compress_model_stats = cls.build_query(dict(
                original_python_info=original_model_record._build_python_info(),
                original_definer_key=original_model_record.compressed_primary_key
            )).map(lambda data: {'value': data['value'], 'weight': data['weight']}).run(conn)
    ...
    @classmethod
    def automatic_original_data_compress_for_model(cls, original_model: Union[Model, TimeSeriesMixin], days_before: int, offset: int):
        ...
        with register.pool.connect() as conn:
            groups = original_model.unique_groups_query().run(conn)
        for group in groups:
            group_search_query = original_model.build_query(original_model.additional_statistic_filter(group))\
                    .filter(R.row[original_model_timestamp_field] <= start_time)\
                    .order_by(R.desc('complete_timestamp')).skip(offset)
            group_records = query(group_search_query)
            if group_records:
                cls.compress_original_data(group_records)

    @classmethod
    def automatic_original_data_compress(cls, days_before: int, offset: int):
    ...
    @classmethod
    def compress_original_data(cls, original_models: List[Union[Model, TimeSeriesMixin]]):
    ...
    @classmethod
    def automatic_compressed_data_compress(cls, statistic: 'StatisticEngine'):
        with register.pool.connect() as conn:
            groups = cls.unique_groups_query().run(conn)
        for group in groups:
            data_list = query(cls.build_query(group))
            if data_list:
