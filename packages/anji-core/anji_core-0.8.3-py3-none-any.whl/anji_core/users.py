from anji_orm import StringField, ListField, BooleanField, query
...
class UserCartridge(AbstractCartridge):
    ...
    def user_register(self, mess, _args):
        user = query(User.build_query(dict(technical_name=mess.frm.person)))
        if user:
    ...
    def user_approve(self, mess, users):
        user_records = query(User.build_query(dict(name=users, is_authorized=False)))
        if user_records:
