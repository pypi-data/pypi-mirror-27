...
class SignalCartridge(AbstractCartridge):
    ...
    def get_subscribe_record(self, mess: Message):
        ...
        model = query(self.shared.bot.users.build_query(dict(technical_name=technical_name)))
        return model[0] if model else self.shared.bot.users.create_record(technical_name=technical_name, name=mess.frm.username, signal_types=[])
