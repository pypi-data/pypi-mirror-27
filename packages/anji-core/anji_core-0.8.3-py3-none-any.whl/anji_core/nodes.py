...
from anji_orm import query, StringField, DatetimeField, IntField, EnumField
...
class Node(ManagedModel):
    ...
    def free_node_tasks(self):
        ...
        if self.type == NodeType.worker:
            ...
            return
        tasks_to_free = query(task_class.build_query(dict(
    ...
    def create_node_from_bot(cls, bot):
        ...
        node_record = query(cls.build_query({'technical_name': node_name}).limit(1))
        if not node_record:
            node_record = cls()
        else:
