...
from anji_orm import query, Model
...
class SecurityGuard(object):
    ...
    def update_from_storage(self):
        ...
        self.bot[SECURITY_COMMAND_RULES_STORAGE_KEY] = Model.prettify_value(self._secutiry_command_rules)
    ...
    def check_security_rule(self, user_technical_name, protection_level):
        ...
        if protection_level == SecurityModel.admin_only:
            ...
            user = query(self.bot.manager_env['users'].build_query(dict(technical_name=user_technical_name)))
            user = user[0]
