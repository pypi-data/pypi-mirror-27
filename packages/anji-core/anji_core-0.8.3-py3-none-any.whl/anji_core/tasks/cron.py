...
from anji_orm import query, StringField, DatetimeField, FloatField, EnumField
import humanize
...
class CronCartridge(AbstractCartridge):
    ...
    def cron_reset(self, mess, technical_names=None, auto=None):
        ...
        if technical_names:
            cron_tasks = query(CronTask.build_query(dict(status=TaskStatus.in_progress, technical_name=technical_names)))
...
class CronManager(SecurityManager):
    ...
    def get_stuck_cron_tasks(self, rms_coef=2):
        cron_tasks = query(CronTask.build_query(dict(status=TaskStatus.in_progress)))
