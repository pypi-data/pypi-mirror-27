...
from anji_orm import register, query
...
def anji_delayed_task(  # pylint: disable=too-many-statements
    ...
    def wrapped_func(  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        ...
        for command_argument in command_arguments:
            ...
            task = func(self, mess, **task_params)
            ...
            if _task_mode == TaskExecutionMode.normal:
                ...
                search_domains = task.build_similarity_dict()
                search_domains['status'] = TaskStatus.done
                task_query = task.build_query(search_domains).order_by(R.desc('complete_timestamp')).limit(1)
                with register.pool.connect() as conn:
                    task_dict = task_query.run(conn)
                if task_dict:
                ...
                search_domains['status'] = TaskStatus.done
                with register.pool.connect() as conn:
                    task_dicts = list(task.build_query(search_domains).run(conn))
                if task_dicts:
                    ...
                    with self.shared.render as r:
                        ...
                        with r.field_list() as lst:
                            ...
                            lst.add('Average execution time (in seconds)', average_time)
                            lst.add('Max execution time (in seconds)', max_time)
                            lst.add('Min execution time (in seconds)', min_time)
        ...
        if not task_processes:
            if external_message_link:
                mess = query(Message.get_query(external_message_link)).to_errbot_message()
            self.shared.signals.fire(
                MessageReactionSignal(
                    mess,
                    Reaction.blocked,
