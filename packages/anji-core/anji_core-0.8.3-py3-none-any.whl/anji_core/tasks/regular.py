...
from anji_orm import query, IntField, LinkField, register
import humanize
...
class RegularTask(BaseTask):  # pylint: disable=abstract-method
    ...
    def generate_technical_name() -> int:
        base_query = RegularTask.build_query(
            dict(
                status=[TaskStatus.pending, TaskStatus.in_progress, TaskStatus.delayed]
            )
        )
        with register.pool.connect() as conn:
            if base_query.count().run(conn) == 0:
                return 1
            min_number = base_query.pluck('technical_name').min().run(conn)['technical_name']
            max_number = base_query.pluck('technical_name').max().run(conn)['technical_name']
...
class TaskManager(SecurityManager):
    ...
    active_record_filter = dict(status=[TaskStatus.in_progress, TaskStatus.pending])
    ...
    def fetch_all_tasks(self) -> List[RegularTask]:
        task_query = RegularTask.build_query(
            dict(
                status=TaskStatus.pending,
                worker_hostname=[self.bot.bot_config.ANJI_HOSTNAME, '*']
            )
        )
        return query(task_query)

    def subscribe_query(self) -> R.RqlQuery:
        return RegularTask.build_query(
            dict(
                status=TaskStatus.pending,
                worker_hostname=[self.bot.bot_config.ANJI_HOSTNAME, '*']
            )
