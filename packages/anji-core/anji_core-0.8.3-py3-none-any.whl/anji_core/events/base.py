...
from anji_orm import Model, StringField, query, ListField, EnumField, DatetimeField
...
class AbstractEventConsumer(ManagedModel):
    ...
    def consume_events(self):
        events = query(AbstractEvent.build_query(dict(
            type=self.consume_event_types,
...
class EventCollectorCronTask(CronTask):
    ...
    def execute(self):
        event_collectors = query(AbstractEventCollector.build_query(dict(technical_name=self.collector_technical_names)))
...
class EventConsumerCronTask(CronTask):
    ...
    def execute(self):
        event_consumers = query(AbstractEventConsumer.build_query(dict(technical_name=self.consumer_technical_names)))
