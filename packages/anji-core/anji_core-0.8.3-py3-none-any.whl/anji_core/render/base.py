    ...
    def auto_task_result(self, body='', title=None, message_type=MessageType.warning, in_reply_to=None, reports=(), **kwargs):
        with self as r:
            ...
            with r.field_list() as lst:
                if kwargs.get('created_timestamp'):
                    lst.add('Last command start time', kwargs.get('created_timestamp'))
                if kwargs.get('complete_timestamp'):
                    lst.add('Last command finished time', kwargs.get('complete_timestamp'))
