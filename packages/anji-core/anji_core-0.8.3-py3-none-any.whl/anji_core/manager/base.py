...
from anji_orm import query, Model, process_functions, StringField, register
...
class BaseManager(metaclass=ManagerMetaclass):
    ...
    active_record_filter: Dict = dict()
    ...
    @classmethod
    def build_query(cls, group_dict: Dict) -> R.RqlQuery:
        return cls.controlled_model.build_query(group_dict)
    ...
    def record_list_fetcher(
            ...
            additional_filters: Dict = None) -> List[ManagedModel]:
        if additional_filters is None:
            additional_filters = dict()
        additional_filters.update(self.active_record_filter)
        return query(self.controlled_model.build_query(additional_filters))
