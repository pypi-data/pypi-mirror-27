from anji_orm import Model, query, RethinkModelMetaclass, StringField, register
...
class CartridgeMetaclass(RethinkModelMetaclass):
...
class CartridgeRegistry:
    ...
    def load_cartridges(self) -> None:
        ...
        for cartridge_class in filter(lambda x: x != AbstractCartridge, register.tables_model_link.get(AbstractCartridge._table)):
            cartridge_object = query(AbstractCartridge.build_query(dict(cartridge_indentity=cartridge_class.technical_name)))
            if not cartridge_object:
                cartridge_object = cartridge_class(self.bot)
