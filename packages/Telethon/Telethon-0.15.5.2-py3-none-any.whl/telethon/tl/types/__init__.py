...
class InputMediaUploadedDocument(TLObject):
    ...
    def __bytes__(self):
        return b''.join((
            b'\xfd!\x96\xe3',
            struct.pack('<I', (8 if self.nosound_video else 0) | (4 if self.thumb else 0) | (1 if self.stickers else 0) | (2 if self.ttl_seconds else 0)),
            bytes(self.file),
...
class Message(TLObject):
    ...
    def __bytes__(self):
        return b''.join((
            ...
            b'' if not self.grouped_id else (struct.pack('<q', self.grouped_id)),
        ))
...
class MessageFwdHeader(TLObject):
    ...
    def __bytes__(self):
        assert (self.saved_from_peer and self.saved_from_msg_id) or (not self.saved_from_peer and not self.saved_from_msg_id), 'saved_from_peer, saved_from_msg_id parameters must all be False-y (like None) or all me True-y'
        return b''.join((
            b'm\xbe\x9eU',
            struct.pack('<I', (1 if self.from_id else 0) | (2 if self.channel_id else 0) | (4 if self.channel_post else 0) | (8 if self.post_author else 0) | (16 if self.saved_from_peer else 0) | (16 if self.saved_from_msg_id else 0)),
            b'' if not self.from_id else (struct.pack('<i', self.from_id)),
            b'\0\0\0\0' if self.date is None else struct.pack('<I', int(self.date.timestamp())),
            ...
            b'' if not self.post_author else (TLObject.serialize_bytes(self.post_author)),
            b'' if not self.saved_from_peer else (bytes(self.saved_from_peer)),
            b'' if not self.saved_from_msg_id else (struct.pack('<i', self.saved_from_msg_id)),
        ))
